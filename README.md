...
> Summary:
> - **692** official PowerBASIC statement/function keywords currently available for use
>   - **544** implemented or completed by this fork
...
> - **164** DDT/GUI-class keywords deferred (Tier 3)
...
> Updated through batch 167 (v0.2.024). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 692 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | DESKTOP GET SIZE | GRAPHIC SCALE PIXELS | MKS$ | THREAD GET PRIORITY |
| ACCEL ATTACH | DIALOG DEFAULT FONT | GRAPHIC SET AUTOSIZE | MKWRD | THREAD RESUME |
| ACODE | DIALOG DISABLE | GRAPHIC SET BITS | MKWRD$ | THREAD SET PRIORITY |
| ACODE$ | DIALOG DOEVENTS | GRAPHIC SET CAPTION | MOD | THREAD STATUS |
| ACOS | DIALOG ENABLE | GRAPHIC SET CLIP | MONTHNAME | THREAD SUSPEND |
| ACOSH | DIALOG END | GRAPHIC SET FIXED | MONTHNAME$ | THREADCOUNT |
| AND | DIALOG GET CLIENT | GRAPHIC SET FONT | MOUSEPTR | THREADED |
| ARRAY ADD | DIALOG GET LOC | GRAPHIC SET MIX | MSGBOX | THREADID |
| ARRAY ARRAYIX | DIALOG GET SIZE | GRAPHIC SET PIXEL | NAME | TIMER |
| ARRAY ASSIGN | DIALOG GET TEXT | GRAPHIC SET POS | NOT | TIX |
| ARRAY COPY | DIALOG GET USER | GRAPHIC SET SIZE | NUL | TOOLBAR ADD BUTTON |
| ARRAY DELETE | DIALOG HIDE | GRAPHIC SET STRETCHMODE | NUL$ | TOOLBAR ADD SEPARATOR |
| ARRAY INSERT | DIALOG MAXIMIZE | GRAPHIC SET TEXTALIGN | OBJECT | TOOLBAR DELETE BUTTON |
| ARRAY REDIM DECR | DIALOG MINIMIZE | GRAPHIC SET VIEW | OCT | TOOLBAR GET COUNT |
| ARRAY REDIM INCR | DIALOG NEW | GRAPHIC SET VIRTUAL | OCT$ | TOOLBAR GET STATE |
| ARRAY REVERSE | DIALOG NONSTABLE | GRAPHIC SET WORDWRAP | OEMTOCHR | TOOLBAR SET IMAGELIST |
| ARRAY SCAN | DIALOG NORMALIZE | GRAPHIC SET WRAP | OEMTOCHR$ | TOOLBAR SET STATE |
| ARRAY SHUFFLE | DIALOG PIXELS | GRAPHIC STYLE | ON CALL | TRACE |
| ARRAY SORT | DIALOG POST | GRAPHIC TEXT SIZE | ON ERROR | TREEVIEW DELETE |
| ARRAY SWAP | DIALOG REDRAW | GRAPHIC WIDTH | ON GOSUB | TREEVIEW GET COUNT |
| ARRAY TAGARRAY | DIALOG SEND | HEADER GET COUNT | ON GOTO | TREEVIEW GET TEXT |
| ARRAY TAGARRAY ERASE | DIALOG SET CLIENT | HEADER GET ITEM | OPEN | TREEVIEW INSERT ITEM |
| ARRAY UNIQUE | DIALOG SET COLOR | HEADER SEND | OPTION EXPLICIT | TREEVIEW RESET |
| ASC | DIALOG SET ICON | HEADER SET ITEM | OR | TRIM |
| ASIN | DIALOG SET LOC | HEX | PARSE | TRIM$ |
| ASINH | DIALOG SET SIZE | HEX$ | PARSE$ | TRUNC |
| ASM | DIALOG SET TEXT | HI | PARSECOUNT | TRY/END TRY |
| ASMDATA / END ASMDATA | DIALOG SET USER | HIWRD | PATHNAME | TYPE SET |
| ATANH | DIALOG SHOW MODAL | HOST ADDR | PATHNAME$ | TYPE/END TYPE |
| ATN | DIALOG SHOW MODELESS | HOST NAME | PATHSCAN | UCASE |
| ATN2 | DIALOG SHOW STATE | HYPOT | PATHSCAN$ | UCASE$ |
| BEEP | DIALOG STABILIZE | IF | PEEK | UCODE$ |
| BGR | DIALOG UNITS | IF/END IF | PLAY | UCODEPAGE |
| BIN | DIM | IIF | PLAY SOUND | UDP CLOSE |
| BIN$ | DIR | IMAGELIST COUNT | PLAY WAVE | UDP NOTIFY |
| BIT | DIR CLOSE | IMAGELIST GET COUNT | POKE | UDP OPEN |
| BIT CALC | DIR$ | IMAGELIST KILL | PREFIX | UDP RECV |
| BITS | DISKFREE | IMAGELIST NEW | PRINT# | UDP SEND |
| BITS$ | DISKSIZE | IMAGELIST NEW BITMAP | PRINTER$ | UNLOCK |
| BITSE | DISPLAY BROWSE | IMP | PRINTERCOUNT | UNWRAP |
| BUILD | DISPLAY COLOR | IMPORT | PROCESS GET PRIORITY | UNWRAP$ |
| BUILD$ | DISPLAY FONT | INCR | PROCESS SET PRIORITY | USING |
| BYTE | DISPLAY OPENFILE | INPUT FLUSH | PROFILE | USING$ |
| CALL | DISPLAY SAVEFILE | INPUT# | PROGRESSBAR GET POS | UTF8TOCHR |
| CALL DWORD | DOUBLE | INSTANCE | PROGRESSBAR GET RANGE | UTF8TOCHR$ |
| CALLSTK | END | INSTR | PROGRESSBAR SET POS | VAL |
| CALLSTK$ | ENVIRON | INT | PROGRESSBAR SET RANGE | VARPTR |
| CALLSTKCOUNT | ENVIRON$ | INTEGER | PROGRESSBAR SET STEP | VERIFY |
| CBOOL | EOF | INTERFACE / END INTERFACE (DIRECT) | PROGRESSBAR STEP | WAITKEY |
| CBRT | EQV | INTERFACE/END INTERFACE (IDBIND) | PUT | WAITKEY$ |
| CBYTE | ERASE | ISEVEN | PUT STR | WINDOW GET |
| CDBL | ERF | ISFALSE | PUT WSTR | WINDOW SET |
| CDWORD | ERL | ISFILE | PUT$ | WORD |
| CEIL | ERL$ | ISFOLDER | PUT$$ | WRAP |
| CFLT | ERR | ISINFINITE | QUAD | WRAP$ |
| CHDIR | ERRCLEAR | ISNORMAL | RAISEEVENT | WRITE |
| CHDRIVE | ERROR | ISODD | RANDOMIZE | WRITE# |
| CHOOSE | ERROR$ | ISTRUE | READ | XOR |
| CHR | EVENT SOURCE | ISWIN | READ$ | XPRINT ARC |
| CHR$ | EVENTS | ITERATE | REDIM | XPRINT ATTACH |
| CHRBYTES | EXE | JOIN$ | REGEXPR | XPRINT BOX |
| CHRTOOEM | EXIST | KILL | REGISTER | XPRINT CANCEL |
| CHRTOOEM$ | EXIT | LCASE | REGREPL | XPRINT CELL |
| CHRTOUTF8 | EXP | LCASE$ | REM | XPRINT CELL SIZE |
| CHRTOUTF8$ | EXP10 | LEFT | REMAIN | XPRINT CHR SIZE |
| CLASS/END CLASS | EXP2 | LEFT$ | REMAIN$ | XPRINT CLOSE |
| CLIP | EXPM1 | LEN | REMOVE | XPRINT COLOR |
| CLIP$ | EXTRACT | LET | REMOVE$ | XPRINT COPY |
| CLIPBOARD | EXTRACT$ | LET *(WITH OBJECTS)* | REPEAT | XPRINT ELLIPSE |
| CLOSE | FIELD | LET *(WITH TYPES)* | REPEAT$ | XPRINT FORMFEED |
| CLS | FILEATTR | LET *(WITH VARIANTS)* | REPLACE | XPRINT GET ATTACH |
| CODEPTR | FILECOPY | LINE INPUT# | RESET | XPRINT GET CANVAS |
| COLOR | FILENAME | LISTBOX ADD | RESOURCE SAVE FILE | XPRINT GET CLIENT |
| COMBOBOX ADD | FILENAME$ | LISTVIEW DELETE ITEM | RESOURCE$ | XPRINT GET CLIP |
| COMM CLOSE | FILESCAN | LISTVIEW GET COUNT | RESUME | XPRINT GET COLLATE |
| COMM LINE | FIX | LISTVIEW GET TEXT | RETAIN | XPRINT GET COLOR |
| COMM LINE INPUT | FLOOR | LISTVIEW INSERT COLUMN | RETAIN$ | XPRINT GET COLORMODE |
| COMM OPEN | FLUSH | LISTVIEW INSERT ITEM | RETURN | XPRINT GET COPIES |
| COMM PRINT | FONT END | LISTVIEW RESET | RGB | XPRINT GET DC |
| COMM RECV | FONT NEW | LISTVIEW SET TEXT | RIGHT | XPRINT GET DUPLEX |
| COMM RESET | FOR / NEXT | LO | RIGHT$ | XPRINT GET LINES |
| COMM SEND | FORMAT | LOC | RMDIR | XPRINT GET MARGIN |
| COMM SET | FORMAT$ | LOCAL | RND | XPRINT GET MIX |
| COMM TIMEOUT | FRAC | LOCK | ROTATE | XPRINT GET ORIENTATION |
| COMMAND | FRE | LOF | ROUND | XPRINT GET OVERLAP |
| COMMAND$ | FREEFILE | LOG | RSET | XPRINT GET PAGES |
| CONTROL ADD | FUNCNAME | LOG10 | RSET$ | XPRINT GET PAPER |
| CONTROL ADD BUTTON | FUNCNAME$ | LOG1P | RTRIM | XPRINT GET PAPERS |
| CONTROL ADD CHECKBOX | FUNCTION / END FUNCTION | LOG2 | RTRIM$ | XPRINT GET PIXEL |
| CONTROL ADD COMBOBOX | GET | LONG | SEC | XPRINT GET POS |
| CONTROL ADD LABEL | GET STR | LOWRD | SECH | XPRINT GET PPI |
| CONTROL ADD LISTBOX | GET WSTR | LPRINT | SEEK | XPRINT GET QUALITY |
| CONTROL ADD LISTVIEW | GET$ | LPRINT ATTACH | SELECT CASE/END SELECT | XPRINT GET SCALE |
| CONTROL ADD PROGRESSBAR | GET$$ | LPRINT CLOSE | SETATTR | XPRINT GET SELECTION |
| CONTROL ADD SCROLLBAR | GETATTR | LPRINT FLUSH | SETEOF | XPRINT GET SIZE |
| CONTROL ADD STATUSBAR | GLOBAL | LPRINT FORMFEED | SGN | XPRINT GET STRETCHMODE |
| CONTROL ADD TOOLBAR | GLOBALMEM | LPRINT$ | SHELL | XPRINT GET TEXTALIGN |
| CONTROL ADD TREEVIEW | GRAPHIC ARC | LSET | SHIFT | XPRINT GET TRAY |
| CONTROL DISABLE | GRAPHIC ATTACH | LSET$ | SHRINK | XPRINT GET TRAYS |
| CONTROL ENABLE | GRAPHIC BITMAP END | LTRIM | SHRINK$ | XPRINT GET WORDWRAP |
| CONTROL GET CHECK | GRAPHIC BITMAP LOAD | LTRIM$ | SIN | XPRINT GET WRAP |
| CONTROL GET TEXT | GRAPHIC BITMAP NEW | MACRO/END MACRO | SINGLE | XPRINT IMAGELIST |
| CONTROL HIDE | GRAPHIC BOX | MAK | SINH | XPRINT LINE |
| CONTROL KILL | GRAPHIC CELL | MAT | SIZEOF | XPRINT PIE |
| CONTROL SET CHECK | GRAPHIC CELL SIZE | MAX | SLEEP | XPRINT POLYGON |
| CONTROL SET TEXT | GRAPHIC CHR SIZE | MCASE | SPACE | XPRINT POLYLINE |
| COS | GRAPHIC CIRCLE | MCASE$ | SPACE$ | XPRINT PREVIEW |
| COSH | GRAPHIC CLEAR | MEMORY | SPLIT | XPRINT PRINT |
| COT | GRAPHIC COLOR | MEMORY FILL | SQR | XPRINT RENDER |
| COTH | GRAPHIC COPY | MEMORY FILLS | STATIC | XPRINT SCALE |
| CQUAD | GRAPHIC DETACH | MENU ADD POPUP | STATUSBAR SET PARTS | XPRINT SET CLIP |
| CSC | GRAPHIC ELLIPSE | MENU ADD STRING | STATUSBAR SET TEXT | XPRINT SET COLLATE |
| CSCH | GRAPHIC GET BITS | MENU DELETE | STR | XPRINT SET COLOR |
| CSET | GRAPHIC GET CANVAS | MENU GET STATE | STR$ | XPRINT SET COLORMODE |
| CSET$ | GRAPHIC GET CAPTION | MENU GET TEXT | STRDELETE | XPRINT SET COPIES |
| CSNG | GRAPHIC GET CLIENT | MENU NEW BAR | STRDELETE$ | XPRINT SET DUPLEX |
| CSTR | GRAPHIC GET CLIP | MENU NEW POPUP | STRING | XPRINT SET FONT |
| CULNG | GRAPHIC GET DC | MENU SET STATE | STRINSERT | XPRINT SET MIX |
| CURDIR | GRAPHIC GET LINES | MENU SET TEXT | STRINSERT$ | XPRINT SET ORIENTATION |
| CURDIR$ | GRAPHIC GET LOC | METHOD / END METHOD | STRPTR | XPRINT SET OVERLAP |
| CVBYT | GRAPHIC GET MIX | METRICS | STRREVERSE | XPRINT SET PAGES |
| CVCUX | GRAPHIC GET PIXEL | MID | STRREVERSE$ | XPRINT SET PAPER |
| CVDWD | GRAPHIC GET POS | MID$ | SWAP | XPRINT SET PIXEL |
| CVL | GRAPHIC GET PPI | MIN | SWITCH | XPRINT SET POS |
| CVQ | GRAPHIC GET SCALE | MKBYT | SWITCH$ | XPRINT SET QUALITY |
| CVW | GRAPHIC GET SIZE | MKBYT$ | TAB$ | XPRINT SET STRETCHMODE |
| CWORD | GRAPHIC GET STRETCHMODE | MKCUR$ | TALLY | XPRINT SET TEXTALIGN |
| DATA | GRAPHIC GET TEXTALIGN | MKCUX | TAN | XPRINT SET TRAY |
| DATACOUNT | GRAPHIC GET VIEW | MKCUX$ | TANH | XPRINT SET WORDWRAP |
| DAYNAME | GRAPHIC GET WORDWRAP | MKD$ | TCP ACCEPT | XPRINT SET WRAP |
| DAYNAME$ | GRAPHIC GET WRAP | MKDIR | TCP CLOSE | XPRINT SPLIT |
| DEC | GRAPHIC LINE | MKDWD | TCP LINE INPUT | XPRINT STRETCH |
| DEC$ | GRAPHIC PAINT | MKDWD$ | TCP NOTIFY | XPRINT STYLE |
| DECLARE | GRAPHIC PIE | MKE | TCP OPEN | XPRINT TEXT SIZE |
| DECR | GRAPHIC POLYGON | MKE$ | TCP PRINT | XPRINT WIDTH |
| DEF | GRAPHIC POLYLINE | MKI$ | TCP RECV |  |
| DESKTOP GET CLIENT | GRAPHIC PRINT | MKL$ | TCP SEND |  |
| DESKTOP GET LOC | GRAPHIC SAVE | MKQ$ | THREAD CLOSE |  |
| DESKTOP GET PPI | GRAPHIC SCALE | MKS | THREAD CREATE |  |

## **Newly** implemented by this branch

> **544 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DIALOG SHOW MODELESS | INT | SQR |
| ACCEL ATTACH | DIALOG SHOW STATE | INTEGER | STATUSBAR SET PARTS |
| ACODE | DIALOG STABILIZE | ISEVEN | STATUSBAR SET TEXT |
| ACODE$ | DIALOG UNITS | ISFALSE | STR |
| ACOS | DIM | ISFILE | STR$ |
| ACOSH | DIR | ISFOLDER | STRDELETE |
| ARRAY ADD | DIR CLOSE | ISINFINITE | STRDELETE$ |
| ARRAY ARRAYIX | DIR$ | ISNORMAL | STRING |
| ARRAY ASSIGN | DISKFREE | ISODD | STRINSERT |
| ARRAY COPY | DISKSIZE | ISTRUE | STRINSERT$ |
| ARRAY DELETE | DISPLAY BROWSE | ISWIN | STRPTR |
| ARRAY INSERT | DISPLAY COLOR | JOIN$ | STRREVERSE |
| ARRAY REDIM DECR | DISPLAY FONT | LCASE | STRREVERSE$ |
| ARRAY REDIM INCR | DISPLAY OPENFILE | LCASE$ | SWITCH |
| ARRAY REVERSE | DISPLAY SAVEFILE | LEFT | SWITCH$ |
| ARRAY SCAN | DOUBLE | LEFT$ | TAB$ |
| ARRAY SHUFFLE | ENVIRON$ | LEN | TALLY |
| ARRAY SWAP | EOF | LET *(WITH OBJECTS)* | TAN |
| ARRAY TAGARRAY | EQV | LET *(WITH VARIANTS)* | TANH |
| ARRAY TAGARRAY ERASE | ERF | LINE INPUT# | TCP NOTIFY |
| ARRAY UNIQUE | ERL | LISTBOX ADD | THREADCOUNT |
| ASIN | ERL$ | LISTVIEW DELETE ITEM | THREADID |
| ASINH | ERR | LISTVIEW GET COUNT | TIMER |
| ATANH | ERRCLEAR | LISTVIEW GET TEXT | TOOLBAR ADD BUTTON |
| ATN | ERROR$ | LISTVIEW INSERT COLUMN | TOOLBAR ADD SEPARATOR |
| ATN2 | EVENT SOURCE | LISTVIEW INSERT ITEM | TOOLBAR DELETE BUTTON |
| BGR | EVENTS | LISTVIEW RESET | TOOLBAR GET COUNT |
| BIN | EXE | LISTVIEW SET TEXT | TOOLBAR GET STATE |
| BIN$ | EXIST | LO | TOOLBAR SET IMAGELIST |
| BITS | EXP | LOC | TOOLBAR SET STATE |
| BITS$ | EXP10 | LOF | TREEVIEW DELETE |
| BITSE | EXP2 | LOG | TREEVIEW GET COUNT |
| BUILD | EXPM1 | LOG10 | TREEVIEW GET TEXT |
| BUILD$ | EXTRACT | LOG1P | TREEVIEW INSERT ITEM |
| BYTE | EXTRACT$ | LOG2 | TREEVIEW RESET |
| CALLSTK$ | FILEATTR | LONG | TRIM |
| CALLSTKCOUNT | FILENAME | LOWRD | TRIM$ |
| CBOOL | FILENAME$ | LPRINT$ | TRUNC |
| CBRT | FIX | LSET$ | UCASE |
| CBYTE | FLOOR | LTRIM | UCASE$ |
| CDBL | FONT END | LTRIM$ | UCODE$ |
| CDWORD | FONT NEW | MAK | UCODEPAGE |
| CEIL | FORMAT | MAX | UDP NOTIFY |
| CFLT | FORMAT$ | MCASE | UNWRAP |
| CHOOSE | FRAC | MCASE$ | UNWRAP$ |
| CHR | FRE | MEMORY FILL | USING |
| CHR$ | FREEFILE | MEMORY FILLS | USING$ |
| CHRBYTES | FUNCNAME | MENU ADD POPUP | UTF8TOCHR |
| CHRTOOEM | FUNCNAME$ | MENU ADD STRING | UTF8TOCHR$ |
| CHRTOOEM$ | GET | MENU DELETE | VARPTR |
| CHRTOUTF8 | GET STR | MENU GET STATE | VERIFY |
| CHRTOUTF8$ | GET WSTR | MENU GET TEXT | WAITKEY |
| CLIP | GET$ | MENU NEW BAR | WAITKEY$ |
| CLIP$ | GET$$ | MENU NEW POPUP | WORD |
| CLOSE | GETATTR | MENU SET STATE | WRAP |
| CODEPTR | GRAPHIC ARC | MENU SET TEXT | WRAP$ |
| COMBOBOX ADD | GRAPHIC ATTACH | METRICS | WRITE |
| COMM LINE INPUT | GRAPHIC BITMAP END | MID | WRITE# |
| COMMAND | GRAPHIC BITMAP LOAD | MIN | XPRINT ARC |
| COMMAND$ | GRAPHIC BITMAP NEW | MKBYT | XPRINT ATTACH |
| CONTROL ADD | GRAPHIC BOX | MKCUX | XPRINT BOX |
| CONTROL ADD BUTTON | GRAPHIC CELL | MKDWD | XPRINT CANCEL |
| CONTROL ADD CHECKBOX | GRAPHIC CELL SIZE | MKE | XPRINT CELL |
| CONTROL ADD COMBOBOX | GRAPHIC CHR SIZE | MKS | XPRINT CELL SIZE |
| CONTROL ADD LABEL | GRAPHIC CIRCLE | MKWRD | XPRINT CHR SIZE |
| CONTROL ADD LISTBOX | GRAPHIC CLEAR | MOD | XPRINT CLOSE |
| CONTROL ADD LISTVIEW | GRAPHIC COLOR | MONTHNAME | XPRINT COLOR |
| CONTROL ADD PROGRESSBAR | GRAPHIC COPY | MONTHNAME$ | XPRINT COPY |
| CONTROL ADD SCROLLBAR | GRAPHIC DETACH | NUL$ | XPRINT ELLIPSE |
| CONTROL ADD STATUSBAR | GRAPHIC ELLIPSE | OCT | XPRINT FORMFEED |
| CONTROL ADD TOOLBAR | GRAPHIC GET BITS | OCT$ | XPRINT GET ATTACH |
| CONTROL ADD TREEVIEW | GRAPHIC GET CANVAS | OEMTOCHR | XPRINT GET CANVAS |
| CONTROL DISABLE | GRAPHIC GET CAPTION | OEMTOCHR$ | XPRINT GET CLIENT |
| CONTROL ENABLE | GRAPHIC GET CLIENT | ON CALL | XPRINT GET CLIP |
| CONTROL GET CHECK | GRAPHIC GET CLIP | ON GOSUB | XPRINT GET COLLATE |
| CONTROL GET TEXT | GRAPHIC GET DC | ON GOTO | XPRINT GET COLOR |
| CONTROL HIDE | GRAPHIC GET LINES | OPEN | XPRINT GET COLORMODE |
| CONTROL KILL | GRAPHIC GET LOC | PARSE$ | XPRINT GET COPIES |
| CONTROL SET CHECK | GRAPHIC GET MIX | PARSECOUNT | XPRINT GET DC |
| CONTROL SET TEXT | GRAPHIC GET PIXEL | PATHNAME | XPRINT GET DUPLEX |
| COS | GRAPHIC GET POS | PATHNAME$ | XPRINT GET LINES |
| COSH | GRAPHIC GET PPI | PATHSCAN | XPRINT GET MARGIN |
| COT | GRAPHIC GET SCALE | PATHSCAN$ | XPRINT GET MIX |
| COTH | GRAPHIC GET SIZE | PEEK | XPRINT GET ORIENTATION |
| CQUAD | GRAPHIC GET STRETCHMODE | PLAY | XPRINT GET OVERLAP |
| CSC | GRAPHIC GET TEXTALIGN | PLAY SOUND | XPRINT GET PAGES |
| CSCH | GRAPHIC GET VIEW | POKE | XPRINT GET PAPER |
| CSET$ | GRAPHIC GET WORDWRAP | PRINTER$ | XPRINT GET PAPERS |
| CSNG | GRAPHIC GET WRAP | PRINTERCOUNT | XPRINT GET PIXEL |
| CSTR | GRAPHIC LINE | PROGRESSBAR GET POS | XPRINT GET POS |
| CULNG | GRAPHIC PAINT | PROGRESSBAR GET RANGE | XPRINT GET PPI |
| CURDIR | GRAPHIC PIE | PROGRESSBAR SET POS | XPRINT GET QUALITY |
| CURDIR$ | GRAPHIC POLYGON | PROGRESSBAR SET RANGE | XPRINT GET SCALE |
| CVBYT | GRAPHIC POLYLINE | PROGRESSBAR SET STEP | XPRINT GET SELECTION |
| CVCUX | GRAPHIC PRINT | PROGRESSBAR STEP | XPRINT GET SIZE |
| CVDWD | GRAPHIC SAVE | PUT | XPRINT GET STRETCHMODE |
| CVL | GRAPHIC SCALE | PUT STR | XPRINT GET TEXTALIGN |
| CVQ | GRAPHIC SCALE PIXELS | PUT WSTR | XPRINT GET TRAY |
| CVW | GRAPHIC SET AUTOSIZE | PUT$ | XPRINT GET TRAYS |
| CWORD | GRAPHIC SET BITS | PUT$$ | XPRINT GET WORDWRAP |
| DATACOUNT | GRAPHIC SET CAPTION | QUAD | XPRINT GET WRAP |
| DAYNAME | GRAPHIC SET CLIP | RAISEEVENT | XPRINT IMAGELIST |
| DAYNAME$ | GRAPHIC SET FIXED | READ | XPRINT LINE |
| DEC | GRAPHIC SET FONT | READ$ | XPRINT PIE |
| DEC$ | GRAPHIC SET MIX | REDIM | XPRINT POLYGON |
| DECLARE | GRAPHIC SET PIXEL | REMAIN | XPRINT POLYLINE |
| DEF | GRAPHIC SET POS | REMAIN$ | XPRINT PREVIEW |
| DESKTOP GET PPI | GRAPHIC SET SIZE | REMOVE | XPRINT PRINT |
| DIALOG DEFAULT FONT | GRAPHIC SET STRETCHMODE | REMOVE$ | XPRINT RENDER |
| DIALOG DISABLE | GRAPHIC SET TEXTALIGN | REPEAT | XPRINT SCALE |
| DIALOG DOEVENTS | GRAPHIC SET VIEW | REPEAT$ | XPRINT SET CLIP |
| DIALOG ENABLE | GRAPHIC SET VIRTUAL | RESOURCE SAVE FILE | XPRINT SET COLLATE |
| DIALOG END | GRAPHIC SET WORDWRAP | RESOURCE$ | XPRINT SET COLOR |
| DIALOG GET CLIENT | GRAPHIC SET WRAP | RETAIN | XPRINT SET COLORMODE |
| DIALOG GET LOC | GRAPHIC STYLE | RETAIN$ | XPRINT SET COPIES |
| DIALOG GET SIZE | GRAPHIC TEXT SIZE | RGB | XPRINT SET DUPLEX |
| DIALOG GET TEXT | GRAPHIC WIDTH | RIGHT | XPRINT SET FONT |
| DIALOG GET USER | HEADER GET COUNT | RIGHT$ | XPRINT SET MIX |
| DIALOG HIDE | HEADER GET ITEM | RND | XPRINT SET ORIENTATION |
| DIALOG MAXIMIZE | HEADER SEND | ROUND | XPRINT SET OVERLAP |
| DIALOG MINIMIZE | HEADER SET ITEM | RSET$ | XPRINT SET PAGES |
| DIALOG NEW | HEX | RTRIM | XPRINT SET PAPER |
| DIALOG NONSTABLE | HEX$ | RTRIM$ | XPRINT SET PIXEL |
| DIALOG NORMALIZE | HI | SEC | XPRINT SET POS |
| DIALOG PIXELS | HIWRD | SECH | XPRINT SET QUALITY |
| DIALOG POST | HYPOT | SEEK | XPRINT SET STRETCHMODE |
| DIALOG REDRAW | IIF | SETEOF | XPRINT SET TEXTALIGN |
| DIALOG SEND | IMAGELIST COUNT | SGN | XPRINT SET TRAY |
| DIALOG SET CLIENT | IMAGELIST GET COUNT | SHRINK | XPRINT SET WORDWRAP |
| DIALOG SET COLOR | IMAGELIST KILL | SHRINK$ | XPRINT SET WRAP |
| DIALOG SET ICON | IMAGELIST NEW | SIN | XPRINT SPLIT |
| DIALOG SET LOC | IMAGELIST NEW BITMAP | SINGLE | XPRINT STRETCH |
| DIALOG SET SIZE | IMP | SINH | XPRINT STYLE |
| DIALOG SET TEXT | INPUT# | SIZEOF | XPRINT TEXT SIZE |
| DIALOG SET USER | INSTANCE | SPACE | XPRINT WIDTH |
| DIALOG SHOW MODAL | INSTR | SPACE$ |  |
