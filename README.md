...
> Summary:
> - **842** official PowerBASIC statement/function keywords currently available for use
>   - **694** implemented or completed by this fork
...
> - **13** DDT/GUI-class keywords deferred (Tier 3)
...
> Updated through batch 181 (v0.2.038). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 842 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | CVBYT | GRAPHIC SET LOC | MENU NEW POPUP | TALLY |
| ACCEL ATTACH | CVCUX | GRAPHIC SET MIX | MENU SET STATE | TAN |
| ACODE | CVDWD | GRAPHIC SET OVERLAP | MENU SET TEXT | TANH |
| ACODE$ | CVL | GRAPHIC SET PIXEL | METHOD / END METHOD | TCP ACCEPT |
| ACOS | CVQ | GRAPHIC SET POS | METRICS | TCP CLOSE |
| ACOSH | CVW | GRAPHIC SET SIZE | MID | TCP LINE INPUT |
| AND | CWORD | GRAPHIC SET STRETCHMODE | MID$ | TCP NOTIFY |
| ARRAY ADD | DATA | GRAPHIC SET TEXTALIGN | MIN | TCP OPEN |
| ARRAY ARRAYIX | DATACOUNT | GRAPHIC SET VIEW | MKBYT | TCP PRINT |
| ARRAY ASSIGN | DAYNAME | GRAPHIC SET VIRTUAL | MKBYT$ | TCP RECV |
| ARRAY COPY | DAYNAME$ | GRAPHIC SET WORDWRAP | MKCUR$ | TCP SEND |
| ARRAY DELETE | DEC | GRAPHIC SET WRAP | MKCUX | THREAD CLOSE |
| ARRAY INSERT | DEC$ | GRAPHIC STYLE | MKCUX$ | THREAD CREATE |
| ARRAY REDIM DECR | DECLARE | GRAPHIC TEXT SIZE | MKD$ | THREAD GET PRIORITY |
| ARRAY REDIM INCR | DECR | GRAPHIC WIDTH | MKDIR | THREAD RESUME |
| ARRAY REVERSE | DEF | GRAPHIC WINDOW | MKDWD | THREAD SET PRIORITY |
| ARRAY SCAN | DESKTOP GET CLIENT | GRAPHIC WINDOW CLICK | MKDWD$ | THREAD STATUS |
| ARRAY SHUFFLE | DESKTOP GET LOC | GRAPHIC WINDOW END | MKE | THREAD SUSPEND |
| ARRAY SORT | DESKTOP GET PPI | GRAPHIC WINDOW HIDE | MKE$ | THREADCOUNT |
| ARRAY SWAP | DESKTOP GET SIZE | GRAPHIC WINDOW MINIMIZE | MKI$ | THREADED |
| ARRAY TAGARRAY | DIALOG DEFAULT FONT | GRAPHIC WINDOW NONSTABLE | MKL$ | THREADID |
| ARRAY TAGARRAY ERASE | DIALOG DISABLE | GRAPHIC WINDOW NORMALIZE | MKQ$ | TIMER |
| ARRAY UNIQUE | DIALOG DOEVENTS | GRAPHIC WINDOW STABILIZE | MKS | TIX |
| ASC | DIALOG ENABLE | HEADER GET COUNT | MKS$ | TOOLBAR ADD BUTTON |
| ASIN | DIALOG END | HEADER GET ITEM | MKWRD | TOOLBAR ADD SEPARATOR |
| ASINH | DIALOG GET CLIENT | HEADER SEND | MKWRD$ | TOOLBAR DELETE BUTTON |
| ASM | DIALOG GET LOC | HEADER SET ITEM | MOD | TOOLBAR GET COUNT |
| ASMDATA / END ASMDATA | DIALOG GET SIZE | HEX | MONTHNAME | TOOLBAR GET STATE |
| ATANH | DIALOG GET TEXT | HEX$ | MONTHNAME$ | TOOLBAR SET IMAGELIST |
| ATN | DIALOG GET USER | HI | MOUSEPTR | TOOLBAR SET STATE |
| ATN2 | DIALOG HIDE | HIWRD | MSGBOX | TRACE |
| BEEP | DIALOG MAXIMIZE | HOST ADDR | NAME | TREEVIEW DELETE |
| BGR | DIALOG MINIMIZE | HOST NAME | NOT | TREEVIEW GET BOLD |
| BIN | DIALOG NEW | HYPOT | NUL | TREEVIEW GET CHECK |
| BIN$ | DIALOG NONSTABLE | IF | NUL$ | TREEVIEW GET CHILD |
| BIT | DIALOG NORMALIZE | IF/END IF | OBJECT | TREEVIEW GET COUNT |
| BIT CALC | DIALOG PIXELS | IIF | OCT | TREEVIEW GET EXPANDED |
| BITS | DIALOG POST | IMAGELIST ADD BITMAP | OCT$ | TREEVIEW GET NEXT |
| BITS$ | DIALOG REDRAW | IMAGELIST ADD ICON | OEMTOCHR | TREEVIEW GET PARENT |
| BITSE | DIALOG SEND | IMAGELIST ADD MASKED | OEMTOCHR$ | TREEVIEW GET PREVIOUS |
| BUILD | DIALOG SET CLIENT | IMAGELIST COUNT | ON CALL | TREEVIEW GET ROOT |
| BUILD$ | DIALOG SET COLOR | IMAGELIST GET COUNT | ON ERROR | TREEVIEW GET SELECT |
| BYTE | DIALOG SET ICON | IMAGELIST KILL | ON GOSUB | TREEVIEW GET TEXT |
| CALL | DIALOG SET LOC | IMAGELIST NEW | ON GOTO | TREEVIEW GET USER |
| CALL DWORD | DIALOG SET SIZE | IMAGELIST NEW BITMAP | OPEN | TREEVIEW INSERT ITEM |
| CALLSTK | DIALOG SET TEXT | IMAGELIST NEW ICON | OPTION EXPLICIT | TREEVIEW RESET |
| CALLSTK$ | DIALOG SET USER | IMAGELIST SET OVERLAY | OR | TREEVIEW SELECT |
| CALLSTKCOUNT | DIALOG SHOW MODAL | IMP | PARSE | TREEVIEW SET BOLD |
| CBOOL | DIALOG SHOW MODELESS | IMPORT | PARSE$ | TREEVIEW SET CHECK |
| CBRT | DIALOG SHOW STATE | INCR | PARSECOUNT | TREEVIEW SET EXPANDED |
| CBYTE | DIALOG STABILIZE | INPUT FLUSH | PATHNAME | TREEVIEW SET IMAGELIST |
| CDBL | DIALOG UNITS | INPUT# | PATHNAME$ | TREEVIEW SET TEXT |
| CDWORD | DIM | INSTANCE | PATHSCAN | TREEVIEW SET USER |
| CEIL | DIR | INSTR | PATHSCAN$ | TREEVIEW UNSELECT |
| CFLT | DIR CLOSE | INT | PEEK | TRIM |
| CHDIR | DIR$ | INTEGER | PLAY | TRIM$ |
| CHDRIVE | DISKFREE | INTERFACE / END INTERFACE (DIRECT) | PLAY SOUND | TRUNC |
| CHOOSE | DISKSIZE | INTERFACE/END INTERFACE (IDBIND) | PLAY WAVE | TRY/END TRY |
| CHR | DISPLAY BROWSE | ISEVEN | POKE | TYPE SET |
| CHR$ | DISPLAY COLOR | ISFALSE | PREFIX | TYPE/END TYPE |
| CHRBYTES | DISPLAY FONT | ISFILE | PRINT# | UCASE |
| CHRTOOEM | DISPLAY OPENFILE | ISFOLDER | PRINTER$ | UCASE$ |
| CHRTOOEM$ | DISPLAY SAVEFILE | ISINFINITE | PRINTERCOUNT | UCODE$ |
| CHRTOUTF8 | DOUBLE | ISNORMAL | PROCESS GET PRIORITY | UCODEPAGE |
| CHRTOUTF8$ | END | ISODD | PROCESS SET PRIORITY | UDP CLOSE |
| CLASS/END CLASS | ENVIRON | ISTRUE | PROFILE | UDP NOTIFY |
| CLIP | ENVIRON$ | ISWIN | PROGRESSBAR GET POS | UDP OPEN |
| CLIP$ | EOF | ITERATE | PROGRESSBAR GET RANGE | UDP RECV |
| CLIPBOARD | EQV | JOIN$ | PROGRESSBAR SET POS | UDP SEND |
| CLOSE | ERASE | KILL | PROGRESSBAR SET RANGE | UNLOCK |
| CLS | ERF | LCASE | PROGRESSBAR SET STEP | UNWRAP |
| CODEPTR | ERL | LCASE$ | PROGRESSBAR STEP | UNWRAP$ |
| COLOR | ERL$ | LEFT | PUT | USING |
| COMBOBOX ADD | ERR | LEFT$ | PUT STR | USING$ |
| COMBOBOX DELETE | ERRCLEAR | LEN | PUT WSTR | UTF8TOCHR |
| COMBOBOX FIND | ERROR | LET | PUT$ | UTF8TOCHR$ |
| COMBOBOX FIND EXACT | ERROR$ | LET *(WITH OBJECTS)* | PUT$$ | VAL |
| COMBOBOX GET COUNT | EVENT SOURCE | LET *(WITH TYPES)* | QUAD | VARPTR |
| COMBOBOX GET SELCOUNT | EVENTS | LET *(WITH VARIANTS)* | RAISEEVENT | VERIFY |
| COMBOBOX GET SELECT | EXE | LINE INPUT# | RANDOMIZE | WAITKEY |
| COMBOBOX GET STATE | EXIST | LISTBOX ADD | READ | WAITKEY$ |
| COMBOBOX GET TEXT | EXIT | LISTBOX DELETE | READ$ | WINDOW GET |
| COMBOBOX GET USER | EXP | LISTBOX FIND | REDIM | WINDOW SET |
| COMBOBOX INSERT | EXP10 | LISTBOX FIND EXACT | REGEXPR | WORD |
| COMBOBOX RESET | EXP2 | LISTBOX GET COUNT | REGISTER | WRAP |
| COMBOBOX SELECT | EXPM1 | LISTBOX GET SELCOUNT | REGREPL | WRAP$ |
| COMBOBOX SET TEXT | EXTRACT | LISTBOX GET SELECT | REM | WRITE |
| COMBOBOX SET USER | EXTRACT$ | LISTBOX GET STATE | REMAIN | WRITE# |
| COMBOBOX UNSELECT | FIELD | LISTBOX GET TEXT | REMAIN$ | XOR |
| COMM CLOSE | FILEATTR | LISTBOX GET USER | REMOVE | XPRINT ARC |
| COMM LINE | FILECOPY | LISTBOX INSERT | REMOVE$ | XPRINT ATTACH |
| COMM LINE INPUT | FILENAME | LISTBOX RESET | REPEAT | XPRINT BOX |
| COMM OPEN | FILENAME$ | LISTBOX SELECT | REPEAT$ | XPRINT CANCEL |
| COMM PRINT | FILESCAN | LISTBOX SET TEXT | REPLACE | XPRINT CELL |
| COMM RECV | FIX | LISTBOX SET USER | RESET | XPRINT CELL SIZE |
| COMM RESET | FLOOR | LISTBOX UNSELECT | RESOURCE SAVE FILE | XPRINT CHR SIZE |
| COMM SEND | FLUSH | LISTVIEW DELETE COLUMN | RESOURCE$ | XPRINT CLOSE |
| COMM SET | FONT END | LISTVIEW DELETE ITEM | RESUME | XPRINT COLOR |
| COMM TIMEOUT | FONT NEW | LISTVIEW FIND | RETAIN | XPRINT COPY |
| COMMAND | FOR / NEXT | LISTVIEW FIND EXACT | RETAIN$ | XPRINT ELLIPSE |
| COMMAND$ | FORMAT | LISTVIEW FIT CONTENT | RETURN | XPRINT FORMFEED |
| CONTROL ADD | FORMAT$ | LISTVIEW FIT HEADER | RGB | XPRINT GET ATTACH |
| CONTROL ADD BUTTON | FRAC | LISTVIEW GET COLUMN | RIGHT | XPRINT GET CANVAS |
| CONTROL ADD CHECK3STATE | FRE | LISTVIEW GET COUNT | RIGHT$ | XPRINT GET CLIENT |
| CONTROL ADD CHECKBOX | FREEFILE | LISTVIEW GET HEADER | RMDIR | XPRINT GET CLIP |
| CONTROL ADD COMBOBOX | FUNCNAME | LISTVIEW GET HEADERID | RND | XPRINT GET COLLATE |
| CONTROL ADD FRAME | FUNCNAME$ | LISTVIEW GET MODE | ROTATE | XPRINT GET COLOR |
| CONTROL ADD GRAPHIC | FUNCTION / END FUNCTION | LISTVIEW GET SELCOUNT | ROUND | XPRINT GET COLORMODE |
| CONTROL ADD HEADER | GET | LISTVIEW GET SELECT | RSET | XPRINT GET COPIES |
| CONTROL ADD IMAGE | GET STR | LISTVIEW GET STATE | RSET$ | XPRINT GET DC |
| CONTROL ADD IMAGEX | GET WSTR | LISTVIEW GET STYLEXX | RTRIM | XPRINT GET DUPLEX |
| CONTROL ADD IMGBUTTON | GET$ | LISTVIEW GET TEXT | RTRIM$ | XPRINT GET LINES |
| CONTROL ADD IMGBUTTONX | GET$$ | LISTVIEW GET USER | SCROLLBAR GET PAGESIZE | XPRINT GET MARGIN |
| CONTROL ADD LABEL | GETATTR | LISTVIEW INSERT COLUMN | SCROLLBAR GET POS | XPRINT GET MIX |
| CONTROL ADD LINE | GLOBAL | LISTVIEW INSERT ITEM | SCROLLBAR GET RANGE | XPRINT GET ORIENTATION |
| CONTROL ADD LISTBOX | GLOBALMEM | LISTVIEW RESET | SCROLLBAR GET TRACKPOS | XPRINT GET OVERLAP |
| CONTROL ADD LISTVIEW | GRAPHIC ARC | LISTVIEW SELECT | SCROLLBAR SET PAGESIZE | XPRINT GET PAGES |
| CONTROL ADD OPTION | GRAPHIC ATTACH | LISTVIEW SET COLUMN | SCROLLBAR SET POS | XPRINT GET PAPER |
| CONTROL ADD PROGRESSBAR | GRAPHIC BITMAP END | LISTVIEW SET HEADER | SCROLLBAR SET RANGE | XPRINT GET PAPERS |
| CONTROL ADD SCROLLBAR | GRAPHIC BITMAP LOAD | LISTVIEW SET IMAGE | SEC | XPRINT GET PIXEL |
| CONTROL ADD STATUSBAR | GRAPHIC BITMAP NEW | LISTVIEW SET IMAGE2 | SECH | XPRINT GET POS |
| CONTROL ADD TAB | GRAPHIC BOX | LISTVIEW SET IMAGELIST | SEEK | XPRINT GET PPI |
| CONTROL ADD TEXTBOX | GRAPHIC CELL | LISTVIEW SET MODE | SELECT CASE/END SELECT | XPRINT GET QUALITY |
| CONTROL ADD TOOLBAR | GRAPHIC CELL SIZE | LISTVIEW SET OVERLAY | SETATTR | XPRINT GET SCALE |
| CONTROL ADD TREEVIEW | GRAPHIC CHR SIZE | LISTVIEW SET STYLEXX | SETEOF | XPRINT GET SELECTION |
| CONTROL DISABLE | GRAPHIC CIRCLE | LISTVIEW SET TEXT | SGN | XPRINT GET SIZE |
| CONTROL ENABLE | GRAPHIC CLEAR | LISTVIEW SET USER | SHELL | XPRINT GET STRETCHMODE |
| CONTROL GET CHECK | GRAPHIC COLOR | LISTVIEW SORT | SHIFT | XPRINT GET TEXTALIGN |
| CONTROL GET CLIENT | GRAPHIC COPY | LISTVIEW UNSELECT | SHRINK | XPRINT GET TRAY |
| CONTROL GET LOC | GRAPHIC DETACH | LISTVIEW VISIBLE | SHRINK$ | XPRINT GET TRAYS |
| CONTROL GET SIZE | GRAPHIC ELLIPSE | LO | SIN | XPRINT GET WORDWRAP |
| CONTROL GET TEXT | GRAPHIC GET BITS | LOC | SINGLE | XPRINT GET WRAP |
| CONTROL GET USER | GRAPHIC GET CANVAS | LOCAL | SINH | XPRINT IMAGELIST |
| CONTROL HANDLE | GRAPHIC GET CAPTION | LOCK | SIZEOF | XPRINT LINE |
| CONTROL HIDE | GRAPHIC GET CLIENT | LOF | SLEEP | XPRINT PIE |
| CONTROL KILL | GRAPHIC GET CLIP | LOG | SPACE | XPRINT POLYGON |
| CONTROL NORMALIZE | GRAPHIC GET DC | LOG10 | SPACE$ | XPRINT POLYLINE |
| CONTROL POST | GRAPHIC GET LINES | LOG1P | SPLIT | XPRINT PREVIEW |
| CONTROL REDRAW | GRAPHIC GET LOC | LOG2 | SQR | XPRINT PRINT |
| CONTROL SEND | GRAPHIC GET MIX | LONG | STATIC | XPRINT RENDER |
| CONTROL SET CHECK | GRAPHIC GET OVERLAP | LOWRD | STATUSBAR SET PARTS | XPRINT SCALE |
| CONTROL SET CLIENT | GRAPHIC GET PIXEL | LPRINT | STATUSBAR SET TEXT | XPRINT SET CLIP |
| CONTROL SET COLOR | GRAPHIC GET POS | LPRINT ATTACH | STR | XPRINT SET COLLATE |
| CONTROL SET FOCUS | GRAPHIC GET PPI | LPRINT CLOSE | STR$ | XPRINT SET COLOR |
| CONTROL SET FONT | GRAPHIC GET SCALE | LPRINT FLUSH | STRDELETE | XPRINT SET COLORMODE |
| CONTROL SET IMAGE | GRAPHIC GET SIZE | LPRINT FORMFEED | STRDELETE$ | XPRINT SET COPIES |
| CONTROL SET IMAGEX | GRAPHIC GET STRETCHMODE | LPRINT$ | STRING | XPRINT SET DUPLEX |
| CONTROL SET IMGBUTTON | GRAPHIC GET TEXTALIGN | LSET | STRINSERT | XPRINT SET FONT |
| CONTROL SET IMGBUTTONX | GRAPHIC GET VIEW | LSET$ | STRINSERT$ | XPRINT SET MIX |
| CONTROL SET LOC | GRAPHIC GET WORDWRAP | LTRIM | STRPTR | XPRINT SET ORIENTATION |
| CONTROL SET OPTION | GRAPHIC GET WRAP | LTRIM$ | STRREVERSE | XPRINT SET OVERLAP |
| CONTROL SET SIZE | GRAPHIC LINE | MACRO/END MACRO | STRREVERSE$ | XPRINT SET PAGES |
| CONTROL SET TEXT | GRAPHIC PAINT | MAK | SWAP | XPRINT SET PAPER |
| CONTROL SET USER | GRAPHIC PIE | MAT | SWITCH | XPRINT SET PIXEL |
| CONTROL SHOW STATE | GRAPHIC POLYGON | MAX | SWITCH$ | XPRINT SET POS |
| COS | GRAPHIC POLYLINE | MCASE | TAB DELETE | XPRINT SET QUALITY |
| COSH | GRAPHIC PRINT | MCASE$ | TAB GET COUNT | XPRINT SET STRETCHMODE |
| COT | GRAPHIC REDRAW | MEMORY | TAB GET DIALOG | XPRINT SET TEXTALIGN |
| COTH | GRAPHIC SAVE | MEMORY FILL | TAB GET IMAGE | XPRINT SET TRAY |
| CQUAD | GRAPHIC SCALE | MEMORY FILLS | TAB GET PAGE | XPRINT SET WORDWRAP |
| CSC | GRAPHIC SCALE PIXELS | MENU ADD POPUP | TAB GET SELECT | XPRINT SET WRAP |
| CSCH | GRAPHIC SET AUTOSIZE | MENU ADD STRING | TAB GET TEXT | XPRINT SPLIT |
| CSET | GRAPHIC SET BITS | MENU ATTACH | TAB INSERT PAGE | XPRINT STRETCH |
| CSET$ | GRAPHIC SET CAPTION | MENU CONTEXT | TAB RESET | XPRINT STYLE |
| CSNG | GRAPHIC SET CLIENT | MENU DELETE | TAB SELECT | XPRINT TEXT SIZE |
| CSTR | GRAPHIC SET CLIP | MENU DRAW BAR | TAB SET IMAGE | XPRINT WIDTH |
| CULNG | GRAPHIC SET FIXED | MENU GET STATE | TAB SET IMAGELIST |  |
| CURDIR | GRAPHIC SET FOCUS | MENU GET TEXT | TAB SET TEXT |  |
| CURDIR$ | GRAPHIC SET FONT | MENU NEW BAR | TAB$ |  |

## **Newly** implemented by this branch

> **694 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DIALOG POST | ISODD | SIN |
| ACCEL ATTACH | DIALOG REDRAW | ISTRUE | SINGLE |
| ACODE | DIALOG SEND | ISWIN | SINH |
| ACODE$ | DIALOG SET CLIENT | JOIN$ | SIZEOF |
| ACOS | DIALOG SET COLOR | LCASE | SPACE |
| ACOSH | DIALOG SET ICON | LCASE$ | SPACE$ |
| ARRAY ADD | DIALOG SET LOC | LEFT | SQR |
| ARRAY ARRAYIX | DIALOG SET SIZE | LEFT$ | STATUSBAR SET PARTS |
| ARRAY ASSIGN | DIALOG SET TEXT | LEN | STATUSBAR SET TEXT |
| ARRAY COPY | DIALOG SET USER | LET *(WITH OBJECTS)* | STR |
| ARRAY DELETE | DIALOG SHOW MODAL | LET *(WITH VARIANTS)* | STR$ |
| ARRAY INSERT | DIALOG SHOW MODELESS | LINE INPUT# | STRDELETE |
| ARRAY REDIM DECR | DIALOG SHOW STATE | LISTBOX ADD | STRDELETE$ |
| ARRAY REDIM INCR | DIALOG STABILIZE | LISTBOX DELETE | STRING |
| ARRAY REVERSE | DIALOG UNITS | LISTBOX FIND | STRINSERT |
| ARRAY SCAN | DIM | LISTBOX FIND EXACT | STRINSERT$ |
| ARRAY SHUFFLE | DIR | LISTBOX GET COUNT | STRPTR |
| ARRAY SWAP | DIR CLOSE | LISTBOX GET SELCOUNT | STRREVERSE |
| ARRAY TAGARRAY | DIR$ | LISTBOX GET SELECT | STRREVERSE$ |
| ARRAY TAGARRAY ERASE | DISKFREE | LISTBOX GET STATE | SWITCH |
| ARRAY UNIQUE | DISKSIZE | LISTBOX GET TEXT | SWITCH$ |
| ASIN | DISPLAY BROWSE | LISTBOX GET USER | TAB DELETE |
| ASINH | DISPLAY COLOR | LISTBOX INSERT | TAB GET COUNT |
| ATANH | DISPLAY FONT | LISTBOX RESET | TAB GET DIALOG |
| ATN | DISPLAY OPENFILE | LISTBOX SELECT | TAB GET IMAGE |
| ATN2 | DISPLAY SAVEFILE | LISTBOX SET TEXT | TAB GET PAGE |
| BGR | DOUBLE | LISTBOX SET USER | TAB GET SELECT |
| BIN | ENVIRON$ | LISTBOX UNSELECT | TAB GET TEXT |
| BIN$ | EOF | LISTVIEW DELETE COLUMN | TAB INSERT PAGE |
| BITS | EQV | LISTVIEW DELETE ITEM | TAB RESET |
| BITS$ | ERF | LISTVIEW FIND | TAB SELECT |
| BITSE | ERL | LISTVIEW FIND EXACT | TAB SET IMAGE |
| BUILD | ERL$ | LISTVIEW FIT CONTENT | TAB SET IMAGELIST |
| BUILD$ | ERR | LISTVIEW FIT HEADER | TAB SET TEXT |
| BYTE | ERRCLEAR | LISTVIEW GET COLUMN | TAB$ |
| CALLSTK$ | ERROR$ | LISTVIEW GET COUNT | TALLY |
| CALLSTKCOUNT | EVENT SOURCE | LISTVIEW GET HEADER | TAN |
| CBOOL | EVENTS | LISTVIEW GET HEADERID | TANH |
| CBRT | EXE | LISTVIEW GET MODE | TCP NOTIFY |
| CBYTE | EXIST | LISTVIEW GET SELCOUNT | THREADCOUNT |
| CDBL | EXP | LISTVIEW GET SELECT | THREADID |
| CDWORD | EXP10 | LISTVIEW GET STATE | TIMER |
| CEIL | EXP2 | LISTVIEW GET STYLEXX | TOOLBAR ADD BUTTON |
| CFLT | EXPM1 | LISTVIEW GET TEXT | TOOLBAR ADD SEPARATOR |
| CHOOSE | EXTRACT | LISTVIEW GET USER | TOOLBAR DELETE BUTTON |
| CHR | EXTRACT$ | LISTVIEW INSERT COLUMN | TOOLBAR GET COUNT |
| CHR$ | FILEATTR | LISTVIEW INSERT ITEM | TOOLBAR GET STATE |
| CHRBYTES | FILENAME | LISTVIEW RESET | TOOLBAR SET IMAGELIST |
| CHRTOOEM | FILENAME$ | LISTVIEW SELECT | TOOLBAR SET STATE |
| CHRTOOEM$ | FIX | LISTVIEW SET COLUMN | TREEVIEW DELETE |
| CHRTOUTF8 | FLOOR | LISTVIEW SET HEADER | TREEVIEW GET BOLD |
| CHRTOUTF8$ | FONT END | LISTVIEW SET IMAGE | TREEVIEW GET CHECK |
| CLIP | FONT NEW | LISTVIEW SET IMAGE2 | TREEVIEW GET CHILD |
| CLIP$ | FORMAT | LISTVIEW SET IMAGELIST | TREEVIEW GET COUNT |
| CLOSE | FORMAT$ | LISTVIEW SET MODE | TREEVIEW GET EXPANDED |
| CODEPTR | FRAC | LISTVIEW SET OVERLAY | TREEVIEW GET NEXT |
| COMBOBOX ADD | FRE | LISTVIEW SET STYLEXX | TREEVIEW GET PARENT |
| COMBOBOX DELETE | FREEFILE | LISTVIEW SET TEXT | TREEVIEW GET PREVIOUS |
| COMBOBOX FIND | FUNCNAME | LISTVIEW SET USER | TREEVIEW GET ROOT |
| COMBOBOX FIND EXACT | FUNCNAME$ | LISTVIEW SORT | TREEVIEW GET SELECT |
| COMBOBOX GET COUNT | GET | LISTVIEW UNSELECT | TREEVIEW GET TEXT |
| COMBOBOX GET SELCOUNT | GET STR | LISTVIEW VISIBLE | TREEVIEW GET USER |
| COMBOBOX GET SELECT | GET WSTR | LO | TREEVIEW INSERT ITEM |
| COMBOBOX GET STATE | GET$ | LOC | TREEVIEW RESET |
| COMBOBOX GET TEXT | GET$$ | LOF | TREEVIEW SELECT |
| COMBOBOX GET USER | GETATTR | LOG | TREEVIEW SET BOLD |
| COMBOBOX INSERT | GRAPHIC ARC | LOG10 | TREEVIEW SET CHECK |
| COMBOBOX RESET | GRAPHIC ATTACH | LOG1P | TREEVIEW SET EXPANDED |
| COMBOBOX SELECT | GRAPHIC BITMAP END | LOG2 | TREEVIEW SET IMAGELIST |
| COMBOBOX SET TEXT | GRAPHIC BITMAP LOAD | LONG | TREEVIEW SET TEXT |
| COMBOBOX SET USER | GRAPHIC BITMAP NEW | LOWRD | TREEVIEW SET USER |
| COMBOBOX UNSELECT | GRAPHIC BOX | LPRINT$ | TREEVIEW UNSELECT |
| COMM LINE INPUT | GRAPHIC CELL | LSET$ | TRIM |
| COMMAND | GRAPHIC CELL SIZE | LTRIM | TRIM$ |
| COMMAND$ | GRAPHIC CHR SIZE | LTRIM$ | TRUNC |
| CONTROL ADD | GRAPHIC CIRCLE | MAK | UCASE |
| CONTROL ADD BUTTON | GRAPHIC CLEAR | MAX | UCASE$ |
| CONTROL ADD CHECK3STATE | GRAPHIC COLOR | MCASE | UCODE$ |
| CONTROL ADD CHECKBOX | GRAPHIC COPY | MCASE$ | UCODEPAGE |
| CONTROL ADD COMBOBOX | GRAPHIC DETACH | MEMORY FILL | UDP NOTIFY |
| CONTROL ADD FRAME | GRAPHIC ELLIPSE | MEMORY FILLS | UNWRAP |
| CONTROL ADD GRAPHIC | GRAPHIC GET BITS | MENU ADD POPUP | UNWRAP$ |
| CONTROL ADD HEADER | GRAPHIC GET CANVAS | MENU ADD STRING | USING |
| CONTROL ADD IMAGE | GRAPHIC GET CAPTION | MENU ATTACH | USING$ |
| CONTROL ADD IMAGEX | GRAPHIC GET CLIENT | MENU CONTEXT | UTF8TOCHR |
| CONTROL ADD IMGBUTTON | GRAPHIC GET CLIP | MENU DELETE | UTF8TOCHR$ |
| CONTROL ADD IMGBUTTONX | GRAPHIC GET DC | MENU DRAW BAR | VARPTR |
| CONTROL ADD LABEL | GRAPHIC GET LINES | MENU GET STATE | VERIFY |
| CONTROL ADD LINE | GRAPHIC GET LOC | MENU GET TEXT | WAITKEY |
| CONTROL ADD LISTBOX | GRAPHIC GET MIX | MENU NEW BAR | WAITKEY$ |
| CONTROL ADD LISTVIEW | GRAPHIC GET OVERLAP | MENU NEW POPUP | WORD |
| CONTROL ADD OPTION | GRAPHIC GET PIXEL | MENU SET STATE | WRAP |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET POS | MENU SET TEXT | WRAP$ |
| CONTROL ADD SCROLLBAR | GRAPHIC GET PPI | METRICS | WRITE |
| CONTROL ADD STATUSBAR | GRAPHIC GET SCALE | MID | WRITE# |
| CONTROL ADD TAB | GRAPHIC GET SIZE | MIN | XPRINT ARC |
| CONTROL ADD TEXTBOX | GRAPHIC GET STRETCHMODE | MKBYT | XPRINT ATTACH |
| CONTROL ADD TOOLBAR | GRAPHIC GET TEXTALIGN | MKCUX | XPRINT BOX |
| CONTROL ADD TREEVIEW | GRAPHIC GET VIEW | MKDWD | XPRINT CANCEL |
| CONTROL DISABLE | GRAPHIC GET WORDWRAP | MKE | XPRINT CELL |
| CONTROL ENABLE | GRAPHIC GET WRAP | MKS | XPRINT CELL SIZE |
| CONTROL GET CHECK | GRAPHIC LINE | MKWRD | XPRINT CHR SIZE |
| CONTROL GET CLIENT | GRAPHIC PAINT | MOD | XPRINT CLOSE |
| CONTROL GET LOC | GRAPHIC PIE | MONTHNAME | XPRINT COLOR |
| CONTROL GET SIZE | GRAPHIC POLYGON | MONTHNAME$ | XPRINT COPY |
| CONTROL GET TEXT | GRAPHIC POLYLINE | NUL$ | XPRINT ELLIPSE |
| CONTROL GET USER | GRAPHIC PRINT | OCT | XPRINT FORMFEED |
| CONTROL HANDLE | GRAPHIC REDRAW | OCT$ | XPRINT GET ATTACH |
| CONTROL HIDE | GRAPHIC SAVE | OEMTOCHR | XPRINT GET CANVAS |
| CONTROL KILL | GRAPHIC SCALE | OEMTOCHR$ | XPRINT GET CLIENT |
| CONTROL NORMALIZE | GRAPHIC SCALE PIXELS | ON CALL | XPRINT GET CLIP |
| CONTROL POST | GRAPHIC SET AUTOSIZE | ON GOSUB | XPRINT GET COLLATE |
| CONTROL REDRAW | GRAPHIC SET BITS | ON GOTO | XPRINT GET COLOR |
| CONTROL SEND | GRAPHIC SET CAPTION | OPEN | XPRINT GET COLORMODE |
| CONTROL SET CHECK | GRAPHIC SET CLIENT | PARSE$ | XPRINT GET COPIES |
| CONTROL SET CLIENT | GRAPHIC SET CLIP | PARSECOUNT | XPRINT GET DC |
| CONTROL SET COLOR | GRAPHIC SET FIXED | PATHNAME | XPRINT GET DUPLEX |
| CONTROL SET FOCUS | GRAPHIC SET FOCUS | PATHNAME$ | XPRINT GET LINES |
| CONTROL SET FONT | GRAPHIC SET FONT | PATHSCAN | XPRINT GET MARGIN |
| CONTROL SET IMAGE | GRAPHIC SET LOC | PATHSCAN$ | XPRINT GET MIX |
| CONTROL SET IMAGEX | GRAPHIC SET MIX | PEEK | XPRINT GET ORIENTATION |
| CONTROL SET IMGBUTTON | GRAPHIC SET OVERLAP | PLAY | XPRINT GET OVERLAP |
| CONTROL SET IMGBUTTONX | GRAPHIC SET PIXEL | PLAY SOUND | XPRINT GET PAGES |
| CONTROL SET LOC | GRAPHIC SET POS | POKE | XPRINT GET PAPER |
| CONTROL SET OPTION | GRAPHIC SET SIZE | PRINTER$ | XPRINT GET PAPERS |
| CONTROL SET SIZE | GRAPHIC SET STRETCHMODE | PRINTERCOUNT | XPRINT GET PIXEL |
| CONTROL SET TEXT | GRAPHIC SET TEXTALIGN | PROGRESSBAR GET POS | XPRINT GET POS |
| CONTROL SET USER | GRAPHIC SET VIEW | PROGRESSBAR GET RANGE | XPRINT GET PPI |
| CONTROL SHOW STATE | GRAPHIC SET VIRTUAL | PROGRESSBAR SET POS | XPRINT GET QUALITY |
| COS | GRAPHIC SET WORDWRAP | PROGRESSBAR SET RANGE | XPRINT GET SCALE |
| COSH | GRAPHIC SET WRAP | PROGRESSBAR SET STEP | XPRINT GET SELECTION |
| COT | GRAPHIC STYLE | PROGRESSBAR STEP | XPRINT GET SIZE |
| COTH | GRAPHIC TEXT SIZE | PUT | XPRINT GET STRETCHMODE |
| CQUAD | GRAPHIC WIDTH | PUT STR | XPRINT GET TEXTALIGN |
| CSC | GRAPHIC WINDOW | PUT WSTR | XPRINT GET TRAY |
| CSCH | GRAPHIC WINDOW CLICK | PUT$ | XPRINT GET TRAYS |
| CSET$ | GRAPHIC WINDOW END | PUT$$ | XPRINT GET WORDWRAP |
| CSNG | GRAPHIC WINDOW HIDE | QUAD | XPRINT GET WRAP |
| CSTR | GRAPHIC WINDOW MINIMIZE | RAISEEVENT | XPRINT IMAGELIST |
| CULNG | GRAPHIC WINDOW NONSTABLE | READ | XPRINT LINE |
| CURDIR | GRAPHIC WINDOW NORMALIZE | READ$ | XPRINT PIE |
| CURDIR$ | GRAPHIC WINDOW STABILIZE | REDIM | XPRINT POLYGON |
| CVBYT | HEADER GET COUNT | REMAIN | XPRINT POLYLINE |
| CVCUX | HEADER GET ITEM | REMAIN$ | XPRINT PREVIEW |
| CVDWD | HEADER SEND | REMOVE | XPRINT PRINT |
| CVL | HEADER SET ITEM | REMOVE$ | XPRINT RENDER |
| CVQ | HEX | REPEAT | XPRINT SCALE |
| CVW | HEX$ | REPEAT$ | XPRINT SET CLIP |
| CWORD | HI | RESOURCE SAVE FILE | XPRINT SET COLLATE |
| DATACOUNT | HIWRD | RESOURCE$ | XPRINT SET COLOR |
| DAYNAME | HYPOT | RETAIN | XPRINT SET COLORMODE |
| DAYNAME$ | IIF | RETAIN$ | XPRINT SET COPIES |
| DEC | IMAGELIST ADD BITMAP | RGB | XPRINT SET DUPLEX |
| DEC$ | IMAGELIST ADD ICON | RIGHT | XPRINT SET FONT |
| DECLARE | IMAGELIST ADD MASKED | RIGHT$ | XPRINT SET MIX |
| DEF | IMAGELIST COUNT | RND | XPRINT SET ORIENTATION |
| DESKTOP GET PPI | IMAGELIST GET COUNT | ROUND | XPRINT SET OVERLAP |
| DIALOG DEFAULT FONT | IMAGELIST KILL | RSET$ | XPRINT SET PAGES |
| DIALOG DISABLE | IMAGELIST NEW | RTRIM | XPRINT SET PAPER |
| DIALOG DOEVENTS | IMAGELIST NEW BITMAP | RTRIM$ | XPRINT SET PIXEL |
| DIALOG ENABLE | IMAGELIST NEW ICON | SCROLLBAR GET PAGESIZE | XPRINT SET POS |
| DIALOG END | IMAGELIST SET OVERLAY | SCROLLBAR GET POS | XPRINT SET QUALITY |
| DIALOG GET CLIENT | IMP | SCROLLBAR GET RANGE | XPRINT SET STRETCHMODE |
| DIALOG GET LOC | INPUT# | SCROLLBAR GET TRACKPOS | XPRINT SET TEXTALIGN |
| DIALOG GET SIZE | INSTANCE | SCROLLBAR SET PAGESIZE | XPRINT SET TRAY |
| DIALOG GET TEXT | INSTR | SCROLLBAR SET POS | XPRINT SET WORDWRAP |
| DIALOG GET USER | INT | SCROLLBAR SET RANGE | XPRINT SET WRAP |
| DIALOG HIDE | INTEGER | SEC | XPRINT SPLIT |
| DIALOG MAXIMIZE | ISEVEN | SECH | XPRINT STRETCH |
| DIALOG MINIMIZE | ISFALSE | SEEK | XPRINT STYLE |
| DIALOG NEW | ISFILE | SETEOF | XPRINT TEXT SIZE |
| DIALOG NONSTABLE | ISFOLDER | SGN | XPRINT WIDTH |
| DIALOG NORMALIZE | ISINFINITE | SHRINK |  |
| DIALOG PIXELS | ISNORMAL | SHRINK$ |  |
