...
>   - **454** implemented or completed by this fork
...
>
> Counts are deduplicated official keywords (internal codegen helper names are excluded; they appear in the full [statement-coverage.md](docs/statement-coverage.md) table). Updated through batch 118 (v0.1.118).
...
| ACODE | ACODE$ | ACOS | ACOSH | AND |
| ARRAY ADD | ARRAY ARRAYIX | ARRAY ASSIGN | ARRAY COPY | ARRAY DELETE |
| ARRAY INSERT | ARRAY REDIM DECR | ARRAY REDIM INCR | ARRAY REVERSE | ARRAY SCAN |
| ARRAY SELECT | ARRAY SHUFFLE | ARRAY SORT | ARRAY SWAP | ARRAY UNIQUE |
| ASC | ASIN | ASINH | ASM | ASMDATA / END ASMDATA |
| ATANH | ATN | BEEP | BIN$ | BIT |
| BIT CALC | BITS$ | BITSE | BUILD$ | CALL |
| CALL DWORD | CALLSTK | CALLSTK$ | CBRT | CEIL |
| CHDIR | CHDRIVE | CHR$ | CHRTOOEM$ | CHRTOUTF8$ |
| CLASS/END CLASS | CLIP$ | CLIPBOARD | CLOSE | CLS |
| COLOR | COMM CLOSE | COMM LINE | COMM LINE INPUT | COMM OPEN |
| COMM PRINT | COMM RECV | COMM RESET | COMM SEND | COMM SET |
| COMM TIMEOUT | COMMAND$ | COS | COSH | COT |
| COTH | CSC | CSCH | CSET | CSET$ |
| CURDIR$ | DATA | DAYNAME$ | DEC$ | DECLARE |
| DECR | DESKTOP GET CLIENT | DESKTOP GET LOC | DESKTOP GET PPI | DESKTOP GET SIZE |
| DIM | DIR CLOSE | DIR$ | DISPLAY BROWSE | DISPLAY COLOR |
| DISPLAY FONT | DISPLAY OPENFILE | DISPLAY SAVEFILE | END | ENVIRON |
| ENVIRON$ | EQV | ERASE | ERF | ERL$ |
| ERROR | ERROR$ | EXIT | EXP | EXP10 |
| EXP2 | EXPM1 | EXTRACT$ | FIELD | FILECOPY |
| FILENAME$ | FILESCAN | FLOOR | FLUSH | FONT END |
| FONT NEW | FOR / NEXT | FORMAT$ | FUNCNAME | FUNCNAME$ |
| FUNCTION / END FUNCTION | GET | GET$ | GET$$ | GLOBAL |
...
| GRAPHIC PAINT | GRAPHIC PIE | GRAPHIC POLYGON | GRAPHIC POLYLINE | GRAPHIC PRINT |
| GRAPHIC SAVE | GRAPHIC SCALE | GRAPHIC SET AUTOSIZE | GRAPHIC SET BITS | GRAPHIC SET CAPTION |
| GRAPHIC SET CLIP | GRAPHIC SET FIXED | GRAPHIC SET FONT | GRAPHIC SET MIX | GRAPHIC SET PIXEL |
| GRAPHIC SET POS | GRAPHIC SET SIZE | GRAPHIC SET STRETCHMODE | GRAPHIC SET TEXTALIGN | GRAPHIC SET VIEW |
| GRAPHIC SET VIRTUAL | GRAPHIC SET WORDWRAP | GRAPHIC SET WRAP | GRAPHIC STYLE | GRAPHIC TEXT SIZE |
| GRAPHIC WIDTH | HEX$ | HI | HOST ADDR | HOST NAME |
| HYPOT | IF | IF/END IF | IMP | IMPORT |
| INCR | INPUT FLUSH | INPUT# | INTERFACE / END INTERFACE (DIRECT) | INTERFACE/END INTERFACE (IDBIND) |
| ISINFINITE | ISNORMAL | ISWIN | ITERATE | JOIN$ |
| KILL | LCASE$ | LEFT$ | LET | LET *(WITH TYPES)* |
| LINE INPUT# | LOC | LOCAL | LOCK | LOF |
| LOG | LOG10 | LOG1P | LOG2 | LPRINT |
| LPRINT ATTACH | LPRINT CLOSE | LPRINT FLUSH | LPRINT FORMFEED | LPRINT$ |
| LSET | LSET$ | LTRIM$ | MACRO/END MACRO | MAT |
| MCASE | MCASE$ | MEMORY | MENU ADD POPUP | MENU ADD STRING |
| MENU DELETE | MENU GET STATE | MENU GET TEXT | MENU NEW BAR | MENU NEW POPUP |
| MENU SET STATE | MENU SET TEXT | METHOD / END METHOD | METRICS | MID$ |
| MKBYT$ | MKCUR$ | MKCUX$ | MKD$ | MKDIR |
| MKDWD$ | MKE$ | MKI$ | MKL$ | MKQ$ |
| MKS$ | MKWRD$ | MONTHNAME$ | MOUSEPTR | MSGBOX |
| NAME | NOT | NUL | NUL$ | OBJECT |
| OCT$ | OEMTOCHR$ | ON CALL | ON ERROR | ON GOSUB |
| ON GOTO | OPEN | OPTION EXPLICIT | OR | PARSE |
| PARSE$ | PATHNAME$ | PATHSCAN$ | PLAY | PLAY SOUND |
| PLAY WAVE | PREFIX | PRINT# | PRINTER$ | PROCESS GET PRIORITY |
| PROCESS SET PRIORITY | PROFILE | PUT | PUT$ | PUT$$ |
| RANDOMIZE | READ$ | REDIM | REGEXPR | REGISTER |
| REGREPL | REM | REMAIN$ | REMOVE$ | REPEAT$ |
| REPLACE | RESET | RESOURCE SAVE FILE | RESOURCE$ | RESUME |
| RETAIN$ | RETURN | RGB | RIGHT$ | RMDIR |
| ROTATE | ROUND | RSET | RSET$ | RTRIM$ |
| SEC | SECH | SEEK | SELECT CASE/END SELECT | SETATTR |
| SETEOF | SHELL | SHIFT | SHRINK$ | SIN |
| SINH | SLEEP | SPACE$ | SPLIT | SQR |
| STATIC | STR$ | STRDELETE$ | STRINSERT$ | STRREVERSE$ |
| SWAP | SWITCH | TAB$ | TAN | TANH |
| TCP ACCEPT | TCP CLOSE | TCP LINE INPUT | TCP OPEN | TCP PRINT |
| TCP RECV | TCP SEND | THREAD CLOSE | THREAD CREATE | THREAD GET PRIORITY |
| THREAD RESUME | THREAD SET PRIORITY | THREAD STATUS | THREAD SUSPEND | THREADED |
| THREADID | TIX | TRACE | TRIM$ | TRUNC |
| TRY/END TRY | TYPE SET | TYPE/END TYPE | UCASE$ | UCODE$ |
| UCODEPAGE | UDP CLOSE | UDP OPEN | UDP RECV | UDP SEND |
| UNLOCK | UNWRAP$ | USING$ | UTF8TOCHR$ | VAL |
| WAITKEY$ | WINDOW GET | WINDOW SET | WRAP$ | WRITE# |
| XOR | XPRINT ARC | XPRINT ATTACH | XPRINT BOX | XPRINT CANCEL |
...
| #OPTION METASTATEMENT | #PAGE METASTATEMENT | #PBFORMS METASTATEMENT | #REGISTER METASTATEMENT | #RESOURCE METASTATEMENT |
| #STACK METASTATEMENT | #TOOLS METASTATEMENT | #UNIQUE METASTATEMENT | #UTILITY METASTATEMENT | ABS |
| ACODE | ACODE$ | ACOS | ACOSH | ARRAY ADD |
| ARRAY ARRAYIX | ARRAY ASSIGN | ARRAY COPY | ARRAY DELETE | ARRAY INSERT |
| ARRAY REDIM DECR | ARRAY REDIM INCR | ARRAY REVERSE | ARRAY SCAN | ARRAY SELECT |
| ARRAY SHUFFLE | ARRAY SORT | ARRAY SWAP | ARRAY UNIQUE | ASIN |
| ASINH | ASM | ASMDATA / END ASMDATA | ATANH | ATN |
| BEEP | BIN$ | BIT | BIT CALC | BITS$ |
| BITSE | BUILD$ | CALL DWORD | CALLSTK | CALLSTK$ |
| CBRT | CEIL | CHDIR | CHDRIVE | CHR$ |
| CHRTOOEM$ | CHRTOUTF8$ | CLASS/END CLASS | CLIP$ | CLIPBOARD |
| CLOSE | CLS | COLOR | COMM CLOSE | COMM LINE |
...
| CSET | CSET$ | CURDIR$ | DAYNAME$ | DEC$ |
| DECLARE | DECR | DESKTOP GET CLIENT | DESKTOP GET LOC | DESKTOP GET PPI |
| DESKTOP GET SIZE | DIM | DIR CLOSE | DIR$ | DISPLAY BROWSE |
| DISPLAY COLOR | DISPLAY FONT | DISPLAY OPENFILE | DISPLAY SAVEFILE | ENVIRON |
| ENVIRON$ | EQV | ERASE | ERF | ERL$ |
| ERROR | ERROR$ | EXP | EXP10 | EXP2 |
| EXPM1 | EXTRACT$ | FIELD | FILECOPY | FILENAME$ |
| FILESCAN | FLOOR | FLUSH | FONT END | FONT NEW |
| FORMAT$ | FUNCNAME | FUNCNAME$ | GET | GET$ |
| GET$$ | GLOBALMEM | GRAPHIC ARC | GRAPHIC ATTACH | GRAPHIC BITMAP END |
| GRAPHIC BITMAP LOAD | GRAPHIC BITMAP NEW | GRAPHIC BOX | GRAPHIC CELL | GRAPHIC CELL SIZE |
| GRAPHIC CHR SIZE | GRAPHIC CLEAR | GRAPHIC COLOR | GRAPHIC COPY | GRAPHIC DETACH |
...
| GRAPHIC PRINT | GRAPHIC SAVE | GRAPHIC SCALE | GRAPHIC SET AUTOSIZE | GRAPHIC SET BITS |
...
| GRAPHIC TEXT SIZE | GRAPHIC WIDTH | HEX$ | HI | HOST ADDR |
| HOST NAME | HYPOT | IMP | IMPORT | INCR |
| INPUT FLUSH | INPUT# | INTERFACE / END INTERFACE (DIRECT) | INTERFACE/END INTERFACE (IDBIND) | ISINFINITE |
| ISNORMAL | ISWIN | ITERATE | JOIN$ | KILL |
| LCASE$ | LEFT$ | LET *(WITH TYPES)* | LINE INPUT# | LOC |
| LOCK | LOF | LOG | LOG10 | LOG1P |
| LOG2 | LPRINT | LPRINT ATTACH | LPRINT CLOSE | LPRINT FLUSH |
| LPRINT FORMFEED | LPRINT$ | LSET | LSET$ | LTRIM$ |
| MACRO/END MACRO | MAT | MCASE | MCASE$ | MEMORY |
| MENU ADD POPUP | MENU ADD STRING | MENU DELETE | MENU GET STATE | MENU GET TEXT |
| MENU NEW BAR | MENU NEW POPUP | MENU SET STATE | MENU SET TEXT | METHOD / END METHOD |
| METRICS | MKBYT$ | MKCUR$ | MKCUX$ | MKD$ |
| MKDIR | MKDWD$ | MKE$ | MKI$ | MKL$ |
| MKQ$ | MKS$ | MKWRD$ | MONTHNAME$ | MOUSEPTR |
| MSGBOX | NAME | NUL$ | OBJECT | OCT$ |
| OEMTOCHR$ | ON CALL | ON ERROR | ON GOSUB | ON GOTO |
| OPEN | OPTION EXPLICIT | PARSE$ | PATHNAME$ | PATHSCAN$ |
| PLAY | PLAY SOUND | PLAY WAVE | PREFIX | PRINTER$ |
| PROCESS GET PRIORITY | PROCESS SET PRIORITY | PROFILE | PUT | PUT$ |
| PUT$$ | READ$ | REDIM | REGEXPR | REGISTER |
| REGREPL | REM | REMAIN$ | REMOVE$ | REPEAT$ |
| REPLACE | RESET | RESOURCE SAVE FILE | RESOURCE$ | RESUME |
| RETAIN$ | RGB | RIGHT$ | RMDIR | ROTATE |
| ROUND | RSET | RSET$ | RTRIM$ | SEC |
| SECH | SEEK | SETATTR | SETEOF | SHELL |
| SHIFT | SHRINK$ | SIN | SINH | SPACE$ |
| SPLIT | SQR | STR$ | STRDELETE$ | STRINSERT$ |
| STRREVERSE$ | SWAP | SWITCH | TAB$ | TAN |
| TANH | TCP ACCEPT | TCP CLOSE | TCP LINE INPUT | TCP OPEN |
| TCP PRINT | TCP RECV | TCP SEND | THREAD CLOSE | THREAD CREATE |
| THREAD GET PRIORITY | THREAD RESUME | THREAD SET PRIORITY | THREAD STATUS | THREAD SUSPEND |
| THREADED | THREADID | TIX | TRACE | TRIM$ |
| TRUNC | TRY/END TRY | TYPE SET | UCASE$ | UCODE$ |
| UCODEPAGE | UDP CLOSE | UDP OPEN | UDP RECV | UDP SEND |
| UNLOCK | UNWRAP$ | USING$ | UTF8TOCHR$ | WAITKEY$ |
| WINDOW GET | WINDOW SET | WRAP$ | WRITE# | XPRINT ARC |
| XPRINT ATTACH | XPRINT BOX | XPRINT CANCEL | XPRINT CELL | XPRINT CELL SIZE |
| XPRINT CHR SIZE | XPRINT CLOSE | XPRINT COLOR | XPRINT COPY | XPRINT ELLIPSE |
| XPRINT FORMFEED | XPRINT GET ATTACH | XPRINT GET CANVAS | XPRINT GET CLIENT | XPRINT GET CLIP |
| XPRINT GET COLLATE | XPRINT GET COLORMODE | XPRINT GET COPIES | XPRINT GET DC | XPRINT GET DUPLEX |
| XPRINT GET LINES | XPRINT GET MARGIN | XPRINT GET MIX | XPRINT GET ORIENTATION | XPRINT GET OVERLAP |
| XPRINT GET PAGES | XPRINT GET PAPER | XPRINT GET PAPERS | XPRINT GET PIXEL | XPRINT GET POS |
| XPRINT GET PPI | XPRINT GET QUALITY | XPRINT GET SCALE | XPRINT GET SELECTION | XPRINT GET SIZE |
| XPRINT GET STRETCHMODE | XPRINT GET TEXTALIGN | XPRINT GET TRAY | XPRINT GET TRAYS | XPRINT GET WORDWRAP |
| XPRINT GET WRAP | XPRINT IMAGELIST | XPRINT LINE | XPRINT PIE | XPRINT POLYGON |
| XPRINT POLYLINE | XPRINT PREVIEW | XPRINT PRINT | XPRINT RENDER | XPRINT SCALE |
| XPRINT SET CLIP | XPRINT SET COLLATE | XPRINT SET COLORMODE | XPRINT SET COPIES | XPRINT SET DUPLEX |
| XPRINT SET FONT | XPRINT SET MIX | XPRINT SET ORIENTATION | XPRINT SET OVERLAP | XPRINT SET PAGES |
| XPRINT SET PAPER | XPRINT SET PIXEL | XPRINT SET POS | XPRINT SET QUALITY | XPRINT SET STRETCHMODE |
| XPRINT SET TEXTALIGN | XPRINT SET TRAY | XPRINT SET WORDWRAP | XPRINT SET WRAP | XPRINT SPLIT |
| XPRINT STRETCH | XPRINT STYLE | XPRINT TEXT SIZE | XPRINT WIDTH |  |


## Core statements originally from upstream benstopics

> The ~27 core control-flow structures, operators and basic builtins the original upstream `benstopics/powerbasilisk` (last commit 2026-02-18) shipped with real codegen. Everything else in the matrix above (454 keywords) was added or completed by this fork.

| Keyword | Keyword | Keyword | Keyword | Keyword |
| --- | --- | --- | --- | --- |
| AND | ASC | CALL | DATA | END |
| EXIT | FOR / NEXT | FUNCTION / END FUNCTION | GLOBAL | IF |
| IF/END IF | LET | LOCAL | MID$ | NOT |
| NUL | OR | PARSE | PRINT# | RANDOMIZE |
| RETURN | SELECT CASE/END SELECT | SLEEP | STATIC | TYPE/END TYPE |
| VAL | XOR |  |  |  |
