...
> All 536 statement/function keywords currently available for use (432 by this fork + 83 upstream).
...
| FILENAME$ | FILESCAN | FIX | FLOOR | FLUSH |
| FONT END | FONT NEW | FOR / NEXT | FRE | FREEFILE |
| FUNCTION / END FUNCTION | GET | GET$ | GET$$ | GETATTR |
| GLOBAL | GLOBALMEM | GRAPHIC ARC | GRAPHIC ATTACH | GRAPHIC BITMAP END |
...
| GRAPHIC WIDTH | HEADER | HEX$ | HI | HOST ADDR |
| HOST NAME | HYPOT | IF | IF/END IF | IIF |
| IMAGELIST | IMPORT | INCR | INKEY$ | INPUT FLUSH |
| INPUT# | INSTANCE | INSTR | INT | INTEGER |
| INTERFACE / END INTERFACE (DIRECT) | INTERFACE/END INTERFACE (IDBIND) | ISEVEN | ISFALSE | ISFILE |
| ISFOLDER | ISINFINITE | ISNORMAL | ISODD | ISTRUE |
| ITERATE | JOIN$ | KILL | LCASE$ | LEFT$ |
| LEN | LET | LET *(WITH OBJECTS)* | LET *(WITH TYPES)* | LET *(WITH VARIANTS)* |
| LINE INPUT# | LO | LOC | LOCAL | LOCK |
| LOF | LOG | LOG10 | LOG1P | LOG2 |
| LPRINT | LPRINT ATTACH | LPRINT CLOSE | LPRINT FLUSH | LPRINT FORMFEED |
| LSET | LTRIM$ | MACRO/END MACRO | MAT | MEMORY |
...
| MKQ$ | MKS$ | MKWRD$ | MOD | MONTH |
| MONTHNAME$ | MOUSEPTR | MSGBOX | NAME | OBJECT |
| OCT$ | OEMTOCHR$ | ON CALL | ON ERROR | ON GOSUB |
| ON GOTO | OPEN | OPTION EXPLICIT | PARSE | PARSE$ |
| PATHNAME$ | PATHSCAN$ | PEEK | PLAY SOUND | PLAY WAVE |
| POKE | POW | PREFIX | PRINT# | PRINTERCOUNT |
| PROCESS GET PRIORITY | PROCESS SET PRIORITY | PROFILE | PROGRESSBAR | PUT |
| PUT$ | PUT$$ | RAISEEVENT | RANDOMIZE | REDIM |
| REGEXPR | REGISTER | REGREPL | REM | REMAIN$ |
| REMOVE$ | REPEAT$ | REPLACE | REPLACE$ | RESET |
| RESOURCE SAVE FILE | RESUME | RETAIN$ | RETURN | RGB |
| RIGHT$ | RMDIR | RND | ROTATE | ROUND |
| RSET | RTRIM$ | SEC | SECH | SEEK |
| SELECT CASE/END SELECT | SETATTR | SETEOF | SGN | SHELL |
| SHIFT | SHRINK$ | SIN | SINH | SLEEP |
| SPACE$ | SPLIT | SQR | STATIC | STR$ |
| STRDELETE$ | STRING$ | STRINSERT$ | STRPTR | STRREVERSE$ |
| SWAP | SWITCH$ | TALLY | TAN | TANH |
| TCP ACCEPT | TCP CLOSE | TCP LINE INPUT | TCP NOTIFY | TCP OPEN |
...
| THREADCOUNT | THREADED | TIME$ | TIMER | TIX |
| TRACE | TRIM$ | TRUNC | TRY/END TRY | TYPE SET |
| TYPE/END TYPE | UCASE$ | UCODEPAGE | UDP CLOSE | UDP NOTIFY |
| UDP OPEN | UDP RECV | UDP SEND | UNLOCK | UNWRAP$ |
| UTF8TOCHR$ | VAL | VARPTR | VERIFY | WAITKEY$ |
| WEEKDAY | WINDOW GET | WINDOW SET | WRAP$ | WRITE# |
