...
> Summary:
> - **490** official PowerBASIC statement/function keywords currently available for use
>   - **441** implemented or completed by this fork
...
> - **3** documented keywords not yet implemented
>
> Counts are deduplicated official keywords (internal codegen helper names are excluded; they appear in the full [statement-coverage.md](docs/statement-coverage.md) table). Updated through batch 121 (v0.1.121).

**All 490 keywords currently available for use (alphabetical):**
...
| #IF/#ELSEIF/#ELSE/#ENDIF METASTATEMENT | #INCLUDE METASTATEMENT | #LINK METASTATEMENT | #MESSAGES METASTATEMENT | #OPTIMIZE METASTATEMENT |
| #OPTION METASTATEMENT | #PAGE METASTATEMENT | #PBFORMS METASTATEMENT | #REGISTER METASTATEMENT | #RESOURCE METASTATEMENT |
| #STACK METASTATEMENT | #TOOLS METASTATEMENT | #UNIQUE METASTATEMENT | #UTILITY METASTATEMENT | ACODE |
...
| ARRAY SORT | ARRAY SWAP | ARRAY UNIQUE | ASIN | ASINH |
| ASM | ASMDATA / END ASMDATA | ATANH | ATN | BEEP |
| BIN$ | BIT | BIT CALC | BITS$ | BITSE |
| BUILD$ | CALL DWORD | CALLSTK | CALLSTK$ | CBRT |
| CHDIR | CHDRIVE | CHRTOOEM$ | CHRTOUTF8$ | CLASS/END CLASS |
| CLIP$ | CLIPBOARD | CLS | COLOR | COMM CLOSE |
| COMM LINE | COMM LINE INPUT | COMM OPEN | COMM PRINT | COMM RECV |
| COMM RESET | COMM SEND | COMM SET | COMM TIMEOUT | COMMAND$ |
| COSH | COT | COTH | CSC | CSCH |
| CSET | CSET$ | CURDIR$ | DAYNAME$ | DEC$ |
| DECR | DESKTOP GET CLIENT | DESKTOP GET LOC | DESKTOP GET PPI | DESKTOP GET SIZE |
| DIR CLOSE | DIR$ | DISPLAY BROWSE | DISPLAY COLOR | DISPLAY FONT |
| DISPLAY OPENFILE | DISPLAY SAVEFILE | ENVIRON | ENVIRON$ | EQV |
| ERASE | ERF | ERL$ | ERROR | ERROR$ |
| EXP10 | EXP2 | EXPM1 | EXTRACT$ | FIELD |
| FILECOPY | FILENAME$ | FILESCAN | FLOOR | FLUSH |
| FONT END | FONT NEW | FUNCNAME | FUNCNAME$ | GET |
| GET$ | GET$$ | GLOBALMEM | GRAPHIC ARC | GRAPHIC ATTACH |
...
| HOST ADDR | HOST NAME | HYPOT | IF/END IF | IMP |
| IMPORT | INCR | INPUT FLUSH | INPUT# | INTERFACE / END INTERFACE (DIRECT) |
| INTERFACE/END INTERFACE (IDBIND) | ISINFINITE | ISNORMAL | ISWIN | ITERATE |
| JOIN$ | KILL | LET *(WITH TYPES)* | LINE INPUT# | LOCK |
| LOG10 | LOG1P | LOG2 | LPRINT | LPRINT ATTACH |
| LPRINT CLOSE | LPRINT FLUSH | LPRINT FORMFEED | LPRINT$ | LSET |
| LSET$ | MACRO/END MACRO | MAT | MCASE | MCASE$ |
| MEMORY | MENU ADD POPUP | MENU ADD STRING | MENU DELETE | MENU GET STATE |
| MENU GET TEXT | MENU NEW BAR | MENU NEW POPUP | MENU SET STATE | MENU SET TEXT |
| METHOD / END METHOD | METRICS | MKBYT$ | MKCUR$ | MKCUX$ |
| MKD$ | MKDIR | MKDWD$ | MKE$ | MKI$ |
| MKL$ | MKQ$ | MKS$ | MKWRD$ | MONTHNAME$ |
| MOUSEPTR | MSGBOX | NAME | NUL | NUL$ |
| OBJECT | OCT$ | OEMTOCHR$ | ON CALL | ON ERROR |
| ON GOSUB | ON GOTO | OPTION EXPLICIT | PARSE$ | PATHNAME$ |
| PATHSCAN$ | PLAY | PLAY SOUND | PLAY WAVE | PREFIX |
| PRINTER$ | PROCESS GET PRIORITY | PROCESS SET PRIORITY | PROFILE | PUT |
| PUT$ | PUT$$ | READ$ | REDIM | REGEXPR |
| REGISTER | REGREPL | REM | REMAIN$ | REPEAT$ |
| REPLACE | RESET | RESOURCE SAVE FILE | RESOURCE$ | RESUME |
| RETAIN$ | RGB | RMDIR | ROTATE | RSET |
| RSET$ | SEC | SECH | SEEK | SETATTR |
| SETEOF | SHELL | SHIFT | SHRINK$ | SINH |
| SLEEP | SPLIT | STATIC | STRDELETE$ | STRINSERT$ |
...
| THREADED | THREADID | TIX | TRACE | TRUNC |
| TRY/END TRY | TYPE SET | UCODE$ | UCODEPAGE | UDP CLOSE |
| UDP OPEN | UDP RECV | UDP SEND | UNLOCK | UNWRAP$ |
| USING$ | UTF8TOCHR$ | WAITKEY$ | WINDOW GET | WINDOW SET |
