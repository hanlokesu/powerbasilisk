...
| COMM TIMEOUT | COMMAND$ | CONTROL ADD BUTTON | CONTROL ADD CHECKBOX | CONTROL ADD COMBOBOX |
| CONTROL ADD EDITBOX | CONTROL ADD GROUPBOX | CONTROL ADD LISTBOX | CONTROL ADD RADIOBUTTON | CONTROL GET TEXT |
| CONTROL SET TEXT | COS | COSH | COT | COTH |
| CSC | CSCH | CSET | CSET$ | CURDIR$ |
| DATA | DAYNAME$ | DEC$ | DECLARE | DECR |
| DESKTOP GET CLIENT | DESKTOP GET LOC | DESKTOP GET PPI | DESKTOP GET SIZE | DIM |
| DIR CLOSE | DIR$ | DISPLAY BROWSE | DISPLAY COLOR | DISPLAY FONT |
| DISPLAY OPENFILE | DISPLAY SAVEFILE | END | ENVIRON | ENVIRON$ |
| EQV | ERASE | ERF | ERL$ | ERROR |
| ERROR$ | EXIT | EXP | EXP10 | EXP2 |
| EXPM1 | EXTRACT$ | FIELD | FILECOPY | FILENAME$ |
| FILESCAN | FLOOR | FLUSH | FONT END | FONT NEW |
| FOR / NEXT | FORMAT$ | FUNCNAME | FUNCNAME$ | FUNCTION / END FUNCTION |
| GET | GET$ | GET$$ | GLOBAL | GLOBALMEM |
| GRAPHIC ARC | GRAPHIC ATTACH | GRAPHIC BITMAP END | GRAPHIC BITMAP LOAD | GRAPHIC BITMAP NEW |
| GRAPHIC BOX | GRAPHIC CELL | GRAPHIC CELL SIZE | GRAPHIC CHR SIZE | GRAPHIC CLEAR |
| GRAPHIC COLOR | GRAPHIC COPY | GRAPHIC DETACH | GRAPHIC ELLIPSE | GRAPHIC GET BITS |
| GRAPHIC GET CANVAS | GRAPHIC GET CAPTION | GRAPHIC GET CLIENT | GRAPHIC GET CLIP | GRAPHIC GET DC |
| GRAPHIC GET LINES | GRAPHIC GET LOC | GRAPHIC GET MIX | GRAPHIC GET PIXEL | GRAPHIC GET POS |
| GRAPHIC GET PPI | GRAPHIC GET SCALE | GRAPHIC GET SIZE | GRAPHIC GET STRETCHMODE | GRAPHIC GET TEXTALIGN |
| GRAPHIC GET VIEW | GRAPHIC GET WORDWRAP | GRAPHIC GET WRAP | GRAPHIC LINE | GRAPHIC PAINT |
| GRAPHIC PIE | GRAPHIC POLYGON | GRAPHIC POLYLINE | GRAPHIC PRINT | GRAPHIC SAVE |
...
| HEX$ | HI | HOST ADDR | HOST NAME | HYPOT |
| IF | IF/END IF | IMP | IMPORT | INCR |
...
| LCASE$ | LEFT$ | LET | LET *(WITH TYPES)* | LINE INPUT# |
| LOC | LOCAL | LOCK | LOF | LOG |
| LOG10 | LOG1P | LOG2 | LPRINT | LPRINT ATTACH |
| LPRINT CLOSE | LPRINT FLUSH | LPRINT FORMFEED | LPRINT$ | LSET |
| LSET$ | LTRIM$ | MACRO/END MACRO | MAT | MCASE |
| MCASE$ | MEMORY | MENU ADD POPUP | MENU ADD STRING | MENU DELETE |
| MENU GET STATE | MENU GET TEXT | MENU NEW BAR | MENU NEW POPUP | MENU SET STATE |
| MENU SET TEXT | METHOD / END METHOD | METRICS | MID$ | MKBYT$ |
| MKCUR$ | MKCUX$ | MKD$ | MKDIR | MKDWD$ |
| MKE$ | MKI$ | MKL$ | MKQ$ | MKS$ |
| MKWRD$ | MONTHNAME$ | MOUSEPTR | MSGBOX | NAME |
| NOT | NUL | NUL$ | OBJECT | OCT$ |
| OEMTOCHR$ | ON CALL | ON ERROR | ON GOSUB | ON GOTO |
| OPEN | OPTION EXPLICIT | OR | PARSE | PARSE$ |
| PATHNAME$ | PATHSCAN$ | PLAY | PLAY SOUND | PLAY WAVE |
| PREFIX | PRINT# | PRINTER$ | PROCESS GET PRIORITY | PROCESS SET PRIORITY |
| PROFILE | PUT | PUT$ | PUT$$ | RANDOMIZE |
| READ$ | REDIM | REGEXPR | REGISTER | REGREPL |
| REM | REMAIN$ | REMOVE$ | REPEAT$ | REPLACE |
| RESET | RESOURCE SAVE FILE | RESOURCE$ | RESUME | RETAIN$ |
| RETURN | RGB | RIGHT$ | RMDIR | ROTATE |
| ROUND | RSET | RSET$ | RTRIM$ | SEC |
| SECH | SEEK | SELECT CASE/END SELECT | SETATTR | SETEOF |
| SHELL | SHIFT | SHRINK$ | SIN | SINH |
| SLEEP | SPACE$ | SPLIT | SQR | STATIC |
| STR$ | STRDELETE$ | STRINSERT$ | STRREVERSE$ | SWAP |
| SWITCH | TAB$ | TAN | TANH | TCP ACCEPT |
| TCP CLOSE | TCP LINE INPUT | TCP OPEN | TCP PRINT | TCP RECV |
| TCP SEND | THREAD CLOSE | THREAD CREATE | THREAD GET PRIORITY | THREAD RESUME |
| THREAD SET PRIORITY | THREAD STATUS | THREAD SUSPEND | THREADED | THREADID |
| TIX | TRACE | TRIM$ | TRUNC | TRY/END TRY |
| TYPE SET | TYPE/END TYPE | UCASE$ | UCODE$ | UCODEPAGE |
| UDP CLOSE | UDP OPEN | UDP RECV | UDP SEND | UNLOCK |
| UNWRAP$ | USING$ | UTF8TOCHR$ | VAL | WAITKEY$ |
| WINDOW | WINDOW GET | WINDOW SET | WRAP$ | WRITE# |
