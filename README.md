...
>   - Upstream `benstopics/powerbasilisk` (last commit 2026-02-18) itself shipped only **~68** core statements/builtins (control flow + basic math/string functions). Those ~68 are **real** - upstream generated actual machine code for them (its compiler had to run the 180k-line Wall Street Raider). Any statement upstream merely *parsed* but did **not** implement (silently dropped, no codegen) does **not** count as upstream - this fork added the real code for those, and they are the 309 above. The CSV label "Established" means *mature in the official PB docs* - it is **not** a count of what upstream implemented.
...
| Keyword | Keyword | Keyword | Keyword | Keyword |
| --- | --- | --- | --- | --- |
| #DEBUG BOUNDS METASTATEMENT | #DEBUG DISPLAY METASTATEMENT | #DEBUG ERROR METASTATEMENT | #DEBUG NUMERIC METASTATEMENT | #OPTION METASTATEMENT |
| #RESOURCE METASTATEMENT | ABS | ACODE | ACODE$ | ACOS |
| ACOSH | ARRAY ADD | ARRAY ARRAYIX | ARRAY ASSIGN | ARRAY COPY |
| ARRAY DELETE | ARRAY INSERT | ARRAY REDIM DECR | ARRAY REDIM INCR | ARRAY REVERSE |
| ARRAY SCAN | ARRAY SELECT | ARRAY SHUFFLE | ARRAY SWAP | ARRAY UNIQUE |
| ASIN | ASINH | ATANH | ATN | BIN$ |
| BITS$ | BITSE | BUILD$ | CALLSTK$ | CBRT |
| CEIL | CHR$ | CHRTOOEM$ | CHRTOUTF8$ | CLIP$ |
| CLOSE | COMM LINE INPUT | COMMAND$ | COS | COSH |
| COT | COTH | CSC | CSCH | CSET$ |
| CURDIR$ | DAYNAME$ | DEC$ | DECLARE | DESKTOP GET PPI |
| DIM | DIR CLOSE | DIR$ | DISPLAY BROWSE | DISPLAY COLOR |
| DISPLAY FONT | DISPLAY OPENFILE | DISPLAY SAVEFILE | ENVIRON$ | EQV |
| ERF | ERL$ | ERROR$ | EXP | EXP10 |
| EXP2 | EXPM1 | EXTRACT$ | FILENAME$ | FLOOR |
| FONT END | FONT NEW | FORMAT$ | FUNCNAME | FUNCNAME$ |
| GET | GET$ | GET$$ | GRAPHIC ARC | GRAPHIC ATTACH |
| GRAPHIC BITMAP END | GRAPHIC BITMAP LOAD | GRAPHIC BITMAP NEW | GRAPHIC BOX | GRAPHIC CELL |
| GRAPHIC CELL SIZE | GRAPHIC CHR SIZE | GRAPHIC CLEAR | GRAPHIC COLOR | GRAPHIC COPY |
| GRAPHIC DETACH | GRAPHIC ELLIPSE | GRAPHIC GET BITS | GRAPHIC GET CANVAS | GRAPHIC GET CAPTION |
| GRAPHIC GET CLIENT | GRAPHIC GET CLIP | GRAPHIC GET DC | GRAPHIC GET LINES | GRAPHIC GET LOC |
| GRAPHIC GET MIX | GRAPHIC GET PIXEL | GRAPHIC GET POS | GRAPHIC GET PPI | GRAPHIC GET SCALE |
| GRAPHIC GET SIZE | GRAPHIC GET STRETCHMODE | GRAPHIC GET TEXTALIGN | GRAPHIC GET VIEW | GRAPHIC GET WORDWRAP |
| GRAPHIC GET WRAP | GRAPHIC LINE | GRAPHIC PAINT | GRAPHIC PIE | GRAPHIC POLYGON |
| GRAPHIC POLYLINE | GRAPHIC PRINT | GRAPHIC SAVE | GRAPHIC SCALE | GRAPHIC SET AUTOSIZE |
...
| GRAPHIC STYLE | GRAPHIC TEXT SIZE | GRAPHIC WIDTH | HEX$ | HI |
| HYPOT | IMP | INPUT# | ISINFINITE | ISNORMAL |
| ISWIN | JOIN$ | LCASE$ | LEFT$ | LINE INPUT# |
| LOC | LOF | LOG | LOG10 | LOG1P |
| LOG2 | LPRINT$ | LSET$ | LTRIM$ | MCASE |
| MCASE$ | MENU ADD POPUP | MENU ADD STRING | MENU DELETE | MENU GET STATE |
| MENU GET TEXT | MENU NEW BAR | MENU NEW POPUP | MENU SET STATE | MENU SET TEXT |
| METRICS | MONTHNAME$ | NUL$ | OCT$ | OEMTOCHR$ |
| ON CALL | ON GOSUB | ON GOTO | OPEN | PARSE$ |
| PATHNAME$ | PATHSCAN$ | PLAY | PLAY SOUND | PRINTER$ |
| PUT | PUT$ | PUT$$ | READ$ | REDIM |
| REMAIN$ | REMOVE$ | REPEAT$ | RESOURCE SAVE FILE | RESOURCE$ |
| RETAIN$ | RGB | RIGHT$ | ROUND | RSET$ |
| RTRIM$ | SEC | SECH | SEEK | SETEOF |
| SHRINK$ | SIN | SINH | SPACE$ | SQR |
| STR$ | STRDELETE$ | STRINSERT$ | STRREVERSE$ | SWITCH |
| TAB$ | TAN | TANH | THREADID | TRIM$ |
| TRUNC | UCASE$ | UCODE$ | UNWRAP$ | USING$ |
| UTF8TOCHR$ | WAITKEY$ | WRAP$ | WRITE# | XPRINT ARC |
