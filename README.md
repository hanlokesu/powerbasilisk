...
|---------|---------|---------|---------|---------|
| ABS | DIR CLOSE | GRAPHIC TEXT SIZE | OCT$ | THREADCOUNT |
| ACCEL ATTACH | DIR$ | GRAPHIC WIDTH | OEMTOCHR | THREADED |
| ACODE | DISKFREE | HEADER CTRL | OEMTOCHR$ | THREADID |
| ACODE$ | DISKSIZE | HEX | ON CALL | TIMER |
| ACOS | DISPLAY BROWSE | HEX$ | ON ERROR | TIX |
| ACOSH | DISPLAY COLOR | HI | ON GOSUB | TRACE |
| AND | DISPLAY FONT | HIWRD | ON GOTO | TRIM |
| ARRAY ADD | DISPLAY OPENFILE | HOST ADDR | OPEN | TRIM$ |
| ARRAY ARRAYIX | DISPLAY SAVEFILE | HOST NAME | OPTION EXPLICIT | TRUNC |
| ARRAY ASSIGN | DOUBLE | HYPOT | OR | TRY/END TRY |
| ARRAY COPY | END | IF | PARSE | TYPE SET |
| ARRAY DELETE | ENVIRON | IF/END IF | PARSE$ | TYPE/END TYPE |
| ARRAY INSERT | ENVIRON$ | IIF | PARSECOUNT | UCASE |
| ARRAY REDIM DECR | EOF | IMAGELIST COUNT | PATHNAME | UCASE$ |
| ARRAY REDIM INCR | EQV | IMAGELIST KILL | PATHNAME$ | UCODE$ |
| ARRAY REVERSE | ERASE | IMAGELIST NEW | PATHSCAN | UCODEPAGE |
| ARRAY SCAN | ERF | IMP | PATHSCAN$ | UDP CLOSE |
| ARRAY SELECT | ERL | IMPORT | PEEK | UDP NOTIFY |
| ARRAY SHUFFLE | ERL$ | INCR | PLAY | UDP OPEN |
| ARRAY SORT | ERR | INPUT FLUSH | PLAY SOUND | UDP RECV |
| ARRAY SWAP | ERRCLEAR | INPUT# | PLAY WAVE | UDP SEND |
| ARRAY TAGARRAY | ERROR | INSTANCE | POKE | UNLOCK |
| ARRAY TAGARRAY ERASE | ERROR$ | INSTR | PREFIX | UNWRAP |
| ARRAY UNIQUE | EVENT SOURCE | INT | PRINT# | UNWRAP$ |
| ASC | EVENTS | INTEGER | PRINTER$ | USING |
| ASIN | EXE | INTERFACE / END INTERFACE (DIRECT) | PRINTERCOUNT | USING$ |
| ASINH | EXIST | INTERFACE/END INTERFACE (IDBIND) | PROCESS GET PRIORITY | UTF8TOCHR |
| ASM | EXIT | ISEVEN | PROCESS SET PRIORITY | UTF8TOCHR$ |
| ASMDATA / END ASMDATA | EXP | ISFALSE | PROFILE | VAL |
| ATANH | EXP10 | ISFILE | PUT | VARPTR |
| ATN | EXP2 | ISFOLDER | PUT STR | VERIFY |
| ATN2 | EXPM1 | ISINFINITE | PUT WSTR | WAITKEY |
| BEEP | EXTRACT | ISNORMAL | PUT$ | WAITKEY$ |
| BGR | EXTRACT$ | ISODD | PUT$$ | WINDOW GET |
| BIN | FIELD | ISTRUE | QUAD | WINDOW SET |
| BIN$ | FILEATTR | ISWIN | RAISEEVENT | WORD |
| BIT | FILECOPY | ITERATE | RANDOMIZE | WRAP |
| BIT CALC | FILENAME | JOIN$ | READ | WRAP$ |
| BITS | FILENAME$ | KILL | READ$ | WRITE |
| BITS$ | FILESCAN | LCASE | REDIM | WRITE# |
| BITSE | FIX | LCASE$ | REGEXPR | XOR |
| BUILD | FLOOR | LEFT | REGISTER | XPRINT ARC |
| BUILD$ | FLUSH | LEFT$ | REGREPL | XPRINT ATTACH |
| BYTE | FONT END | LEN | REM | XPRINT BOX |
| CALL | FONT NEW | LET | REMAIN | XPRINT CANCEL |
| CALL DWORD | FOR / NEXT | LET *(WITH OBJECTS)* | REMAIN$ | XPRINT CELL |
| CALLSTK | FORMAT | LET *(WITH TYPES)* | REMOVE | XPRINT CELL SIZE |
| CALLSTK$ | FORMAT$ | LET *(WITH VARIANTS)* | REMOVE$ | XPRINT CHR SIZE |
| CALLSTKCOUNT | FRAC | LINE INPUT# | REPEAT | XPRINT CLOSE |
| CBOOL | FRE | LO | REPEAT$ | XPRINT COLOR |
| CBRT | FREEFILE | LOC | REPLACE | XPRINT COPY |
| CBYTE | FUNCNAME | LOCAL | RESET | XPRINT ELLIPSE |
| CDBL | FUNCNAME$ | LOCK | RESOURCE SAVE FILE | XPRINT FORMFEED |
| CDWORD | FUNCTION / END FUNCTION | LOF | RESOURCE$ | XPRINT GET ATTACH |
| CEIL | GET | LOG | RESUME | XPRINT GET CANVAS |
| CFLT | GET STR | LOG10 | RETAIN | XPRINT GET CLIENT |
| CHDIR | GET WSTR | LOG1P | RETAIN$ | XPRINT GET CLIP |
| CHDRIVE | GET$ | LOG2 | RETURN | XPRINT GET COLLATE |
| CHOOSE | GET$$ | LONG | RGB | XPRINT GET COLOR |
| CHR | GETATTR | LOWRD | RIGHT | XPRINT GET COLORMODE |
| CHR$ | GLOBAL | LPRINT | RIGHT$ | XPRINT GET COPIES |
| CHRBYTES | GLOBALMEM | LPRINT ATTACH | RMDIR | XPRINT GET DC |
| CHRTOOEM | GRAPHIC ARC | LPRINT CLOSE | RND | XPRINT GET DUPLEX |
| CHRTOOEM$ | GRAPHIC ATTACH | LPRINT FLUSH | ROTATE | XPRINT GET LINES |
| CHRTOUTF8 | GRAPHIC BITMAP END | LPRINT FORMFEED | ROUND | XPRINT GET MARGIN |
| CHRTOUTF8$ | GRAPHIC BITMAP LOAD | LPRINT$ | RSET | XPRINT GET MIX |
| CLASS/END CLASS | GRAPHIC BITMAP NEW | LSET | RSET$ | XPRINT GET ORIENTATION |
| CLIP | GRAPHIC BOX | LSET$ | RTRIM | XPRINT GET OVERLAP |
| CLIP$ | GRAPHIC CELL | LTRIM | RTRIM$ | XPRINT GET PAGES |
| CLIPBOARD | GRAPHIC CELL SIZE | LTRIM$ | SEC | XPRINT GET PAPER |
| CLOSE | GRAPHIC CHR SIZE | MACRO/END MACRO | SECH | XPRINT GET PAPERS |
| CLS | GRAPHIC CIRCLE | MAK | SEEK | XPRINT GET PIXEL |
| CODEPTR | GRAPHIC CLEAR | MAT | SELECT CASE/END SELECT | XPRINT GET POS |
| COLOR | GRAPHIC COLOR | MAX | SETATTR | XPRINT GET PPI |
| COMM CLOSE | GRAPHIC COPY | MCASE | SETEOF | XPRINT GET QUALITY |
| COMM LINE | GRAPHIC DETACH | MCASE$ | SGN | XPRINT GET SCALE |
| COMM LINE INPUT | GRAPHIC ELLIPSE | MEMORY | SHELL | XPRINT GET SELECTION |
| COMM OPEN | GRAPHIC GET BITS | MEMORY FILL | SHIFT | XPRINT GET SIZE |
| COMM PRINT | GRAPHIC GET CANVAS | MEMORY FILLS | SHRINK | XPRINT GET STRETCHMODE |
| COMM RECV | GRAPHIC GET CAPTION | MENU ADD POPUP | SHRINK$ | XPRINT GET TEXTALIGN |
| COMM RESET | GRAPHIC GET CLIENT | MENU ADD STRING | SIN | XPRINT GET TRAY |
| COMM SEND | GRAPHIC GET CLIP | MENU DELETE | SINGLE | XPRINT GET TRAYS |
| COMM SET | GRAPHIC GET DC | MENU GET STATE | SINH | XPRINT GET WORDWRAP |
| COMM TIMEOUT | GRAPHIC GET LINES | MENU GET TEXT | SIZEOF | XPRINT GET WRAP |
| COMMAND | GRAPHIC GET LOC | MENU NEW BAR | SLEEP | XPRINT IMAGELIST |
| COMMAND$ | GRAPHIC GET MIX | MENU NEW POPUP | SPACE | XPRINT LINE |
| COS | GRAPHIC GET PIXEL | MENU SET STATE | SPACE$ | XPRINT PIE |
| COSH | GRAPHIC GET POS | MENU SET TEXT | SPLIT | XPRINT POLYGON |
| COT | GRAPHIC GET PPI | METHOD / END METHOD | SQR | XPRINT POLYLINE |
| COTH | GRAPHIC GET SCALE | METRICS | STATIC | XPRINT PREVIEW |
| CQUAD | GRAPHIC GET SIZE | MID | STR | XPRINT PRINT |
| CSC | GRAPHIC GET STRETCHMODE | MID$ | STR$ | XPRINT RENDER |
| CSCH | GRAPHIC GET TEXTALIGN | MIN | STRDELETE | XPRINT SCALE |
| CSET | GRAPHIC GET VIEW | MKBYT | STRDELETE$ | XPRINT SET CLIP |
| CSET$ | GRAPHIC GET WORDWRAP | MKBYT$ | STRING | XPRINT SET COLLATE |
| CSNG | GRAPHIC GET WRAP | MKCUR$ | STRINSERT | XPRINT SET COLOR |
| CSTR | GRAPHIC LINE | MKCUX | STRINSERT$ | XPRINT SET COLORMODE |
| CULNG | GRAPHIC PAINT | MKCUX$ | STRPTR | XPRINT SET COPIES |
| CURDIR | GRAPHIC PIE | MKD$ | STRREVERSE | XPRINT SET DUPLEX |
| CURDIR$ | GRAPHIC POLYGON | MKDIR | STRREVERSE$ | XPRINT SET FONT |
| CVBYT | GRAPHIC POLYLINE | MKDWD | SWAP | XPRINT SET MIX |
| CVCUX | GRAPHIC PRINT | MKDWD$ | SWITCH | XPRINT SET ORIENTATION |
| CVDWD | GRAPHIC SAVE | MKE | SWITCH$ | XPRINT SET OVERLAP |
| CVL | GRAPHIC SCALE | MKE$ | TAB$ | XPRINT SET PAGES |
| CVQ | GRAPHIC SCALE PIXELS | MKI$ | TALLY | XPRINT SET PAPER |
| CVW | GRAPHIC SET AUTOSIZE | MKL$ | TAN | XPRINT SET PIXEL |
| CWORD | GRAPHIC SET BITS | MKQ$ | TANH | XPRINT SET POS |
| DATA | GRAPHIC SET CAPTION | MKS | TCP ACCEPT | XPRINT SET QUALITY |
| DATACOUNT | GRAPHIC SET CLIP | MKS$ | TCP CLOSE | XPRINT SET STRETCHMODE |
| DAYNAME | GRAPHIC SET FIXED | MKWRD | TCP LINE INPUT | XPRINT SET TEXTALIGN |
| DAYNAME$ | GRAPHIC SET FONT | MKWRD$ | TCP NOTIFY | XPRINT SET TRAY |
| DEC | GRAPHIC SET MIX | MOD | TCP OPEN | XPRINT SET WORDWRAP |
| DEC$ | GRAPHIC SET PIXEL | MONTHNAME | TCP PRINT | XPRINT SET WRAP |
| DECLARE | GRAPHIC SET POS | MONTHNAME$ | TCP RECV | XPRINT SPLIT |
| DECR | GRAPHIC SET SIZE | MOUSEPTR | TCP SEND | XPRINT STRETCH |
| DEF | GRAPHIC SET STRETCHMODE | MSGBOX | THREAD CLOSE | XPRINT STYLE |
| DESKTOP GET CLIENT | GRAPHIC SET TEXTALIGN | NAME | THREAD CREATE | XPRINT TEXT SIZE |
| DESKTOP GET LOC | GRAPHIC SET VIEW | NOT | THREAD GET PRIORITY | XPRINT WIDTH |
| DESKTOP GET PPI | GRAPHIC SET VIRTUAL | NUL | THREAD RESUME |  |
| DESKTOP GET SIZE | GRAPHIC SET WORDWRAP | NUL$ | THREAD SET PRIORITY |  |
| DIM | GRAPHIC SET WRAP | OBJECT | THREAD STATUS |  |
...
|---------|---------|---------|---------|
| ABS | EXP2 | LEFT$ | STRPTR |
| ACCEL ATTACH | EXPM1 | LEN | STRREVERSE |
| ACODE | EXTRACT | LET *(WITH OBJECTS)* | STRREVERSE$ |
| ACODE$ | EXTRACT$ | LET *(WITH VARIANTS)* | SWITCH |
| ACOS | FILEATTR | LINE INPUT# | SWITCH$ |
| ACOSH | FILENAME | LO | TAB$ |
| ARRAY ADD | FILENAME$ | LOC | TALLY |
| ARRAY ARRAYIX | FIX | LOF | TAN |
| ARRAY ASSIGN | FLOOR | LOG | TANH |
| ARRAY COPY | FONT END | LOG10 | TCP NOTIFY |
| ARRAY DELETE | FONT NEW | LOG1P | THREADCOUNT |
| ARRAY INSERT | FORMAT | LOG2 | THREADID |
| ARRAY REDIM DECR | FORMAT$ | LONG | TIMER |
| ARRAY REDIM INCR | FRAC | LOWRD | TRIM |
| ARRAY REVERSE | FRE | LPRINT$ | TRIM$ |
| ARRAY SCAN | FREEFILE | LSET$ | TRUNC |
| ARRAY SELECT | FUNCNAME | LTRIM | UCASE |
| ARRAY SHUFFLE | FUNCNAME$ | LTRIM$ | UCASE$ |
| ARRAY SWAP | GET | MAK | UCODE$ |
| ARRAY TAGARRAY | GET STR | MAX | UDP NOTIFY |
| ARRAY TAGARRAY ERASE | GET WSTR | MCASE | UNWRAP |
| ARRAY UNIQUE | GET$ | MCASE$ | UNWRAP$ |
| ASIN | GET$$ | MEMORY FILL | USING |
| ASINH | GETATTR | MEMORY FILLS | USING$ |
| ATANH | GRAPHIC ARC | MENU ADD POPUP | UTF8TOCHR |
| ATN | GRAPHIC ATTACH | MENU ADD STRING | UTF8TOCHR$ |
| ATN2 | GRAPHIC BITMAP END | MENU DELETE | VARPTR |
| BGR | GRAPHIC BITMAP LOAD | MENU GET STATE | VERIFY |
| BIN | GRAPHIC BITMAP NEW | MENU GET TEXT | WAITKEY |
| BIN$ | GRAPHIC BOX | MENU NEW BAR | WAITKEY$ |
| BITS | GRAPHIC CELL | MENU NEW POPUP | WORD |
| BITS$ | GRAPHIC CELL SIZE | MENU SET STATE | WRAP |
| BITSE | GRAPHIC CHR SIZE | MENU SET TEXT | WRAP$ |
| BUILD | GRAPHIC CIRCLE | METRICS | WRITE |
| BUILD$ | GRAPHIC CLEAR | MID | WRITE# |
| BYTE | GRAPHIC COLOR | MIN | XPRINT ARC |
| CALLSTK$ | GRAPHIC COPY | MKBYT | XPRINT ATTACH |
| CALLSTKCOUNT | GRAPHIC DETACH | MKCUX | XPRINT BOX |
| CBOOL | GRAPHIC ELLIPSE | MKDWD | XPRINT CANCEL |
| CBRT | GRAPHIC GET BITS | MKE | XPRINT CELL |
| CBYTE | GRAPHIC GET CANVAS | MKS | XPRINT CELL SIZE |
| CDBL | GRAPHIC GET CAPTION | MKWRD | XPRINT CHR SIZE |
| CDWORD | GRAPHIC GET CLIENT | MOD | XPRINT CLOSE |
| CEIL | GRAPHIC GET CLIP | MONTHNAME | XPRINT COLOR |
| CFLT | GRAPHIC GET DC | MONTHNAME$ | XPRINT COPY |
| CHOOSE | GRAPHIC GET LINES | NUL$ | XPRINT ELLIPSE |
| CHR | GRAPHIC GET LOC | OCT | XPRINT FORMFEED |
| CHR$ | GRAPHIC GET MIX | OCT$ | XPRINT GET ATTACH |
| CHRBYTES | GRAPHIC GET PIXEL | OEMTOCHR | XPRINT GET CANVAS |
| CHRTOOEM | GRAPHIC GET POS | OEMTOCHR$ | XPRINT GET CLIENT |
| CHRTOOEM$ | GRAPHIC GET PPI | ON CALL | XPRINT GET CLIP |
| CHRTOUTF8 | GRAPHIC GET SCALE | ON GOSUB | XPRINT GET COLLATE |
| CHRTOUTF8$ | GRAPHIC GET SIZE | ON GOTO | XPRINT GET COLOR |
| CLIP | GRAPHIC GET STRETCHMODE | OPEN | XPRINT GET COLORMODE |
| CLIP$ | GRAPHIC GET TEXTALIGN | PARSE$ | XPRINT GET COPIES |
| CLOSE | GRAPHIC GET VIEW | PARSECOUNT | XPRINT GET DC |
| CODEPTR | GRAPHIC GET WORDWRAP | PATHNAME | XPRINT GET DUPLEX |
| COMM LINE INPUT | GRAPHIC GET WRAP | PATHNAME$ | XPRINT GET LINES |
| COMMAND | GRAPHIC LINE | PATHSCAN | XPRINT GET MARGIN |
| COMMAND$ | GRAPHIC PAINT | PATHSCAN$ | XPRINT GET MIX |
| COS | GRAPHIC PIE | PEEK | XPRINT GET ORIENTATION |
| COSH | GRAPHIC POLYGON | PLAY | XPRINT GET OVERLAP |
| COT | GRAPHIC POLYLINE | PLAY SOUND | XPRINT GET PAGES |
| COTH | GRAPHIC PRINT | POKE | XPRINT GET PAPER |
| CQUAD | GRAPHIC SAVE | PRINTER$ | XPRINT GET PAPERS |
| CSC | GRAPHIC SCALE | PRINTERCOUNT | XPRINT GET PIXEL |
| CSCH | GRAPHIC SCALE PIXELS | PUT | XPRINT GET POS |
| CSET$ | GRAPHIC SET AUTOSIZE | PUT STR | XPRINT GET PPI |
| CSNG | GRAPHIC SET BITS | PUT WSTR | XPRINT GET QUALITY |
| CSTR | GRAPHIC SET CAPTION | PUT$ | XPRINT GET SCALE |
| CULNG | GRAPHIC SET CLIP | PUT$$ | XPRINT GET SELECTION |
| CURDIR | GRAPHIC SET FIXED | QUAD | XPRINT GET SIZE |
| CURDIR$ | GRAPHIC SET FONT | RAISEEVENT | XPRINT GET STRETCHMODE |
| CVBYT | GRAPHIC SET MIX | READ | XPRINT GET TEXTALIGN |
| CVCUX | GRAPHIC SET PIXEL | READ$ | XPRINT GET TRAY |
| CVDWD | GRAPHIC SET POS | REDIM | XPRINT GET TRAYS |
| CVL | GRAPHIC SET SIZE | REMAIN | XPRINT GET WORDWRAP |
| CVQ | GRAPHIC SET STRETCHMODE | REMAIN$ | XPRINT GET WRAP |
| CVW | GRAPHIC SET TEXTALIGN | REMOVE | XPRINT IMAGELIST |
| CWORD | GRAPHIC SET VIEW | REMOVE$ | XPRINT LINE |
| DATACOUNT | GRAPHIC SET VIRTUAL | REPEAT | XPRINT PIE |
| DAYNAME | GRAPHIC SET WORDWRAP | REPEAT$ | XPRINT POLYGON |
| DAYNAME$ | GRAPHIC SET WRAP | RESOURCE SAVE FILE | XPRINT POLYLINE |
| DEC | GRAPHIC STYLE | RESOURCE$ | XPRINT PREVIEW |
| DEC$ | GRAPHIC TEXT SIZE | RETAIN | XPRINT PRINT |
| DECLARE | GRAPHIC WIDTH | RETAIN$ | XPRINT RENDER |
| DEF | HEADER CTRL | RGB | XPRINT SCALE |
| DESKTOP GET PPI | HEX | RIGHT | XPRINT SET CLIP |
| DIM | HEX$ | RIGHT$ | XPRINT SET COLLATE |
| DIR | HI | RND | XPRINT SET COLOR |
| DIR CLOSE | HIWRD | ROUND | XPRINT SET COLORMODE |
| DIR$ | HYPOT | RSET$ | XPRINT SET COPIES |
| DISKFREE | IIF | RTRIM | XPRINT SET DUPLEX |
| DISKSIZE | IMAGELIST COUNT | RTRIM$ | XPRINT SET FONT |
| DISPLAY BROWSE | IMAGELIST KILL | SEC | XPRINT SET MIX |
| DISPLAY COLOR | IMAGELIST NEW | SECH | XPRINT SET ORIENTATION |
| DISPLAY FONT | IMP | SEEK | XPRINT SET OVERLAP |
| DISPLAY OPENFILE | INPUT# | SETEOF | XPRINT SET PAGES |
| DISPLAY SAVEFILE | INSTANCE | SGN | XPRINT SET PAPER |
| DOUBLE | INSTR | SHRINK | XPRINT SET PIXEL |
| ENVIRON$ | INT | SHRINK$ | XPRINT SET POS |
| EOF | INTEGER | SIN | XPRINT SET QUALITY |
| EQV | ISEVEN | SINGLE | XPRINT SET STRETCHMODE |
| ERF | ISFALSE | SINH | XPRINT SET TEXTALIGN |
| ERL | ISFILE | SIZEOF | XPRINT SET TRAY |
| ERL$ | ISFOLDER | SPACE | XPRINT SET WORDWRAP |
| ERR | ISINFINITE | SPACE$ | XPRINT SET WRAP |
| ERRCLEAR | ISNORMAL | SQR | XPRINT SPLIT |
| ERROR$ | ISODD | STR | XPRINT STRETCH |
| EVENT SOURCE | ISTRUE | STR$ | XPRINT STYLE |
| EVENTS | ISWIN | STRDELETE | XPRINT TEXT SIZE |
| EXE | JOIN$ | STRDELETE$ | XPRINT WIDTH |
| EXIST | LCASE | STRING |  |
| EXP | LCASE$ | STRINSERT |  |
| EXP10 | LEFT | STRINSERT$ |  |
