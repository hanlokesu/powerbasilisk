...
> Summary:
> - **640** official PowerBASIC statement/function keywords currently available for use
>   - **492** implemented or completed by this fork
...
> - **96** DDT/GUI-class keywords deferred (Tier 3)
> - **0** documented keywords not yet implemented
>
> Counts are computed directly from [statement-coverage.csv](docs/statement-coverage.csv) (737 rows, deduplicated). **0** official keywords are not implemented yet; **1** entry is a fork extension, implemented here but not an official PB keyword (ARRAY SELECT). Rows whose internal codegen name uses an underscore are shown here in their spaced form - `GRAPHIC_CIRCLE` appears as `GRAPHIC CIRCLE`; the raw names are in [statement-coverage.md](docs/statement-coverage.md).
> Updated through batch 164 (v0.2.021). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 640 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | DAYNAME | GRAPHIC SET AUTOSIZE | MKWRD$ | TCP SEND |
| ACCEL ATTACH | DAYNAME$ | GRAPHIC SET BITS | MOD | THREAD CLOSE |
| ACODE | DEC | GRAPHIC SET CAPTION | MONTHNAME | THREAD CREATE |
| ACODE$ | DEC$ | GRAPHIC SET CLIP | MONTHNAME$ | THREAD GET PRIORITY |
| ACOS | DECLARE | GRAPHIC SET FIXED | MOUSEPTR | THREAD RESUME |
| ACOSH | DECR | GRAPHIC SET FONT | MSGBOX | THREAD SET PRIORITY |
| AND | DEF | GRAPHIC SET MIX | NAME | THREAD STATUS |
| ARRAY ADD | DESKTOP GET CLIENT | GRAPHIC SET PIXEL | NOT | THREAD SUSPEND |
| ARRAY ARRAYIX | DESKTOP GET LOC | GRAPHIC SET POS | NUL | THREADCOUNT |
| ARRAY ASSIGN | DESKTOP GET PPI | GRAPHIC SET SIZE | NUL$ | THREADED |
| ARRAY COPY | DESKTOP GET SIZE | GRAPHIC SET STRETCHMODE | OBJECT | THREADID |
| ARRAY DELETE | DIALOG CENTER | GRAPHIC SET TEXTALIGN | OCT | TIMER |
| ARRAY INSERT | DIALOG DOEVENTS | GRAPHIC SET VIEW | OCT$ | TIX |
| ARRAY REDIM DECR | DIALOG END | GRAPHIC SET VIRTUAL | OEMTOCHR | TOOLBAR |
| ARRAY REDIM INCR | DIALOG GET SIZE | GRAPHIC SET WORDWRAP | OEMTOCHR$ | TRACE |
| ARRAY REVERSE | DIALOG GET TEXT | GRAPHIC SET WRAP | ON CALL | TREEVIEW |
| ARRAY SCAN | DIALOG NEW | GRAPHIC STYLE | ON ERROR | TRIM |
| ARRAY SHUFFLE | DIALOG SET SIZE | GRAPHIC TEXT SIZE | ON GOSUB | TRIM$ |
| ARRAY SORT | DIALOG SET TEXT | GRAPHIC WIDTH | ON GOTO | TRUNC |
| ARRAY SWAP | DIALOG SHOW MODAL | HEADER | OPEN | TRY/END TRY |
| ARRAY TAGARRAY | DIALOG SHOW STATE | HEX | OPTION EXPLICIT | TYPE SET |
| ARRAY TAGARRAY ERASE | DIM | HEX$ | OR | TYPE/END TYPE |
| ARRAY UNIQUE | DIR | HI | PARSE | UCASE |
| ASC | DIR CLOSE | HIWRD | PARSE$ | UCASE$ |
| ASIN | DIR$ | HOST ADDR | PARSECOUNT | UCODE$ |
| ASINH | DISKFREE | HOST NAME | PATHNAME | UCODEPAGE |
| ASM | DISKSIZE | HYPOT | PATHNAME$ | UDP CLOSE |
| ASMDATA / END ASMDATA | DISPLAY BROWSE | IF | PATHSCAN | UDP NOTIFY |
| ATANH | DISPLAY COLOR | IF/END IF | PATHSCAN$ | UDP OPEN |
| ATN | DISPLAY FONT | IIF | PEEK | UDP RECV |
| ATN2 | DISPLAY OPENFILE | IMAGELIST COUNT | PLAY | UDP SEND |
| BEEP | DISPLAY SAVEFILE | IMAGELIST KILL | PLAY SOUND | UNLOCK |
| BGR | DOUBLE | IMAGELIST NEW | PLAY WAVE | UNWRAP |
| BIN | END | IMP | POKE | UNWRAP$ |
| BIN$ | ENVIRON | IMPORT | PREFIX | USING |
| BIT | ENVIRON$ | INCR | PRINT# | USING$ |
| BIT CALC | EOF | INPUT FLUSH | PRINTER$ | UTF8TOCHR |
| BITS | EQV | INPUT# | PRINTERCOUNT | UTF8TOCHR$ |
| BITS$ | ERASE | INSTANCE | PROCESS GET PRIORITY | VAL |
| BITSE | ERF | INSTR | PROCESS SET PRIORITY | VARPTR |
| BUILD | ERL | INT | PROFILE | VERIFY |
| BUILD$ | ERL$ | INTEGER | PROGRESSBAR | WAITKEY |
| BYTE | ERR | INTERFACE / END INTERFACE (DIRECT) | PUT | WAITKEY$ |
| CALL | ERRCLEAR | INTERFACE/END INTERFACE (IDBIND) | PUT STR | WINDOW GET |
| CALL DWORD | ERROR | ISEVEN | PUT WSTR | WINDOW SET |
| CALLSTK | ERROR$ | ISFALSE | PUT$ | WORD |
| CALLSTK$ | EVENT SOURCE | ISFILE | PUT$$ | WRAP |
| CALLSTKCOUNT | EVENTS | ISFOLDER | QUAD | WRAP$ |
| CBOOL | EXE | ISINFINITE | RAISEEVENT | WRITE |
| CBRT | EXIST | ISNORMAL | RANDOMIZE | WRITE# |
| CBYTE | EXIT | ISODD | READ | XOR |
| CDBL | EXP | ISTRUE | READ$ | XPRINT ARC |
| CDWORD | EXP10 | ISWIN | REDIM | XPRINT ATTACH |
| CEIL | EXP2 | ITERATE | REGEXPR | XPRINT BOX |
| CFLT | EXPM1 | JOIN$ | REGISTER | XPRINT CANCEL |
| CHDIR | EXTRACT | KILL | REGREPL | XPRINT CELL |
| CHDRIVE | EXTRACT$ | LCASE | REM | XPRINT CELL SIZE |
| CHOOSE | FIELD | LCASE$ | REMAIN | XPRINT CHR SIZE |
| CHR | FILEATTR | LEFT | REMAIN$ | XPRINT CLOSE |
| CHR$ | FILECOPY | LEFT$ | REMOVE | XPRINT COLOR |
| CHRBYTES | FILENAME | LEN | REMOVE$ | XPRINT COPY |
| CHRTOOEM | FILENAME$ | LET | REPEAT | XPRINT ELLIPSE |
| CHRTOOEM$ | FILESCAN | LET *(WITH OBJECTS)* | REPEAT$ | XPRINT FORMFEED |
| CHRTOUTF8 | FIX | LET *(WITH TYPES)* | REPLACE | XPRINT GET ATTACH |
| CHRTOUTF8$ | FLOOR | LET *(WITH VARIANTS)* | RESET | XPRINT GET CANVAS |
| CLASS/END CLASS | FLUSH | LINE INPUT# | RESOURCE SAVE FILE | XPRINT GET CLIENT |
| CLIP | FONT END | LISTVIEW | RESOURCE$ | XPRINT GET CLIP |
| CLIP$ | FONT NEW | LO | RESUME | XPRINT GET COLLATE |
| CLIPBOARD | FOR / NEXT | LOC | RETAIN | XPRINT GET COLOR |
| CLOSE | FORMAT | LOCAL | RETAIN$ | XPRINT GET COLORMODE |
| CLS | FORMAT$ | LOCK | RETURN | XPRINT GET COPIES |
| CODEPTR | FRAC | LOF | RGB | XPRINT GET DC |
| COLOR | FRE | LOG | RIGHT | XPRINT GET DUPLEX |
| COMM CLOSE | FREEFILE | LOG10 | RIGHT$ | XPRINT GET LINES |
| COMM LINE | FUNCNAME | LOG1P | RMDIR | XPRINT GET MARGIN |
| COMM LINE INPUT | FUNCNAME$ | LOG2 | RND | XPRINT GET MIX |
| COMM OPEN | FUNCTION / END FUNCTION | LONG | ROTATE | XPRINT GET ORIENTATION |
| COMM PRINT | GET | LOWRD | ROUND | XPRINT GET OVERLAP |
| COMM RECV | GET STR | LPRINT | RSET | XPRINT GET PAGES |
| COMM RESET | GET WSTR | LPRINT ATTACH | RSET$ | XPRINT GET PAPER |
| COMM SEND | GET$ | LPRINT CLOSE | RTRIM | XPRINT GET PAPERS |
| COMM SET | GET$$ | LPRINT FLUSH | RTRIM$ | XPRINT GET PIXEL |
| COMM TIMEOUT | GETATTR | LPRINT FORMFEED | SEC | XPRINT GET POS |
| COMMAND | GLOBAL | LPRINT$ | SECH | XPRINT GET PPI |
| COMMAND$ | GLOBALMEM | LSET | SEEK | XPRINT GET QUALITY |
| CONTROL ADD BUTTON | GRAPHIC ARC | LSET$ | SELECT CASE/END SELECT | XPRINT GET SCALE |
| CONTROL ADD CHECKBOX | GRAPHIC ATTACH | LTRIM | SETATTR | XPRINT GET SELECTION |
| CONTROL ADD COMBOBOX | GRAPHIC BITMAP END | LTRIM$ | SETEOF | XPRINT GET SIZE |
| CONTROL ADD LABEL | GRAPHIC BITMAP LOAD | MACRO/END MACRO | SGN | XPRINT GET STRETCHMODE |
| CONTROL ADD LISTBOX | GRAPHIC BITMAP NEW | MAK | SHELL | XPRINT GET TEXTALIGN |
| CONTROL ADD LISTVIEW | GRAPHIC BOX | MAT | SHIFT | XPRINT GET TRAY |
| CONTROL ADD PROGRESSBAR | GRAPHIC CELL | MAX | SHRINK | XPRINT GET TRAYS |
| CONTROL ADD SCROLLBAR | GRAPHIC CELL SIZE | MCASE | SHRINK$ | XPRINT GET WORDWRAP |
| CONTROL ADD STATUSBAR | GRAPHIC CHR SIZE | MCASE$ | SIN | XPRINT GET WRAP |
| CONTROL ADD TOOLBAR | GRAPHIC CIRCLE | MEMORY | SINGLE | XPRINT IMAGELIST |
| CONTROL ADD TREEVIEW | GRAPHIC CLEAR | MEMORY FILL | SINH | XPRINT LINE |
| CONTROL ADDSTRING | GRAPHIC COLOR | MEMORY FILLS | SIZEOF | XPRINT PIE |
| CONTROL DISABLE | GRAPHIC COPY | MENU ADD POPUP | SLEEP | XPRINT POLYGON |
| CONTROL ENABLE | GRAPHIC DETACH | MENU ADD STRING | SPACE | XPRINT POLYLINE |
| CONTROL GET CHECK | GRAPHIC ELLIPSE | MENU DELETE | SPACE$ | XPRINT PREVIEW |
| CONTROL GET TEXT | GRAPHIC GET BITS | MENU GET STATE | SPLIT | XPRINT PRINT |
| CONTROL HIDE | GRAPHIC GET CANVAS | MENU GET TEXT | SQR | XPRINT RENDER |
| CONTROL KILL | GRAPHIC GET CAPTION | MENU NEW BAR | STATIC | XPRINT SCALE |
| CONTROL SET CHECK | GRAPHIC GET CLIENT | MENU NEW POPUP | STATUSBAR | XPRINT SET CLIP |
| CONTROL SET TEXT | GRAPHIC GET CLIP | MENU SET STATE | STR | XPRINT SET COLLATE |
| COS | GRAPHIC GET DC | MENU SET TEXT | STR$ | XPRINT SET COLOR |
| COSH | GRAPHIC GET LINES | METHOD / END METHOD | STRDELETE | XPRINT SET COLORMODE |
| COT | GRAPHIC GET LOC | METRICS | STRDELETE$ | XPRINT SET COPIES |
| COTH | GRAPHIC GET MIX | MID | STRING | XPRINT SET DUPLEX |
| CQUAD | GRAPHIC GET PIXEL | MID$ | STRINSERT | XPRINT SET FONT |
| CSC | GRAPHIC GET POS | MIN | STRINSERT$ | XPRINT SET MIX |
| CSCH | GRAPHIC GET PPI | MKBYT | STRPTR | XPRINT SET ORIENTATION |
| CSET | GRAPHIC GET SCALE | MKBYT$ | STRREVERSE | XPRINT SET OVERLAP |
| CSET$ | GRAPHIC GET SIZE | MKCUR$ | STRREVERSE$ | XPRINT SET PAGES |
| CSNG | GRAPHIC GET STRETCHMODE | MKCUX | SWAP | XPRINT SET PAPER |
| CSTR | GRAPHIC GET TEXTALIGN | MKCUX$ | SWITCH | XPRINT SET PIXEL |
| CULNG | GRAPHIC GET VIEW | MKD$ | SWITCH$ | XPRINT SET POS |
| CURDIR | GRAPHIC GET WORDWRAP | MKDIR | TAB$ | XPRINT SET QUALITY |
| CURDIR$ | GRAPHIC GET WRAP | MKDWD | TALLY | XPRINT SET STRETCHMODE |
| CVBYT | GRAPHIC LINE | MKDWD$ | TAN | XPRINT SET TEXTALIGN |
| CVCUX | GRAPHIC PAINT | MKE | TANH | XPRINT SET TRAY |
| CVDWD | GRAPHIC PIE | MKE$ | TCP ACCEPT | XPRINT SET WORDWRAP |
| CVL | GRAPHIC POLYGON | MKI$ | TCP CLOSE | XPRINT SET WRAP |
| CVQ | GRAPHIC POLYLINE | MKL$ | TCP LINE INPUT | XPRINT SPLIT |
| CVW | GRAPHIC PRINT | MKQ$ | TCP NOTIFY | XPRINT STRETCH |
| CWORD | GRAPHIC SAVE | MKS | TCP OPEN | XPRINT STYLE |
| DATA | GRAPHIC SCALE | MKS$ | TCP PRINT | XPRINT TEXT SIZE |
| DATACOUNT | GRAPHIC SCALE PIXELS | MKWRD | TCP RECV | XPRINT WIDTH |

## **Newly** implemented by this branch

> **492 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DISPLAY BROWSE | ISEVEN | STATUSBAR |
| ACCEL ATTACH | DISPLAY COLOR | ISFALSE | STR |
| ACODE | DISPLAY FONT | ISFILE | STR$ |
| ACODE$ | DISPLAY OPENFILE | ISFOLDER | STRDELETE |
| ACOS | DISPLAY SAVEFILE | ISINFINITE | STRDELETE$ |
| ACOSH | DOUBLE | ISNORMAL | STRING |
| ARRAY ADD | ENVIRON$ | ISODD | STRINSERT |
| ARRAY ARRAYIX | EOF | ISTRUE | STRINSERT$ |
| ARRAY ASSIGN | EQV | ISWIN | STRPTR |
| ARRAY COPY | ERF | JOIN$ | STRREVERSE |
| ARRAY DELETE | ERL | LCASE | STRREVERSE$ |
| ARRAY INSERT | ERL$ | LCASE$ | SWITCH |
| ARRAY REDIM DECR | ERR | LEFT | SWITCH$ |
| ARRAY REDIM INCR | ERRCLEAR | LEFT$ | TAB$ |
| ARRAY REVERSE | ERROR$ | LEN | TALLY |
| ARRAY SCAN | EVENT SOURCE | LET *(WITH OBJECTS)* | TAN |
| ARRAY SHUFFLE | EVENTS | LET *(WITH VARIANTS)* | TANH |
| ARRAY SWAP | EXE | LINE INPUT# | TCP NOTIFY |
| ARRAY TAGARRAY | EXIST | LISTVIEW | THREADCOUNT |
| ARRAY TAGARRAY ERASE | EXP | LO | THREADID |
| ARRAY UNIQUE | EXP10 | LOC | TIMER |
| ASIN | EXP2 | LOF | TOOLBAR |
| ASINH | EXPM1 | LOG | TREEVIEW |
| ATANH | EXTRACT | LOG10 | TRIM |
| ATN | EXTRACT$ | LOG1P | TRIM$ |
| ATN2 | FILEATTR | LOG2 | TRUNC |
| BGR | FILENAME | LONG | UCASE |
| BIN | FILENAME$ | LOWRD | UCASE$ |
| BIN$ | FIX | LPRINT$ | UCODE$ |
| BITS | FLOOR | LSET$ | UCODEPAGE |
| BITS$ | FONT END | LTRIM | UDP NOTIFY |
| BITSE | FONT NEW | LTRIM$ | UNWRAP |
| BUILD | FORMAT | MAK | UNWRAP$ |
| BUILD$ | FORMAT$ | MAX | USING |
| BYTE | FRAC | MCASE | USING$ |
| CALLSTK$ | FRE | MCASE$ | UTF8TOCHR |
| CALLSTKCOUNT | FREEFILE | MEMORY FILL | UTF8TOCHR$ |
| CBOOL | FUNCNAME | MEMORY FILLS | VARPTR |
| CBRT | FUNCNAME$ | MENU ADD POPUP | VERIFY |
| CBYTE | GET | MENU ADD STRING | WAITKEY |
| CDBL | GET STR | MENU DELETE | WAITKEY$ |
| CDWORD | GET WSTR | MENU GET STATE | WORD |
| CEIL | GET$ | MENU GET TEXT | WRAP |
| CFLT | GET$$ | MENU NEW BAR | WRAP$ |
| CHOOSE | GETATTR | MENU NEW POPUP | WRITE |
| CHR | GRAPHIC ARC | MENU SET STATE | WRITE# |
| CHR$ | GRAPHIC ATTACH | MENU SET TEXT | XPRINT ARC |
| CHRBYTES | GRAPHIC BITMAP END | METRICS | XPRINT ATTACH |
| CHRTOOEM | GRAPHIC BITMAP LOAD | MID | XPRINT BOX |
| CHRTOOEM$ | GRAPHIC BITMAP NEW | MIN | XPRINT CANCEL |
| CHRTOUTF8 | GRAPHIC BOX | MKBYT | XPRINT CELL |
| CHRTOUTF8$ | GRAPHIC CELL | MKCUX | XPRINT CELL SIZE |
| CLIP | GRAPHIC CELL SIZE | MKDWD | XPRINT CHR SIZE |
| CLIP$ | GRAPHIC CHR SIZE | MKE | XPRINT CLOSE |
| CLOSE | GRAPHIC CIRCLE | MKS | XPRINT COLOR |
| CODEPTR | GRAPHIC CLEAR | MKWRD | XPRINT COPY |
| COMM LINE INPUT | GRAPHIC COLOR | MOD | XPRINT ELLIPSE |
| COMMAND | GRAPHIC COPY | MONTHNAME | XPRINT FORMFEED |
| COMMAND$ | GRAPHIC DETACH | MONTHNAME$ | XPRINT GET ATTACH |
| CONTROL ADD BUTTON | GRAPHIC ELLIPSE | NUL$ | XPRINT GET CANVAS |
| CONTROL ADD CHECKBOX | GRAPHIC GET BITS | OCT | XPRINT GET CLIENT |
| CONTROL ADD COMBOBOX | GRAPHIC GET CANVAS | OCT$ | XPRINT GET CLIP |
| CONTROL ADD LABEL | GRAPHIC GET CAPTION | OEMTOCHR | XPRINT GET COLLATE |
| CONTROL ADD LISTBOX | GRAPHIC GET CLIENT | OEMTOCHR$ | XPRINT GET COLOR |
| CONTROL ADD LISTVIEW | GRAPHIC GET CLIP | ON CALL | XPRINT GET COLORMODE |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET DC | ON GOSUB | XPRINT GET COPIES |
| CONTROL ADD SCROLLBAR | GRAPHIC GET LINES | ON GOTO | XPRINT GET DC |
| CONTROL ADD STATUSBAR | GRAPHIC GET LOC | OPEN | XPRINT GET DUPLEX |
| CONTROL ADD TOOLBAR | GRAPHIC GET MIX | PARSE$ | XPRINT GET LINES |
| CONTROL ADD TREEVIEW | GRAPHIC GET PIXEL | PARSECOUNT | XPRINT GET MARGIN |
| CONTROL ADDSTRING | GRAPHIC GET POS | PATHNAME | XPRINT GET MIX |
| CONTROL DISABLE | GRAPHIC GET PPI | PATHNAME$ | XPRINT GET ORIENTATION |
| CONTROL ENABLE | GRAPHIC GET SCALE | PATHSCAN | XPRINT GET OVERLAP |
| CONTROL GET CHECK | GRAPHIC GET SIZE | PATHSCAN$ | XPRINT GET PAGES |
| CONTROL GET TEXT | GRAPHIC GET STRETCHMODE | PEEK | XPRINT GET PAPER |
| CONTROL HIDE | GRAPHIC GET TEXTALIGN | PLAY | XPRINT GET PAPERS |
| CONTROL KILL | GRAPHIC GET VIEW | PLAY SOUND | XPRINT GET PIXEL |
| CONTROL SET CHECK | GRAPHIC GET WORDWRAP | POKE | XPRINT GET POS |
| CONTROL SET TEXT | GRAPHIC GET WRAP | PRINTER$ | XPRINT GET PPI |
| COS | GRAPHIC LINE | PRINTERCOUNT | XPRINT GET QUALITY |
| COSH | GRAPHIC PAINT | PROGRESSBAR | XPRINT GET SCALE |
| COT | GRAPHIC PIE | PUT | XPRINT GET SELECTION |
| COTH | GRAPHIC POLYGON | PUT STR | XPRINT GET SIZE |
| CQUAD | GRAPHIC POLYLINE | PUT WSTR | XPRINT GET STRETCHMODE |
| CSC | GRAPHIC PRINT | PUT$ | XPRINT GET TEXTALIGN |
| CSCH | GRAPHIC SAVE | PUT$$ | XPRINT GET TRAY |
| CSET$ | GRAPHIC SCALE | QUAD | XPRINT GET TRAYS |
| CSNG | GRAPHIC SCALE PIXELS | RAISEEVENT | XPRINT GET WORDWRAP |
| CSTR | GRAPHIC SET AUTOSIZE | READ | XPRINT GET WRAP |
| CULNG | GRAPHIC SET BITS | READ$ | XPRINT IMAGELIST |
| CURDIR | GRAPHIC SET CAPTION | REDIM | XPRINT LINE |
| CURDIR$ | GRAPHIC SET CLIP | REMAIN | XPRINT PIE |
| CVBYT | GRAPHIC SET FIXED | REMAIN$ | XPRINT POLYGON |
| CVCUX | GRAPHIC SET FONT | REMOVE | XPRINT POLYLINE |
| CVDWD | GRAPHIC SET MIX | REMOVE$ | XPRINT PREVIEW |
| CVL | GRAPHIC SET PIXEL | REPEAT | XPRINT PRINT |
| CVQ | GRAPHIC SET POS | REPEAT$ | XPRINT RENDER |
| CVW | GRAPHIC SET SIZE | RESOURCE SAVE FILE | XPRINT SCALE |
| CWORD | GRAPHIC SET STRETCHMODE | RESOURCE$ | XPRINT SET CLIP |
| DATACOUNT | GRAPHIC SET TEXTALIGN | RETAIN | XPRINT SET COLLATE |
| DAYNAME | GRAPHIC SET VIEW | RETAIN$ | XPRINT SET COLOR |
| DAYNAME$ | GRAPHIC SET VIRTUAL | RGB | XPRINT SET COLORMODE |
| DEC | GRAPHIC SET WORDWRAP | RIGHT | XPRINT SET COPIES |
| DEC$ | GRAPHIC SET WRAP | RIGHT$ | XPRINT SET DUPLEX |
| DECLARE | GRAPHIC STYLE | RND | XPRINT SET FONT |
| DEF | GRAPHIC TEXT SIZE | ROUND | XPRINT SET MIX |
| DESKTOP GET PPI | GRAPHIC WIDTH | RSET$ | XPRINT SET ORIENTATION |
| DIALOG CENTER | HEADER | RTRIM | XPRINT SET OVERLAP |
| DIALOG DOEVENTS | HEX | RTRIM$ | XPRINT SET PAGES |
| DIALOG END | HEX$ | SEC | XPRINT SET PAPER |
| DIALOG GET SIZE | HI | SECH | XPRINT SET PIXEL |
| DIALOG GET TEXT | HIWRD | SEEK | XPRINT SET POS |
| DIALOG NEW | HYPOT | SETEOF | XPRINT SET QUALITY |
| DIALOG SET SIZE | IIF | SGN | XPRINT SET STRETCHMODE |
| DIALOG SET TEXT | IMAGELIST COUNT | SHRINK | XPRINT SET TEXTALIGN |
| DIALOG SHOW MODAL | IMAGELIST KILL | SHRINK$ | XPRINT SET TRAY |
| DIALOG SHOW STATE | IMAGELIST NEW | SIN | XPRINT SET WORDWRAP |
| DIM | IMP | SINGLE | XPRINT SET WRAP |
| DIR | INPUT# | SINH | XPRINT SPLIT |
| DIR CLOSE | INSTANCE | SIZEOF | XPRINT STRETCH |
| DIR$ | INSTR | SPACE | XPRINT STYLE |
| DISKFREE | INT | SPACE$ | XPRINT TEXT SIZE |
| DISKSIZE | INTEGER | SQR | XPRINT WIDTH |
