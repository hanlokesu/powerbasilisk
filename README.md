...
>   - **486** implemented or completed by this fork
>   - **149** official keywords that are long established in the PB documentation, listed in the established-keywords table below. The CSV label "Established" means *mature in the official PB docs* - it is **not** a claim about what upstream shipped.
> - **103** DDT/GUI-class keywords deferred (Tier 3)
> - **0** documented keywords not yet implemented
>
> Counts are computed directly from [statement-coverage.csv](docs/statement-coverage.csv) (738 rows, deduplicated). Rows whose internal codegen name uses an underscore are shown here in their spaced form - `GRAPHIC_CIRCLE` appears as `GRAPHIC CIRCLE`; the raw names are in [statement-coverage.md](docs/statement-coverage.md).
> Updated through batch 158 (v0.2.017). **MILESTONE: 0 non-Tier-3 official keywords remain unimplemented.**

**All 635 keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | DAYNAME | GRAPHIC SET BITS | MKWRD$ | TCP SEND |
| ACCEL ATTACH | DAYNAME$ | GRAPHIC SET CAPTION | MOD | THREAD CLOSE |
| ACODE | DEC | GRAPHIC SET CLIP | MONTHNAME | THREAD CREATE |
| ACODE$ | DEC$ | GRAPHIC SET FIXED | MONTHNAME$ | THREAD GET PRIORITY |
| ACOS | DECLARE | GRAPHIC SET FONT | MOUSEPTR | THREAD RESUME |
| ACOSH | DECR | GRAPHIC SET MIX | MSGBOX | THREAD SET PRIORITY |
| AND | DEF | GRAPHIC SET PIXEL | NAME | THREAD STATUS |
| ARRAY ADD | DESKTOP GET CLIENT | GRAPHIC SET POS | NOT | THREAD SUSPEND |
| ARRAY ARRAYIX | DESKTOP GET LOC | GRAPHIC SET SIZE | NUL | THREADCOUNT |
| ARRAY ASSIGN | DESKTOP GET PPI | GRAPHIC SET STRETCHMODE | NUL$ | THREADED |
| ARRAY COPY | DESKTOP GET SIZE | GRAPHIC SET TEXTALIGN | OBJECT | THREADID |
| ARRAY DELETE | DIALOG CENTER | GRAPHIC SET VIEW | OCT | TIMER |
| ARRAY INSERT | DIALOG DOEVENTS | GRAPHIC SET VIRTUAL | OCT$ | TIX |
| ARRAY REDIM DECR | DIALOG END | GRAPHIC SET WORDWRAP | OEMTOCHR | TRACE |
| ARRAY REDIM INCR | DIALOG GET TEXT | GRAPHIC SET WRAP | OEMTOCHR$ | TREEVIEW |
| ARRAY REVERSE | DIALOG NEW | GRAPHIC STYLE | ON CALL | TRIM |
| ARRAY SCAN | DIALOG SET TEXT | GRAPHIC TEXT SIZE | ON ERROR | TRIM$ |
| ARRAY SELECT | DIALOG SHOW MODAL | GRAPHIC WIDTH | ON GOSUB | TRUNC |
| ARRAY SHUFFLE | DIALOG SHOW STATE | HEADER CTRL | ON GOTO | TRY/END TRY |
| ARRAY SORT | DIM | HEX | OPEN | TYPE SET |
| ARRAY SWAP | DIR | HEX$ | OPTION EXPLICIT | TYPE/END TYPE |
| ARRAY TAGARRAY | DIR CLOSE | HI | OR | UCASE |
| ARRAY TAGARRAY ERASE | DIR$ | HIWRD | PARSE | UCASE$ |
| ARRAY UNIQUE | DISKFREE | HOST ADDR | PARSE$ | UCODE$ |
| ASC | DISKSIZE | HOST NAME | PARSECOUNT | UCODEPAGE |
| ASIN | DISPLAY BROWSE | HYPOT | PATHNAME | UDP CLOSE |
| ASINH | DISPLAY COLOR | IF | PATHNAME$ | UDP NOTIFY |
| ASM | DISPLAY FONT | IF/END IF | PATHSCAN | UDP OPEN |
| ASMDATA / END ASMDATA | DISPLAY OPENFILE | IIF | PATHSCAN$ | UDP RECV |
| ATANH | DISPLAY SAVEFILE | IMAGELIST COUNT | PEEK | UDP SEND |
| ATN | DOUBLE | IMAGELIST KILL | PLAY | UNLOCK |
| ATN2 | END | IMAGELIST NEW | PLAY SOUND | UNWRAP |
| BEEP | ENVIRON | IMP | PLAY WAVE | UNWRAP$ |
| BGR | ENVIRON$ | IMPORT | POKE | USING |
| BIN | EOF | INCR | PREFIX | USING$ |
| BIN$ | EQV | INPUT FLUSH | PRINT# | UTF8TOCHR |
| BIT | ERASE | INPUT# | PRINTER$ | UTF8TOCHR$ |
| BIT CALC | ERF | INSTANCE | PRINTERCOUNT | VAL |
| BITS | ERL | INSTR | PROCESS GET PRIORITY | VARPTR |
| BITS$ | ERL$ | INT | PROCESS SET PRIORITY | VERIFY |
| BITSE | ERR | INTEGER | PROFILE | WAITKEY |
| BUILD | ERRCLEAR | INTERFACE / END INTERFACE (DIRECT) | PROGRESSBAR | WAITKEY$ |
| BUILD$ | ERROR | INTERFACE/END INTERFACE (IDBIND) | PUT | WINDOW GET |
| BYTE | ERROR$ | ISEVEN | PUT STR | WINDOW SET |
| CALL | EVENT SOURCE | ISFALSE | PUT WSTR | WORD |
| CALL DWORD | EVENTS | ISFILE | PUT$ | WRAP |
| CALLSTK | EXE | ISFOLDER | PUT$$ | WRAP$ |
| CALLSTK$ | EXIST | ISINFINITE | QUAD | WRITE |
| CALLSTKCOUNT | EXIT | ISNORMAL | RAISEEVENT | WRITE# |
| CBOOL | EXP | ISODD | RANDOMIZE | XOR |
| CBRT | EXP10 | ISTRUE | READ | XPRINT ARC |
| CBYTE | EXP2 | ISWIN | READ$ | XPRINT ATTACH |
| CDBL | EXPM1 | ITERATE | REDIM | XPRINT BOX |
| CDWORD | EXTRACT | JOIN$ | REGEXPR | XPRINT CANCEL |
| CEIL | EXTRACT$ | KILL | REGISTER | XPRINT CELL |
| CFLT | FIELD | LCASE | REGREPL | XPRINT CELL SIZE |
| CHDIR | FILEATTR | LCASE$ | REM | XPRINT CHR SIZE |
| CHDRIVE | FILECOPY | LEFT | REMAIN | XPRINT CLOSE |
| CHOOSE | FILENAME | LEFT$ | REMAIN$ | XPRINT COLOR |
| CHR | FILENAME$ | LEN | REMOVE | XPRINT COPY |
| CHR$ | FILESCAN | LET | REMOVE$ | XPRINT ELLIPSE |
| CHRBYTES | FIX | LET *(WITH OBJECTS)* | REPEAT | XPRINT FORMFEED |
| CHRTOOEM | FLOOR | LET *(WITH TYPES)* | REPEAT$ | XPRINT GET ATTACH |
| CHRTOOEM$ | FLUSH | LET *(WITH VARIANTS)* | REPLACE | XPRINT GET CANVAS |
| CHRTOUTF8 | FONT END | LINE INPUT# | RESET | XPRINT GET CLIENT |
| CHRTOUTF8$ | FONT NEW | LISTVIEW | RESOURCE SAVE FILE | XPRINT GET CLIP |
| CLASS/END CLASS | FOR / NEXT | LO | RESOURCE$ | XPRINT GET COLLATE |
| CLIP | FORMAT | LOC | RESUME | XPRINT GET COLOR |
| CLIP$ | FORMAT$ | LOCAL | RETAIN | XPRINT GET COLORMODE |
| CLIPBOARD | FRAC | LOCK | RETAIN$ | XPRINT GET COPIES |
| CLOSE | FRE | LOF | RETURN | XPRINT GET DC |
| CLS | FREEFILE | LOG | RGB | XPRINT GET DUPLEX |
| CODEPTR | FUNCNAME | LOG10 | RIGHT | XPRINT GET LINES |
| COLOR | FUNCNAME$ | LOG1P | RIGHT$ | XPRINT GET MARGIN |
| COMM CLOSE | FUNCTION / END FUNCTION | LOG2 | RMDIR | XPRINT GET MIX |
| COMM LINE | GET | LONG | RND | XPRINT GET ORIENTATION |
| COMM LINE INPUT | GET STR | LOWRD | ROTATE | XPRINT GET OVERLAP |
| COMM OPEN | GET WSTR | LPRINT | ROUND | XPRINT GET PAGES |
| COMM PRINT | GET$ | LPRINT ATTACH | RSET | XPRINT GET PAPER |
| COMM RECV | GET$$ | LPRINT CLOSE | RSET$ | XPRINT GET PAPERS |
| COMM RESET | GETATTR | LPRINT FLUSH | RTRIM | XPRINT GET PIXEL |
| COMM SEND | GLOBAL | LPRINT FORMFEED | RTRIM$ | XPRINT GET POS |
| COMM SET | GLOBALMEM | LPRINT$ | SEC | XPRINT GET PPI |
| COMM TIMEOUT | GRAPHIC ARC | LSET | SECH | XPRINT GET QUALITY |
| COMMAND | GRAPHIC ATTACH | LSET$ | SEEK | XPRINT GET SCALE |
| COMMAND$ | GRAPHIC BITMAP END | LTRIM | SELECT CASE/END SELECT | XPRINT GET SELECTION |
| CONTROL ADD BUTTON | GRAPHIC BITMAP LOAD | LTRIM$ | SETATTR | XPRINT GET SIZE |
| CONTROL ADD CHECKBOX | GRAPHIC BITMAP NEW | MACRO/END MACRO | SETEOF | XPRINT GET STRETCHMODE |
| CONTROL ADD COMBOBOX | GRAPHIC BOX | MAK | SGN | XPRINT GET TEXTALIGN |
| CONTROL ADD LABEL | GRAPHIC CELL | MAT | SHELL | XPRINT GET TRAY |
| CONTROL ADD LISTBOX | GRAPHIC CELL SIZE | MAX | SHIFT | XPRINT GET TRAYS |
| CONTROL ADD LISTVIEW | GRAPHIC CHR SIZE | MCASE | SHRINK | XPRINT GET WORDWRAP |
| CONTROL ADD PROGRESSBAR | GRAPHIC CIRCLE | MCASE$ | SHRINK$ | XPRINT GET WRAP |
| CONTROL ADD SCROLLBAR | GRAPHIC CLEAR | MEMORY | SIN | XPRINT IMAGELIST |
| CONTROL ADD TREEVIEW | GRAPHIC COLOR | MEMORY FILL | SINGLE | XPRINT LINE |
| CONTROL ADDSTRING | GRAPHIC COPY | MEMORY FILLS | SINH | XPRINT PIE |
| CONTROL DISABLE | GRAPHIC DETACH | MENU ADD POPUP | SIZEOF | XPRINT POLYGON |
| CONTROL ENABLE | GRAPHIC ELLIPSE | MENU ADD STRING | SLEEP | XPRINT POLYLINE |
| CONTROL GET CHECK | GRAPHIC GET BITS | MENU DELETE | SPACE | XPRINT PREVIEW |
| CONTROL GET TEXT | GRAPHIC GET CANVAS | MENU GET STATE | SPACE$ | XPRINT PRINT |
| CONTROL HIDE | GRAPHIC GET CAPTION | MENU GET TEXT | SPLIT | XPRINT RENDER |
| CONTROL KILL | GRAPHIC GET CLIENT | MENU NEW BAR | SQR | XPRINT SCALE |
| CONTROL SET CHECK | GRAPHIC GET CLIP | MENU NEW POPUP | STATIC | XPRINT SET CLIP |
| CONTROL SET TEXT | GRAPHIC GET DC | MENU SET STATE | STR | XPRINT SET COLLATE |
| COS | GRAPHIC GET LINES | MENU SET TEXT | STR$ | XPRINT SET COLOR |
| COSH | GRAPHIC GET LOC | METHOD / END METHOD | STRDELETE | XPRINT SET COLORMODE |
| COT | GRAPHIC GET MIX | METRICS | STRDELETE$ | XPRINT SET COPIES |
| COTH | GRAPHIC GET PIXEL | MID | STRING | XPRINT SET DUPLEX |
| CQUAD | GRAPHIC GET POS | MID$ | STRINSERT | XPRINT SET FONT |
| CSC | GRAPHIC GET PPI | MIN | STRINSERT$ | XPRINT SET MIX |
| CSCH | GRAPHIC GET SCALE | MKBYT | STRPTR | XPRINT SET ORIENTATION |
| CSET | GRAPHIC GET SIZE | MKBYT$ | STRREVERSE | XPRINT SET OVERLAP |
| CSET$ | GRAPHIC GET STRETCHMODE | MKCUR$ | STRREVERSE$ | XPRINT SET PAGES |
| CSNG | GRAPHIC GET TEXTALIGN | MKCUX | SWAP | XPRINT SET PAPER |
| CSTR | GRAPHIC GET VIEW | MKCUX$ | SWITCH | XPRINT SET PIXEL |
| CULNG | GRAPHIC GET WORDWRAP | MKD$ | SWITCH$ | XPRINT SET POS |
| CURDIR | GRAPHIC GET WRAP | MKDIR | TAB$ | XPRINT SET QUALITY |
| CURDIR$ | GRAPHIC LINE | MKDWD | TALLY | XPRINT SET STRETCHMODE |
| CVBYT | GRAPHIC PAINT | MKDWD$ | TAN | XPRINT SET TEXTALIGN |
| CVCUX | GRAPHIC PIE | MKE | TANH | XPRINT SET TRAY |
| CVDWD | GRAPHIC POLYGON | MKE$ | TCP ACCEPT | XPRINT SET WORDWRAP |
| CVL | GRAPHIC POLYLINE | MKI$ | TCP CLOSE | XPRINT SET WRAP |
| CVQ | GRAPHIC PRINT | MKL$ | TCP LINE INPUT | XPRINT SPLIT |
| CVW | GRAPHIC SAVE | MKQ$ | TCP NOTIFY | XPRINT STRETCH |
| CWORD | GRAPHIC SCALE | MKS | TCP OPEN | XPRINT STYLE |
| DATA | GRAPHIC SCALE PIXELS | MKS$ | TCP PRINT | XPRINT TEXT SIZE |
| DATACOUNT | GRAPHIC SET AUTOSIZE | MKWRD | TCP RECV | XPRINT WIDTH |

## **Newly** implemented by this branch

> **486 keywords implemented by this fork** (every keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DISPLAY FONT | ISFALSE | STR |
| ACCEL ATTACH | DISPLAY OPENFILE | ISFILE | STR$ |
| ACODE | DISPLAY SAVEFILE | ISFOLDER | STRDELETE |
| ACODE$ | DOUBLE | ISINFINITE | STRDELETE$ |
| ACOS | ENVIRON$ | ISNORMAL | STRING |
| ACOSH | EOF | ISODD | STRINSERT |
| ARRAY ADD | EQV | ISTRUE | STRINSERT$ |
| ARRAY ARRAYIX | ERF | ISWIN | STRPTR |
| ARRAY ASSIGN | ERL | JOIN$ | STRREVERSE |
| ARRAY COPY | ERL$ | LCASE | STRREVERSE$ |
| ARRAY DELETE | ERR | LCASE$ | SWITCH |
| ARRAY INSERT | ERRCLEAR | LEFT | SWITCH$ |
| ARRAY REDIM DECR | ERROR$ | LEFT$ | TAB$ |
| ARRAY REDIM INCR | EVENT SOURCE | LEN | TALLY |
| ARRAY REVERSE | EVENTS | LET *(WITH OBJECTS)* | TAN |
| ARRAY SCAN | EXE | LET *(WITH VARIANTS)* | TANH |
| ARRAY SELECT | EXIST | LINE INPUT# | TCP NOTIFY |
| ARRAY SHUFFLE | EXP | LISTVIEW | THREADCOUNT |
| ARRAY SWAP | EXP10 | LO | THREADID |
| ARRAY TAGARRAY | EXP2 | LOC | TIMER |
| ARRAY TAGARRAY ERASE | EXPM1 | LOF | TREEVIEW |
| ARRAY UNIQUE | EXTRACT | LOG | TRIM |
| ASIN | EXTRACT$ | LOG10 | TRIM$ |
| ASINH | FILEATTR | LOG1P | TRUNC |
| ATANH | FILENAME | LOG2 | UCASE |
| ATN | FILENAME$ | LONG | UCASE$ |
| ATN2 | FIX | LOWRD | UCODE$ |
| BGR | FLOOR | LPRINT$ | UDP NOTIFY |
| BIN | FONT END | LSET$ | UNWRAP |
| BIN$ | FONT NEW | LTRIM | UNWRAP$ |
| BITS | FORMAT | LTRIM$ | USING |
| BITS$ | FORMAT$ | MAK | USING$ |
| BITSE | FRAC | MAX | UTF8TOCHR |
| BUILD | FRE | MCASE | UTF8TOCHR$ |
| BUILD$ | FREEFILE | MCASE$ | VARPTR |
| BYTE | FUNCNAME | MEMORY FILL | VERIFY |
| CALLSTK$ | FUNCNAME$ | MEMORY FILLS | WAITKEY |
| CALLSTKCOUNT | GET | MENU ADD POPUP | WAITKEY$ |
| CBOOL | GET STR | MENU ADD STRING | WORD |
| CBRT | GET WSTR | MENU DELETE | WRAP |
| CBYTE | GET$ | MENU GET STATE | WRAP$ |
| CDBL | GET$$ | MENU GET TEXT | WRITE |
| CDWORD | GETATTR | MENU NEW BAR | WRITE# |
| CEIL | GRAPHIC ARC | MENU NEW POPUP | XPRINT ARC |
| CFLT | GRAPHIC ATTACH | MENU SET STATE | XPRINT ATTACH |
| CHOOSE | GRAPHIC BITMAP END | MENU SET TEXT | XPRINT BOX |
| CHR | GRAPHIC BITMAP LOAD | METRICS | XPRINT CANCEL |
| CHR$ | GRAPHIC BITMAP NEW | MID | XPRINT CELL |
| CHRBYTES | GRAPHIC BOX | MIN | XPRINT CELL SIZE |
| CHRTOOEM | GRAPHIC CELL | MKBYT | XPRINT CHR SIZE |
| CHRTOOEM$ | GRAPHIC CELL SIZE | MKCUX | XPRINT CLOSE |
| CHRTOUTF8 | GRAPHIC CHR SIZE | MKDWD | XPRINT COLOR |
| CHRTOUTF8$ | GRAPHIC CIRCLE | MKE | XPRINT COPY |
| CLIP | GRAPHIC CLEAR | MKS | XPRINT ELLIPSE |
| CLIP$ | GRAPHIC COLOR | MKWRD | XPRINT FORMFEED |
| CLOSE | GRAPHIC COPY | MOD | XPRINT GET ATTACH |
| CODEPTR | GRAPHIC DETACH | MONTHNAME | XPRINT GET CANVAS |
| COMM LINE INPUT | GRAPHIC ELLIPSE | MONTHNAME$ | XPRINT GET CLIENT |
| COMMAND | GRAPHIC GET BITS | NUL$ | XPRINT GET CLIP |
| COMMAND$ | GRAPHIC GET CANVAS | OCT | XPRINT GET COLLATE |
| CONTROL ADD BUTTON | GRAPHIC GET CAPTION | OCT$ | XPRINT GET COLOR |
| CONTROL ADD CHECKBOX | GRAPHIC GET CLIENT | OEMTOCHR | XPRINT GET COLORMODE |
| CONTROL ADD COMBOBOX | GRAPHIC GET CLIP | OEMTOCHR$ | XPRINT GET COPIES |
| CONTROL ADD LABEL | GRAPHIC GET DC | ON CALL | XPRINT GET DC |
| CONTROL ADD LISTBOX | GRAPHIC GET LINES | ON GOSUB | XPRINT GET DUPLEX |
| CONTROL ADD LISTVIEW | GRAPHIC GET LOC | ON GOTO | XPRINT GET LINES |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET MIX | OPEN | XPRINT GET MARGIN |
| CONTROL ADD SCROLLBAR | GRAPHIC GET PIXEL | PARSE$ | XPRINT GET MIX |
| CONTROL ADD TREEVIEW | GRAPHIC GET POS | PARSECOUNT | XPRINT GET ORIENTATION |
| CONTROL ADDSTRING | GRAPHIC GET PPI | PATHNAME | XPRINT GET OVERLAP |
| CONTROL DISABLE | GRAPHIC GET SCALE | PATHNAME$ | XPRINT GET PAGES |
| CONTROL ENABLE | GRAPHIC GET SIZE | PATHSCAN | XPRINT GET PAPER |
| CONTROL GET CHECK | GRAPHIC GET STRETCHMODE | PATHSCAN$ | XPRINT GET PAPERS |
| CONTROL GET TEXT | GRAPHIC GET TEXTALIGN | PEEK | XPRINT GET PIXEL |
| CONTROL HIDE | GRAPHIC GET VIEW | PLAY | XPRINT GET POS |
| CONTROL KILL | GRAPHIC GET WORDWRAP | PLAY SOUND | XPRINT GET PPI |
| CONTROL SET CHECK | GRAPHIC GET WRAP | POKE | XPRINT GET QUALITY |
| CONTROL SET TEXT | GRAPHIC LINE | PRINTER$ | XPRINT GET SCALE |
| COS | GRAPHIC PAINT | PRINTERCOUNT | XPRINT GET SELECTION |
| COSH | GRAPHIC PIE | PROGRESSBAR | XPRINT GET SIZE |
| COT | GRAPHIC POLYGON | PUT | XPRINT GET STRETCHMODE |
| COTH | GRAPHIC POLYLINE | PUT STR | XPRINT GET TEXTALIGN |
| CQUAD | GRAPHIC PRINT | PUT WSTR | XPRINT GET TRAY |
| CSC | GRAPHIC SAVE | PUT$ | XPRINT GET TRAYS |
| CSCH | GRAPHIC SCALE | PUT$$ | XPRINT GET WORDWRAP |
| CSET$ | GRAPHIC SCALE PIXELS | QUAD | XPRINT GET WRAP |
| CSNG | GRAPHIC SET AUTOSIZE | RAISEEVENT | XPRINT IMAGELIST |
| CSTR | GRAPHIC SET BITS | READ | XPRINT LINE |
| CULNG | GRAPHIC SET CAPTION | READ$ | XPRINT PIE |
| CURDIR | GRAPHIC SET CLIP | REDIM | XPRINT POLYGON |
| CURDIR$ | GRAPHIC SET FIXED | REMAIN | XPRINT POLYLINE |
| CVBYT | GRAPHIC SET FONT | REMAIN$ | XPRINT PREVIEW |
| CVCUX | GRAPHIC SET MIX | REMOVE | XPRINT PRINT |
| CVDWD | GRAPHIC SET PIXEL | REMOVE$ | XPRINT RENDER |
| CVL | GRAPHIC SET POS | REPEAT | XPRINT SCALE |
| CVQ | GRAPHIC SET SIZE | REPEAT$ | XPRINT SET CLIP |
| CVW | GRAPHIC SET STRETCHMODE | RESOURCE SAVE FILE | XPRINT SET COLLATE |
| CWORD | GRAPHIC SET TEXTALIGN | RESOURCE$ | XPRINT SET COLOR |
| DATACOUNT | GRAPHIC SET VIEW | RETAIN | XPRINT SET COLORMODE |
| DAYNAME | GRAPHIC SET VIRTUAL | RETAIN$ | XPRINT SET COPIES |
| DAYNAME$ | GRAPHIC SET WORDWRAP | RGB | XPRINT SET DUPLEX |
| DEC | GRAPHIC SET WRAP | RIGHT | XPRINT SET FONT |
| DEC$ | GRAPHIC STYLE | RIGHT$ | XPRINT SET MIX |
| DECLARE | GRAPHIC TEXT SIZE | RND | XPRINT SET ORIENTATION |
| DEF | GRAPHIC WIDTH | ROUND | XPRINT SET OVERLAP |
| DESKTOP GET PPI | HEADER CTRL | RSET$ | XPRINT SET PAGES |
| DIALOG CENTER | HEX | RTRIM | XPRINT SET PAPER |
| DIALOG DOEVENTS | HEX$ | RTRIM$ | XPRINT SET PIXEL |
| DIALOG END | HI | SEC | XPRINT SET POS |
| DIALOG GET TEXT | HIWRD | SECH | XPRINT SET QUALITY |
| DIALOG NEW | HYPOT | SEEK | XPRINT SET STRETCHMODE |
| DIALOG SET TEXT | IIF | SETEOF | XPRINT SET TEXTALIGN |
| DIALOG SHOW MODAL | IMAGELIST COUNT | SGN | XPRINT SET TRAY |
| DIALOG SHOW STATE | IMAGELIST KILL | SHRINK | XPRINT SET WORDWRAP |
| DIM | IMAGELIST NEW | SHRINK$ | XPRINT SET WRAP |
| DIR | IMP | SIN | XPRINT SPLIT |
| DIR CLOSE | INPUT# | SINGLE | XPRINT STRETCH |
| DIR$ | INSTANCE | SINH | XPRINT STYLE |
| DISKFREE | INSTR | SIZEOF | XPRINT TEXT SIZE |
| DISKSIZE | INT | SPACE | XPRINT WIDTH |
| DISPLAY BROWSE | INTEGER | SPACE$ |  |
| DISPLAY COLOR | ISEVEN | SQR |  |
...
| COMM OPEN | IMPORT | MKL$ | ROTATE | UCODEPAGE |
| COMM PRINT | INCR | MKQ$ | RSET | UDP CLOSE |
| COMM RECV | INPUT FLUSH | MKS$ | SELECT CASE/END SELECT | UDP OPEN |
| COMM RESET | INTERFACE / END INTERFACE (DIRECT) | MKWRD$ | SETATTR | UDP RECV |
| COMM SEND | INTERFACE/END INTERFACE (IDBIND) | MOUSEPTR | SHELL | UDP SEND |
| COMM SET | ITERATE | MSGBOX | SHIFT | UNLOCK |
| COMM TIMEOUT | KILL | NAME | SLEEP | VAL |
| CSET | LET | NOT | SPLIT | WINDOW GET |
| DATA | LET *(WITH TYPES)* | NUL | STATIC | WINDOW SET |
| DECR | LOCAL | OBJECT | SWAP | XOR |
