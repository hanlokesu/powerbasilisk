...
> Summary:
> - **836** official PowerBASIC statement/function keywords currently available for use
>   - **688** implemented or completed by this fork
...
> - **19** DDT/GUI-class keywords deferred (Tier 3)
...
> Updated through batch 180 (v0.2.037). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 836 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | CURDIR$ | GRAPHIC SET POS | METHOD / END METHOD | TANH |
| ACCEL ATTACH | CVBYT | GRAPHIC SET SIZE | METRICS | TCP ACCEPT |
| ACODE | CVCUX | GRAPHIC SET STRETCHMODE | MID | TCP CLOSE |
| ACODE$ | CVDWD | GRAPHIC SET TEXTALIGN | MID$ | TCP LINE INPUT |
| ACOS | CVL | GRAPHIC SET VIEW | MIN | TCP NOTIFY |
| ACOSH | CVQ | GRAPHIC SET VIRTUAL | MKBYT | TCP OPEN |
| AND | CVW | GRAPHIC SET WORDWRAP | MKBYT$ | TCP PRINT |
| ARRAY ADD | CWORD | GRAPHIC SET WRAP | MKCUR$ | TCP RECV |
| ARRAY ARRAYIX | DATA | GRAPHIC STYLE | MKCUX | TCP SEND |
| ARRAY ASSIGN | DATACOUNT | GRAPHIC TEXT SIZE | MKCUX$ | THREAD CLOSE |
| ARRAY COPY | DAYNAME | GRAPHIC WIDTH | MKD$ | THREAD CREATE |
| ARRAY DELETE | DAYNAME$ | GRAPHIC WINDOW | MKDIR | THREAD GET PRIORITY |
| ARRAY INSERT | DEC | GRAPHIC WINDOW CLICK | MKDWD | THREAD RESUME |
| ARRAY REDIM DECR | DEC$ | GRAPHIC WINDOW END | MKDWD$ | THREAD SET PRIORITY |
| ARRAY REDIM INCR | DECLARE | GRAPHIC WINDOW HIDE | MKE | THREAD STATUS |
| ARRAY REVERSE | DECR | GRAPHIC WINDOW MINIMIZE | MKE$ | THREAD SUSPEND |
| ARRAY SCAN | DEF | GRAPHIC WINDOW NONSTABLE | MKI$ | THREADCOUNT |
| ARRAY SHUFFLE | DESKTOP GET CLIENT | GRAPHIC WINDOW NORMALIZE | MKL$ | THREADED |
| ARRAY SORT | DESKTOP GET LOC | GRAPHIC WINDOW STABILIZE | MKQ$ | THREADID |
| ARRAY SWAP | DESKTOP GET PPI | HEADER GET COUNT | MKS | TIMER |
| ARRAY TAGARRAY | DESKTOP GET SIZE | HEADER GET ITEM | MKS$ | TIX |
| ARRAY TAGARRAY ERASE | DIALOG DEFAULT FONT | HEADER SEND | MKWRD | TOOLBAR ADD BUTTON |
| ARRAY UNIQUE | DIALOG DISABLE | HEADER SET ITEM | MKWRD$ | TOOLBAR ADD SEPARATOR |
| ASC | DIALOG DOEVENTS | HEX | MOD | TOOLBAR DELETE BUTTON |
| ASIN | DIALOG ENABLE | HEX$ | MONTHNAME | TOOLBAR GET COUNT |
| ASINH | DIALOG END | HI | MONTHNAME$ | TOOLBAR GET STATE |
| ASM | DIALOG GET CLIENT | HIWRD | MOUSEPTR | TOOLBAR SET IMAGELIST |
| ASMDATA / END ASMDATA | DIALOG GET LOC | HOST ADDR | MSGBOX | TOOLBAR SET STATE |
| ATANH | DIALOG GET SIZE | HOST NAME | NAME | TRACE |
| ATN | DIALOG GET TEXT | HYPOT | NOT | TREEVIEW DELETE |
| ATN2 | DIALOG GET USER | IF | NUL | TREEVIEW GET BOLD |
| BEEP | DIALOG HIDE | IF/END IF | NUL$ | TREEVIEW GET CHECK |
| BGR | DIALOG MAXIMIZE | IIF | OBJECT | TREEVIEW GET CHILD |
| BIN | DIALOG MINIMIZE | IMAGELIST ADD BITMAP | OCT | TREEVIEW GET COUNT |
| BIN$ | DIALOG NEW | IMAGELIST ADD ICON | OCT$ | TREEVIEW GET EXPANDED |
| BIT | DIALOG NONSTABLE | IMAGELIST ADD MASKED | OEMTOCHR | TREEVIEW GET NEXT |
| BIT CALC | DIALOG NORMALIZE | IMAGELIST COUNT | OEMTOCHR$ | TREEVIEW GET PARENT |
| BITS | DIALOG PIXELS | IMAGELIST GET COUNT | ON CALL | TREEVIEW GET PREVIOUS |
| BITS$ | DIALOG POST | IMAGELIST KILL | ON ERROR | TREEVIEW GET ROOT |
| BITSE | DIALOG REDRAW | IMAGELIST NEW | ON GOSUB | TREEVIEW GET SELECT |
| BUILD | DIALOG SEND | IMAGELIST NEW BITMAP | ON GOTO | TREEVIEW GET TEXT |
| BUILD$ | DIALOG SET CLIENT | IMAGELIST NEW ICON | OPEN | TREEVIEW GET USER |
| BYTE | DIALOG SET COLOR | IMAGELIST SET OVERLAY | OPTION EXPLICIT | TREEVIEW INSERT ITEM |
| CALL | DIALOG SET ICON | IMP | OR | TREEVIEW RESET |
| CALL DWORD | DIALOG SET LOC | IMPORT | PARSE | TREEVIEW SELECT |
| CALLSTK | DIALOG SET SIZE | INCR | PARSE$ | TREEVIEW SET BOLD |
| CALLSTK$ | DIALOG SET TEXT | INPUT FLUSH | PARSECOUNT | TREEVIEW SET CHECK |
| CALLSTKCOUNT | DIALOG SET USER | INPUT# | PATHNAME | TREEVIEW SET EXPANDED |
| CBOOL | DIALOG SHOW MODAL | INSTANCE | PATHNAME$ | TREEVIEW SET IMAGELIST |
| CBRT | DIALOG SHOW MODELESS | INSTR | PATHSCAN | TREEVIEW SET TEXT |
| CBYTE | DIALOG SHOW STATE | INT | PATHSCAN$ | TREEVIEW SET USER |
| CDBL | DIALOG STABILIZE | INTEGER | PEEK | TREEVIEW UNSELECT |
| CDWORD | DIALOG UNITS | INTERFACE / END INTERFACE (DIRECT) | PLAY | TRIM |
| CEIL | DIM | INTERFACE/END INTERFACE (IDBIND) | PLAY SOUND | TRIM$ |
| CFLT | DIR | ISEVEN | PLAY WAVE | TRUNC |
| CHDIR | DIR CLOSE | ISFALSE | POKE | TRY/END TRY |
| CHDRIVE | DIR$ | ISFILE | PREFIX | TYPE SET |
| CHOOSE | DISKFREE | ISFOLDER | PRINT# | TYPE/END TYPE |
| CHR | DISKSIZE | ISINFINITE | PRINTER$ | UCASE |
| CHR$ | DISPLAY BROWSE | ISNORMAL | PRINTERCOUNT | UCASE$ |
| CHRBYTES | DISPLAY COLOR | ISODD | PROCESS GET PRIORITY | UCODE$ |
| CHRTOOEM | DISPLAY FONT | ISTRUE | PROCESS SET PRIORITY | UCODEPAGE |
| CHRTOOEM$ | DISPLAY OPENFILE | ISWIN | PROFILE | UDP CLOSE |
| CHRTOUTF8 | DISPLAY SAVEFILE | ITERATE | PROGRESSBAR GET POS | UDP NOTIFY |
| CHRTOUTF8$ | DOUBLE | JOIN$ | PROGRESSBAR GET RANGE | UDP OPEN |
| CLASS/END CLASS | END | KILL | PROGRESSBAR SET POS | UDP RECV |
| CLIP | ENVIRON | LCASE | PROGRESSBAR SET RANGE | UDP SEND |
| CLIP$ | ENVIRON$ | LCASE$ | PROGRESSBAR SET STEP | UNLOCK |
| CLIPBOARD | EOF | LEFT | PROGRESSBAR STEP | UNWRAP |
| CLOSE | EQV | LEFT$ | PUT | UNWRAP$ |
| CLS | ERASE | LEN | PUT STR | USING |
| CODEPTR | ERF | LET | PUT WSTR | USING$ |
| COLOR | ERL | LET *(WITH OBJECTS)* | PUT$ | UTF8TOCHR |
| COMBOBOX ADD | ERL$ | LET *(WITH TYPES)* | PUT$$ | UTF8TOCHR$ |
| COMBOBOX DELETE | ERR | LET *(WITH VARIANTS)* | QUAD | VAL |
| COMBOBOX FIND | ERRCLEAR | LINE INPUT# | RAISEEVENT | VARPTR |
| COMBOBOX FIND EXACT | ERROR | LISTBOX ADD | RANDOMIZE | VERIFY |
| COMBOBOX GET COUNT | ERROR$ | LISTBOX DELETE | READ | WAITKEY |
| COMBOBOX GET SELCOUNT | EVENT SOURCE | LISTBOX FIND | READ$ | WAITKEY$ |
| COMBOBOX GET SELECT | EVENTS | LISTBOX FIND EXACT | REDIM | WINDOW GET |
| COMBOBOX GET STATE | EXE | LISTBOX GET COUNT | REGEXPR | WINDOW SET |
| COMBOBOX GET TEXT | EXIST | LISTBOX GET SELCOUNT | REGISTER | WORD |
| COMBOBOX GET USER | EXIT | LISTBOX GET SELECT | REGREPL | WRAP |
| COMBOBOX INSERT | EXP | LISTBOX GET STATE | REM | WRAP$ |
| COMBOBOX RESET | EXP10 | LISTBOX GET TEXT | REMAIN | WRITE |
| COMBOBOX SELECT | EXP2 | LISTBOX GET USER | REMAIN$ | WRITE# |
| COMBOBOX SET TEXT | EXPM1 | LISTBOX INSERT | REMOVE | XOR |
| COMBOBOX SET USER | EXTRACT | LISTBOX RESET | REMOVE$ | XPRINT ARC |
| COMBOBOX UNSELECT | EXTRACT$ | LISTBOX SELECT | REPEAT | XPRINT ATTACH |
| COMM CLOSE | FIELD | LISTBOX SET TEXT | REPEAT$ | XPRINT BOX |
| COMM LINE | FILEATTR | LISTBOX SET USER | REPLACE | XPRINT CANCEL |
| COMM LINE INPUT | FILECOPY | LISTBOX UNSELECT | RESET | XPRINT CELL |
| COMM OPEN | FILENAME | LISTVIEW DELETE COLUMN | RESOURCE SAVE FILE | XPRINT CELL SIZE |
| COMM PRINT | FILENAME$ | LISTVIEW DELETE ITEM | RESOURCE$ | XPRINT CHR SIZE |
| COMM RECV | FILESCAN | LISTVIEW FIND | RESUME | XPRINT CLOSE |
| COMM RESET | FIX | LISTVIEW FIND EXACT | RETAIN | XPRINT COLOR |
| COMM SEND | FLOOR | LISTVIEW FIT CONTENT | RETAIN$ | XPRINT COPY |
| COMM SET | FLUSH | LISTVIEW FIT HEADER | RETURN | XPRINT ELLIPSE |
| COMM TIMEOUT | FONT END | LISTVIEW GET COLUMN | RGB | XPRINT FORMFEED |
| COMMAND | FONT NEW | LISTVIEW GET COUNT | RIGHT | XPRINT GET ATTACH |
| COMMAND$ | FOR / NEXT | LISTVIEW GET HEADER | RIGHT$ | XPRINT GET CANVAS |
| CONTROL ADD | FORMAT | LISTVIEW GET HEADERID | RMDIR | XPRINT GET CLIENT |
| CONTROL ADD BUTTON | FORMAT$ | LISTVIEW GET MODE | RND | XPRINT GET CLIP |
| CONTROL ADD CHECK3STATE | FRAC | LISTVIEW GET SELCOUNT | ROTATE | XPRINT GET COLLATE |
| CONTROL ADD CHECKBOX | FRE | LISTVIEW GET SELECT | ROUND | XPRINT GET COLOR |
| CONTROL ADD COMBOBOX | FREEFILE | LISTVIEW GET STATE | RSET | XPRINT GET COLORMODE |
| CONTROL ADD FRAME | FUNCNAME | LISTVIEW GET STYLEXX | RSET$ | XPRINT GET COPIES |
| CONTROL ADD GRAPHIC | FUNCNAME$ | LISTVIEW GET TEXT | RTRIM | XPRINT GET DC |
| CONTROL ADD HEADER | FUNCTION / END FUNCTION | LISTVIEW GET USER | RTRIM$ | XPRINT GET DUPLEX |
| CONTROL ADD IMAGE | GET | LISTVIEW INSERT COLUMN | SCROLLBAR GET PAGESIZE | XPRINT GET LINES |
| CONTROL ADD IMAGEX | GET STR | LISTVIEW INSERT ITEM | SCROLLBAR GET POS | XPRINT GET MARGIN |
| CONTROL ADD IMGBUTTON | GET WSTR | LISTVIEW RESET | SCROLLBAR GET RANGE | XPRINT GET MIX |
| CONTROL ADD IMGBUTTONX | GET$ | LISTVIEW SELECT | SCROLLBAR GET TRACKPOS | XPRINT GET ORIENTATION |
| CONTROL ADD LABEL | GET$$ | LISTVIEW SET COLUMN | SCROLLBAR SET PAGESIZE | XPRINT GET OVERLAP |
| CONTROL ADD LINE | GETATTR | LISTVIEW SET HEADER | SCROLLBAR SET POS | XPRINT GET PAGES |
| CONTROL ADD LISTBOX | GLOBAL | LISTVIEW SET IMAGE | SCROLLBAR SET RANGE | XPRINT GET PAPER |
| CONTROL ADD LISTVIEW | GLOBALMEM | LISTVIEW SET IMAGE2 | SEC | XPRINT GET PAPERS |
| CONTROL ADD OPTION | GRAPHIC ARC | LISTVIEW SET IMAGELIST | SECH | XPRINT GET PIXEL |
| CONTROL ADD PROGRESSBAR | GRAPHIC ATTACH | LISTVIEW SET MODE | SEEK | XPRINT GET POS |
| CONTROL ADD SCROLLBAR | GRAPHIC BITMAP END | LISTVIEW SET OVERLAY | SELECT CASE/END SELECT | XPRINT GET PPI |
| CONTROL ADD STATUSBAR | GRAPHIC BITMAP LOAD | LISTVIEW SET STYLEXX | SETATTR | XPRINT GET QUALITY |
| CONTROL ADD TAB | GRAPHIC BITMAP NEW | LISTVIEW SET TEXT | SETEOF | XPRINT GET SCALE |
| CONTROL ADD TEXTBOX | GRAPHIC BOX | LISTVIEW SET USER | SGN | XPRINT GET SELECTION |
| CONTROL ADD TOOLBAR | GRAPHIC CELL | LISTVIEW SORT | SHELL | XPRINT GET SIZE |
| CONTROL ADD TREEVIEW | GRAPHIC CELL SIZE | LISTVIEW UNSELECT | SHIFT | XPRINT GET STRETCHMODE |
| CONTROL DISABLE | GRAPHIC CHR SIZE | LISTVIEW VISIBLE | SHRINK | XPRINT GET TEXTALIGN |
| CONTROL ENABLE | GRAPHIC CIRCLE | LO | SHRINK$ | XPRINT GET TRAY |
| CONTROL GET CHECK | GRAPHIC CLEAR | LOC | SIN | XPRINT GET TRAYS |
| CONTROL GET CLIENT | GRAPHIC COLOR | LOCAL | SINGLE | XPRINT GET WORDWRAP |
| CONTROL GET LOC | GRAPHIC COPY | LOCK | SINH | XPRINT GET WRAP |
| CONTROL GET SIZE | GRAPHIC DETACH | LOF | SIZEOF | XPRINT IMAGELIST |
| CONTROL GET TEXT | GRAPHIC ELLIPSE | LOG | SLEEP | XPRINT LINE |
| CONTROL GET USER | GRAPHIC GET BITS | LOG10 | SPACE | XPRINT PIE |
| CONTROL HANDLE | GRAPHIC GET CANVAS | LOG1P | SPACE$ | XPRINT POLYGON |
| CONTROL HIDE | GRAPHIC GET CAPTION | LOG2 | SPLIT | XPRINT POLYLINE |
| CONTROL KILL | GRAPHIC GET CLIENT | LONG | SQR | XPRINT PREVIEW |
| CONTROL NORMALIZE | GRAPHIC GET CLIP | LOWRD | STATIC | XPRINT PRINT |
| CONTROL POST | GRAPHIC GET DC | LPRINT | STATUSBAR SET PARTS | XPRINT RENDER |
| CONTROL REDRAW | GRAPHIC GET LINES | LPRINT ATTACH | STATUSBAR SET TEXT | XPRINT SCALE |
| CONTROL SEND | GRAPHIC GET LOC | LPRINT CLOSE | STR | XPRINT SET CLIP |
| CONTROL SET CHECK | GRAPHIC GET MIX | LPRINT FLUSH | STR$ | XPRINT SET COLLATE |
| CONTROL SET CLIENT | GRAPHIC GET PIXEL | LPRINT FORMFEED | STRDELETE | XPRINT SET COLOR |
| CONTROL SET COLOR | GRAPHIC GET POS | LPRINT$ | STRDELETE$ | XPRINT SET COLORMODE |
| CONTROL SET FOCUS | GRAPHIC GET PPI | LSET | STRING | XPRINT SET COPIES |
| CONTROL SET FONT | GRAPHIC GET SCALE | LSET$ | STRINSERT | XPRINT SET DUPLEX |
| CONTROL SET IMAGE | GRAPHIC GET SIZE | LTRIM | STRINSERT$ | XPRINT SET FONT |
| CONTROL SET IMAGEX | GRAPHIC GET STRETCHMODE | LTRIM$ | STRPTR | XPRINT SET MIX |
| CONTROL SET IMGBUTTON | GRAPHIC GET TEXTALIGN | MACRO/END MACRO | STRREVERSE | XPRINT SET ORIENTATION |
| CONTROL SET IMGBUTTONX | GRAPHIC GET VIEW | MAK | STRREVERSE$ | XPRINT SET OVERLAP |
| CONTROL SET LOC | GRAPHIC GET WORDWRAP | MAT | SWAP | XPRINT SET PAGES |
| CONTROL SET OPTION | GRAPHIC GET WRAP | MAX | SWITCH | XPRINT SET PAPER |
| CONTROL SET SIZE | GRAPHIC LINE | MCASE | SWITCH$ | XPRINT SET PIXEL |
| CONTROL SET TEXT | GRAPHIC PAINT | MCASE$ | TAB DELETE | XPRINT SET POS |
| CONTROL SET USER | GRAPHIC PIE | MEMORY | TAB GET COUNT | XPRINT SET QUALITY |
| CONTROL SHOW STATE | GRAPHIC POLYGON | MEMORY FILL | TAB GET DIALOG | XPRINT SET STRETCHMODE |
| COS | GRAPHIC POLYLINE | MEMORY FILLS | TAB GET IMAGE | XPRINT SET TEXTALIGN |
| COSH | GRAPHIC PRINT | MENU ADD POPUP | TAB GET PAGE | XPRINT SET TRAY |
| COT | GRAPHIC SAVE | MENU ADD STRING | TAB GET SELECT | XPRINT SET WORDWRAP |
| COTH | GRAPHIC SCALE | MENU ATTACH | TAB GET TEXT | XPRINT SET WRAP |
| CQUAD | GRAPHIC SCALE PIXELS | MENU CONTEXT | TAB INSERT PAGE | XPRINT SPLIT |
| CSC | GRAPHIC SET AUTOSIZE | MENU DELETE | TAB RESET | XPRINT STRETCH |
| CSCH | GRAPHIC SET BITS | MENU DRAW BAR | TAB SELECT | XPRINT STYLE |
| CSET | GRAPHIC SET CAPTION | MENU GET STATE | TAB SET IMAGE | XPRINT TEXT SIZE |
| CSET$ | GRAPHIC SET CLIP | MENU GET TEXT | TAB SET IMAGELIST | XPRINT WIDTH |
| CSNG | GRAPHIC SET FIXED | MENU NEW BAR | TAB SET TEXT |  |
| CSTR | GRAPHIC SET FONT | MENU NEW POPUP | TAB$ |  |
| CULNG | GRAPHIC SET MIX | MENU SET STATE | TALLY |  |
| CURDIR | GRAPHIC SET PIXEL | MENU SET TEXT | TAN |  |

## **Newly** implemented by this branch

> **688 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DIALOG NORMALIZE | ISWIN | SIN |
| ACCEL ATTACH | DIALOG PIXELS | JOIN$ | SINGLE |
| ACODE | DIALOG POST | LCASE | SINH |
| ACODE$ | DIALOG REDRAW | LCASE$ | SIZEOF |
| ACOS | DIALOG SEND | LEFT | SPACE |
| ACOSH | DIALOG SET CLIENT | LEFT$ | SPACE$ |
| ARRAY ADD | DIALOG SET COLOR | LEN | SQR |
| ARRAY ARRAYIX | DIALOG SET ICON | LET *(WITH OBJECTS)* | STATUSBAR SET PARTS |
| ARRAY ASSIGN | DIALOG SET LOC | LET *(WITH VARIANTS)* | STATUSBAR SET TEXT |
| ARRAY COPY | DIALOG SET SIZE | LINE INPUT# | STR |
| ARRAY DELETE | DIALOG SET TEXT | LISTBOX ADD | STR$ |
| ARRAY INSERT | DIALOG SET USER | LISTBOX DELETE | STRDELETE |
| ARRAY REDIM DECR | DIALOG SHOW MODAL | LISTBOX FIND | STRDELETE$ |
| ARRAY REDIM INCR | DIALOG SHOW MODELESS | LISTBOX FIND EXACT | STRING |
| ARRAY REVERSE | DIALOG SHOW STATE | LISTBOX GET COUNT | STRINSERT |
| ARRAY SCAN | DIALOG STABILIZE | LISTBOX GET SELCOUNT | STRINSERT$ |
| ARRAY SHUFFLE | DIALOG UNITS | LISTBOX GET SELECT | STRPTR |
| ARRAY SWAP | DIM | LISTBOX GET STATE | STRREVERSE |
| ARRAY TAGARRAY | DIR | LISTBOX GET TEXT | STRREVERSE$ |
| ARRAY TAGARRAY ERASE | DIR CLOSE | LISTBOX GET USER | SWITCH |
| ARRAY UNIQUE | DIR$ | LISTBOX INSERT | SWITCH$ |
| ASIN | DISKFREE | LISTBOX RESET | TAB DELETE |
| ASINH | DISKSIZE | LISTBOX SELECT | TAB GET COUNT |
| ATANH | DISPLAY BROWSE | LISTBOX SET TEXT | TAB GET DIALOG |
| ATN | DISPLAY COLOR | LISTBOX SET USER | TAB GET IMAGE |
| ATN2 | DISPLAY FONT | LISTBOX UNSELECT | TAB GET PAGE |
| BGR | DISPLAY OPENFILE | LISTVIEW DELETE COLUMN | TAB GET SELECT |
| BIN | DISPLAY SAVEFILE | LISTVIEW DELETE ITEM | TAB GET TEXT |
| BIN$ | DOUBLE | LISTVIEW FIND | TAB INSERT PAGE |
| BITS | ENVIRON$ | LISTVIEW FIND EXACT | TAB RESET |
| BITS$ | EOF | LISTVIEW FIT CONTENT | TAB SELECT |
| BITSE | EQV | LISTVIEW FIT HEADER | TAB SET IMAGE |
| BUILD | ERF | LISTVIEW GET COLUMN | TAB SET IMAGELIST |
| BUILD$ | ERL | LISTVIEW GET COUNT | TAB SET TEXT |
| BYTE | ERL$ | LISTVIEW GET HEADER | TAB$ |
| CALLSTK$ | ERR | LISTVIEW GET HEADERID | TALLY |
| CALLSTKCOUNT | ERRCLEAR | LISTVIEW GET MODE | TAN |
| CBOOL | ERROR$ | LISTVIEW GET SELCOUNT | TANH |
| CBRT | EVENT SOURCE | LISTVIEW GET SELECT | TCP NOTIFY |
| CBYTE | EVENTS | LISTVIEW GET STATE | THREADCOUNT |
| CDBL | EXE | LISTVIEW GET STYLEXX | THREADID |
| CDWORD | EXIST | LISTVIEW GET TEXT | TIMER |
| CEIL | EXP | LISTVIEW GET USER | TOOLBAR ADD BUTTON |
| CFLT | EXP10 | LISTVIEW INSERT COLUMN | TOOLBAR ADD SEPARATOR |
| CHOOSE | EXP2 | LISTVIEW INSERT ITEM | TOOLBAR DELETE BUTTON |
| CHR | EXPM1 | LISTVIEW RESET | TOOLBAR GET COUNT |
| CHR$ | EXTRACT | LISTVIEW SELECT | TOOLBAR GET STATE |
| CHRBYTES | EXTRACT$ | LISTVIEW SET COLUMN | TOOLBAR SET IMAGELIST |
| CHRTOOEM | FILEATTR | LISTVIEW SET HEADER | TOOLBAR SET STATE |
| CHRTOOEM$ | FILENAME | LISTVIEW SET IMAGE | TREEVIEW DELETE |
| CHRTOUTF8 | FILENAME$ | LISTVIEW SET IMAGE2 | TREEVIEW GET BOLD |
| CHRTOUTF8$ | FIX | LISTVIEW SET IMAGELIST | TREEVIEW GET CHECK |
| CLIP | FLOOR | LISTVIEW SET MODE | TREEVIEW GET CHILD |
| CLIP$ | FONT END | LISTVIEW SET OVERLAY | TREEVIEW GET COUNT |
| CLOSE | FONT NEW | LISTVIEW SET STYLEXX | TREEVIEW GET EXPANDED |
| CODEPTR | FORMAT | LISTVIEW SET TEXT | TREEVIEW GET NEXT |
| COMBOBOX ADD | FORMAT$ | LISTVIEW SET USER | TREEVIEW GET PARENT |
| COMBOBOX DELETE | FRAC | LISTVIEW SORT | TREEVIEW GET PREVIOUS |
| COMBOBOX FIND | FRE | LISTVIEW UNSELECT | TREEVIEW GET ROOT |
| COMBOBOX FIND EXACT | FREEFILE | LISTVIEW VISIBLE | TREEVIEW GET SELECT |
| COMBOBOX GET COUNT | FUNCNAME | LO | TREEVIEW GET TEXT |
| COMBOBOX GET SELCOUNT | FUNCNAME$ | LOC | TREEVIEW GET USER |
| COMBOBOX GET SELECT | GET | LOF | TREEVIEW INSERT ITEM |
| COMBOBOX GET STATE | GET STR | LOG | TREEVIEW RESET |
| COMBOBOX GET TEXT | GET WSTR | LOG10 | TREEVIEW SELECT |
| COMBOBOX GET USER | GET$ | LOG1P | TREEVIEW SET BOLD |
| COMBOBOX INSERT | GET$$ | LOG2 | TREEVIEW SET CHECK |
| COMBOBOX RESET | GETATTR | LONG | TREEVIEW SET EXPANDED |
| COMBOBOX SELECT | GRAPHIC ARC | LOWRD | TREEVIEW SET IMAGELIST |
| COMBOBOX SET TEXT | GRAPHIC ATTACH | LPRINT$ | TREEVIEW SET TEXT |
| COMBOBOX SET USER | GRAPHIC BITMAP END | LSET$ | TREEVIEW SET USER |
| COMBOBOX UNSELECT | GRAPHIC BITMAP LOAD | LTRIM | TREEVIEW UNSELECT |
| COMM LINE INPUT | GRAPHIC BITMAP NEW | LTRIM$ | TRIM |
| COMMAND | GRAPHIC BOX | MAK | TRIM$ |
| COMMAND$ | GRAPHIC CELL | MAX | TRUNC |
| CONTROL ADD | GRAPHIC CELL SIZE | MCASE | UCASE |
| CONTROL ADD BUTTON | GRAPHIC CHR SIZE | MCASE$ | UCASE$ |
| CONTROL ADD CHECK3STATE | GRAPHIC CIRCLE | MEMORY FILL | UCODE$ |
| CONTROL ADD CHECKBOX | GRAPHIC CLEAR | MEMORY FILLS | UCODEPAGE |
| CONTROL ADD COMBOBOX | GRAPHIC COLOR | MENU ADD POPUP | UDP NOTIFY |
| CONTROL ADD FRAME | GRAPHIC COPY | MENU ADD STRING | UNWRAP |
| CONTROL ADD GRAPHIC | GRAPHIC DETACH | MENU ATTACH | UNWRAP$ |
| CONTROL ADD HEADER | GRAPHIC ELLIPSE | MENU CONTEXT | USING |
| CONTROL ADD IMAGE | GRAPHIC GET BITS | MENU DELETE | USING$ |
| CONTROL ADD IMAGEX | GRAPHIC GET CANVAS | MENU DRAW BAR | UTF8TOCHR |
| CONTROL ADD IMGBUTTON | GRAPHIC GET CAPTION | MENU GET STATE | UTF8TOCHR$ |
| CONTROL ADD IMGBUTTONX | GRAPHIC GET CLIENT | MENU GET TEXT | VARPTR |
| CONTROL ADD LABEL | GRAPHIC GET CLIP | MENU NEW BAR | VERIFY |
| CONTROL ADD LINE | GRAPHIC GET DC | MENU NEW POPUP | WAITKEY |
| CONTROL ADD LISTBOX | GRAPHIC GET LINES | MENU SET STATE | WAITKEY$ |
| CONTROL ADD LISTVIEW | GRAPHIC GET LOC | MENU SET TEXT | WORD |
| CONTROL ADD OPTION | GRAPHIC GET MIX | METRICS | WRAP |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET PIXEL | MID | WRAP$ |
| CONTROL ADD SCROLLBAR | GRAPHIC GET POS | MIN | WRITE |
| CONTROL ADD STATUSBAR | GRAPHIC GET PPI | MKBYT | WRITE# |
| CONTROL ADD TAB | GRAPHIC GET SCALE | MKCUX | XPRINT ARC |
| CONTROL ADD TEXTBOX | GRAPHIC GET SIZE | MKDWD | XPRINT ATTACH |
| CONTROL ADD TOOLBAR | GRAPHIC GET STRETCHMODE | MKE | XPRINT BOX |
| CONTROL ADD TREEVIEW | GRAPHIC GET TEXTALIGN | MKS | XPRINT CANCEL |
| CONTROL DISABLE | GRAPHIC GET VIEW | MKWRD | XPRINT CELL |
| CONTROL ENABLE | GRAPHIC GET WORDWRAP | MOD | XPRINT CELL SIZE |
| CONTROL GET CHECK | GRAPHIC GET WRAP | MONTHNAME | XPRINT CHR SIZE |
| CONTROL GET CLIENT | GRAPHIC LINE | MONTHNAME$ | XPRINT CLOSE |
| CONTROL GET LOC | GRAPHIC PAINT | NUL$ | XPRINT COLOR |
| CONTROL GET SIZE | GRAPHIC PIE | OCT | XPRINT COPY |
| CONTROL GET TEXT | GRAPHIC POLYGON | OCT$ | XPRINT ELLIPSE |
| CONTROL GET USER | GRAPHIC POLYLINE | OEMTOCHR | XPRINT FORMFEED |
| CONTROL HANDLE | GRAPHIC PRINT | OEMTOCHR$ | XPRINT GET ATTACH |
| CONTROL HIDE | GRAPHIC SAVE | ON CALL | XPRINT GET CANVAS |
| CONTROL KILL | GRAPHIC SCALE | ON GOSUB | XPRINT GET CLIENT |
| CONTROL NORMALIZE | GRAPHIC SCALE PIXELS | ON GOTO | XPRINT GET CLIP |
| CONTROL POST | GRAPHIC SET AUTOSIZE | OPEN | XPRINT GET COLLATE |
| CONTROL REDRAW | GRAPHIC SET BITS | PARSE$ | XPRINT GET COLOR |
| CONTROL SEND | GRAPHIC SET CAPTION | PARSECOUNT | XPRINT GET COLORMODE |
| CONTROL SET CHECK | GRAPHIC SET CLIP | PATHNAME | XPRINT GET COPIES |
| CONTROL SET CLIENT | GRAPHIC SET FIXED | PATHNAME$ | XPRINT GET DC |
| CONTROL SET COLOR | GRAPHIC SET FONT | PATHSCAN | XPRINT GET DUPLEX |
| CONTROL SET FOCUS | GRAPHIC SET MIX | PATHSCAN$ | XPRINT GET LINES |
| CONTROL SET FONT | GRAPHIC SET PIXEL | PEEK | XPRINT GET MARGIN |
| CONTROL SET IMAGE | GRAPHIC SET POS | PLAY | XPRINT GET MIX |
| CONTROL SET IMAGEX | GRAPHIC SET SIZE | PLAY SOUND | XPRINT GET ORIENTATION |
| CONTROL SET IMGBUTTON | GRAPHIC SET STRETCHMODE | POKE | XPRINT GET OVERLAP |
| CONTROL SET IMGBUTTONX | GRAPHIC SET TEXTALIGN | PRINTER$ | XPRINT GET PAGES |
| CONTROL SET LOC | GRAPHIC SET VIEW | PRINTERCOUNT | XPRINT GET PAPER |
| CONTROL SET OPTION | GRAPHIC SET VIRTUAL | PROGRESSBAR GET POS | XPRINT GET PAPERS |
| CONTROL SET SIZE | GRAPHIC SET WORDWRAP | PROGRESSBAR GET RANGE | XPRINT GET PIXEL |
| CONTROL SET TEXT | GRAPHIC SET WRAP | PROGRESSBAR SET POS | XPRINT GET POS |
| CONTROL SET USER | GRAPHIC STYLE | PROGRESSBAR SET RANGE | XPRINT GET PPI |
| CONTROL SHOW STATE | GRAPHIC TEXT SIZE | PROGRESSBAR SET STEP | XPRINT GET QUALITY |
| COS | GRAPHIC WIDTH | PROGRESSBAR STEP | XPRINT GET SCALE |
| COSH | GRAPHIC WINDOW | PUT | XPRINT GET SELECTION |
| COT | GRAPHIC WINDOW CLICK | PUT STR | XPRINT GET SIZE |
| COTH | GRAPHIC WINDOW END | PUT WSTR | XPRINT GET STRETCHMODE |
| CQUAD | GRAPHIC WINDOW HIDE | PUT$ | XPRINT GET TEXTALIGN |
| CSC | GRAPHIC WINDOW MINIMIZE | PUT$$ | XPRINT GET TRAY |
| CSCH | GRAPHIC WINDOW NONSTABLE | QUAD | XPRINT GET TRAYS |
| CSET$ | GRAPHIC WINDOW NORMALIZE | RAISEEVENT | XPRINT GET WORDWRAP |
| CSNG | GRAPHIC WINDOW STABILIZE | READ | XPRINT GET WRAP |
| CSTR | HEADER GET COUNT | READ$ | XPRINT IMAGELIST |
| CULNG | HEADER GET ITEM | REDIM | XPRINT LINE |
| CURDIR | HEADER SEND | REMAIN | XPRINT PIE |
| CURDIR$ | HEADER SET ITEM | REMAIN$ | XPRINT POLYGON |
| CVBYT | HEX | REMOVE | XPRINT POLYLINE |
| CVCUX | HEX$ | REMOVE$ | XPRINT PREVIEW |
| CVDWD | HI | REPEAT | XPRINT PRINT |
| CVL | HIWRD | REPEAT$ | XPRINT RENDER |
| CVQ | HYPOT | RESOURCE SAVE FILE | XPRINT SCALE |
| CVW | IIF | RESOURCE$ | XPRINT SET CLIP |
| CWORD | IMAGELIST ADD BITMAP | RETAIN | XPRINT SET COLLATE |
| DATACOUNT | IMAGELIST ADD ICON | RETAIN$ | XPRINT SET COLOR |
| DAYNAME | IMAGELIST ADD MASKED | RGB | XPRINT SET COLORMODE |
| DAYNAME$ | IMAGELIST COUNT | RIGHT | XPRINT SET COPIES |
| DEC | IMAGELIST GET COUNT | RIGHT$ | XPRINT SET DUPLEX |
| DEC$ | IMAGELIST KILL | RND | XPRINT SET FONT |
| DECLARE | IMAGELIST NEW | ROUND | XPRINT SET MIX |
| DEF | IMAGELIST NEW BITMAP | RSET$ | XPRINT SET ORIENTATION |
| DESKTOP GET PPI | IMAGELIST NEW ICON | RTRIM | XPRINT SET OVERLAP |
| DIALOG DEFAULT FONT | IMAGELIST SET OVERLAY | RTRIM$ | XPRINT SET PAGES |
| DIALOG DISABLE | IMP | SCROLLBAR GET PAGESIZE | XPRINT SET PAPER |
| DIALOG DOEVENTS | INPUT# | SCROLLBAR GET POS | XPRINT SET PIXEL |
| DIALOG ENABLE | INSTANCE | SCROLLBAR GET RANGE | XPRINT SET POS |
| DIALOG END | INSTR | SCROLLBAR GET TRACKPOS | XPRINT SET QUALITY |
| DIALOG GET CLIENT | INT | SCROLLBAR SET PAGESIZE | XPRINT SET STRETCHMODE |
| DIALOG GET LOC | INTEGER | SCROLLBAR SET POS | XPRINT SET TEXTALIGN |
| DIALOG GET SIZE | ISEVEN | SCROLLBAR SET RANGE | XPRINT SET TRAY |
| DIALOG GET TEXT | ISFALSE | SEC | XPRINT SET WORDWRAP |
| DIALOG GET USER | ISFILE | SECH | XPRINT SET WRAP |
| DIALOG HIDE | ISFOLDER | SEEK | XPRINT SPLIT |
| DIALOG MAXIMIZE | ISINFINITE | SETEOF | XPRINT STRETCH |
| DIALOG MINIMIZE | ISNORMAL | SGN | XPRINT STYLE |
| DIALOG NEW | ISODD | SHRINK | XPRINT TEXT SIZE |
| DIALOG NONSTABLE | ISTRUE | SHRINK$ | XPRINT WIDTH |
