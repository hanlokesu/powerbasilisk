...
> Summary:
> - **858** official PowerBASIC statement/function keywords currently available for use
>   - **710** implemented or completed by this fork
...
>
> Counts are computed directly from [statement-coverage.csv](docs/statement-coverage.csv) (861 rows, deduplicated). **0** official keywords are not implemented yet; **3** entry is a fork extension, implemented here but not an official PB keyword (ARRAY SELECT, DIALOG CENTER, GRAPHIC BITMAP CAPTURE). Rows whose internal codegen name uses an underscore are shown here in their spaced form - `GRAPHIC_CIRCLE` appears as `GRAPHIC CIRCLE`; the raw names are in [statement-coverage.md](docs/statement-coverage.md).
> Updated through batch 189 (v0.2.043). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 858 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | CURDIR$ | GRAPHIC SET CAPTION | MENU ATTACH | TAB SET IMAGE |
| ACCEL ATTACH | CVBYT | GRAPHIC SET CLIENT | MENU CONTEXT | TAB SET IMAGELIST |
| ACODE | CVCUX | GRAPHIC SET CLIP | MENU DELETE | TAB SET TEXT |
| ACODE$ | CVDWD | GRAPHIC SET FIXED | MENU DRAW BAR | TAB$ |
| ACOS | CVL | GRAPHIC SET FOCUS | MENU GET STATE | TALLY |
| ACOSH | CVQ | GRAPHIC SET FONT | MENU GET TEXT | TAN |
| AND | CVW | GRAPHIC SET LOC | MENU NEW BAR | TANH |
| ARRAY ADD | CWORD | GRAPHIC SET MIX | MENU NEW POPUP | TCP ACCEPT |
| ARRAY ARRAYIX | DATA | GRAPHIC SET OVERLAP | MENU SET STATE | TCP CLOSE |
| ARRAY ASSIGN | DATACOUNT | GRAPHIC SET PIXEL | MENU SET TEXT | TCP LINE INPUT |
| ARRAY COPY | DAYNAME | GRAPHIC SET POS | METHOD / END METHOD | TCP NOTIFY |
| ARRAY DELETE | DAYNAME$ | GRAPHIC SET SCROLLTEXT | METRICS | TCP OPEN |
| ARRAY INSERT | DEC | GRAPHIC SET SIZE | MID | TCP PRINT |
| ARRAY REDIM DECR | DEC$ | GRAPHIC SET STRETCHMODE | MID$ | TCP RECV |
| ARRAY REDIM INCR | DECLARE | GRAPHIC SET TEXTALIGN | MIN | TCP SEND |
| ARRAY REVERSE | DECR | GRAPHIC SET VIEW | MKBYT | THREAD CLOSE |
| ARRAY SCAN | DEF | GRAPHIC SET VIRTUAL | MKBYT$ | THREAD CREATE |
| ARRAY SHUFFLE | DESKTOP GET CLIENT | GRAPHIC SET WORDWRAP | MKCUR$ | THREAD GET PRIORITY |
| ARRAY SORT | DESKTOP GET LOC | GRAPHIC SET WRAP | MKCUX | THREAD RESUME |
| ARRAY SWAP | DESKTOP GET PPI | GRAPHIC SPLIT | MKCUX$ | THREAD SET PRIORITY |
| ARRAY TAGARRAY | DESKTOP GET SIZE | GRAPHIC STRETCH | MKD$ | THREAD STATUS |
| ARRAY TAGARRAY ERASE | DIALOG DEFAULT FONT | GRAPHIC STYLE | MKDIR | THREAD SUSPEND |
| ARRAY UNIQUE | DIALOG DISABLE | GRAPHIC TEXT SIZE | MKDWD | THREADCOUNT |
| ASC | DIALOG DOEVENTS | GRAPHIC WAITKEY$ | MKDWD$ | THREADED |
| ASIN | DIALOG ENABLE | GRAPHIC WIDTH | MKE | THREADID |
| ASINH | DIALOG END | GRAPHIC WINDOW | MKE$ | TIMER |
| ASM | DIALOG GET CLIENT | GRAPHIC WINDOW CLICK | MKI$ | TIX |
| ASMDATA / END ASMDATA | DIALOG GET LOC | GRAPHIC WINDOW END | MKL$ | TOOLBAR ADD BUTTON |
| ATANH | DIALOG GET SIZE | GRAPHIC WINDOW HIDE | MKQ$ | TOOLBAR ADD SEPARATOR |
| ATN | DIALOG GET TEXT | GRAPHIC WINDOW MINIMIZE | MKS | TOOLBAR DELETE BUTTON |
| ATN2 | DIALOG GET USER | GRAPHIC WINDOW NONSTABLE | MKS$ | TOOLBAR GET COUNT |
| BEEP | DIALOG HIDE | GRAPHIC WINDOW NORMALIZE | MKWRD | TOOLBAR GET STATE |
| BGR | DIALOG MAXIMIZE | GRAPHIC WINDOW STABILIZE | MKWRD$ | TOOLBAR SET IMAGELIST |
| BIN | DIALOG MINIMIZE | HEADER GET COUNT | MOD | TOOLBAR SET STATE |
| BIN$ | DIALOG NEW | HEADER GET ITEM | MONTHNAME | TRACE |
| BIT | DIALOG NONSTABLE | HEADER SEND | MONTHNAME$ | TREEVIEW DELETE |
| BIT CALC | DIALOG NORMALIZE | HEADER SET ITEM | MOUSEPTR | TREEVIEW GET BOLD |
| BITS | DIALOG PIXELS | HEX | MSGBOX | TREEVIEW GET CHECK |
| BITS$ | DIALOG POST | HEX$ | NAME | TREEVIEW GET CHILD |
| BITSE | DIALOG REDRAW | HI | NOT | TREEVIEW GET COUNT |
| BUILD | DIALOG SEND | HIWRD | NUL | TREEVIEW GET EXPANDED |
| BUILD$ | DIALOG SET CLIENT | HOST ADDR | NUL$ | TREEVIEW GET NEXT |
| BYTE | DIALOG SET COLOR | HOST NAME | OBJECT | TREEVIEW GET PARENT |
| CALL | DIALOG SET ICON | HYPOT | OCT | TREEVIEW GET PREVIOUS |
| CALL DWORD | DIALOG SET LOC | IF | OCT$ | TREEVIEW GET ROOT |
| CALLSTK | DIALOG SET SIZE | IF/END IF | OEMTOCHR | TREEVIEW GET SELECT |
| CALLSTK$ | DIALOG SET TEXT | IIF | OEMTOCHR$ | TREEVIEW GET TEXT |
| CALLSTKCOUNT | DIALOG SET USER | IMAGELIST ADD BITMAP | ON CALL | TREEVIEW GET USER |
| CBOOL | DIALOG SHOW MODAL | IMAGELIST ADD ICON | ON ERROR | TREEVIEW INSERT ITEM |
| CBRT | DIALOG SHOW MODELESS | IMAGELIST ADD MASKED | ON GOSUB | TREEVIEW RESET |
| CBYTE | DIALOG SHOW STATE | IMAGELIST COUNT | ON GOTO | TREEVIEW SELECT |
| CDBL | DIALOG STABILIZE | IMAGELIST GET COUNT | OPEN | TREEVIEW SET BOLD |
| CDWORD | DIALOG UNITS | IMAGELIST KILL | OPTION EXPLICIT | TREEVIEW SET CHECK |
| CEIL | DIM | IMAGELIST NEW | OR | TREEVIEW SET EXPANDED |
| CFLT | DIR | IMAGELIST NEW BITMAP | PARSE | TREEVIEW SET IMAGELIST |
| CHDIR | DIR CLOSE | IMAGELIST NEW ICON | PARSE$ | TREEVIEW SET TEXT |
| CHDRIVE | DIR$ | IMAGELIST SET OVERLAY | PARSECOUNT | TREEVIEW SET USER |
| CHOOSE | DISKFREE | IMP | PATHNAME | TREEVIEW UNSELECT |
| CHR | DISKSIZE | IMPORT | PATHNAME$ | TRIM |
| CHR$ | DISPLAY BROWSE | INCR | PATHSCAN | TRIM$ |
| CHRBYTES | DISPLAY COLOR | INPUT FLUSH | PATHSCAN$ | TRUNC |
| CHRTOOEM | DISPLAY FONT | INPUT# | PEEK | TRY/END TRY |
| CHRTOOEM$ | DISPLAY OPENFILE | INSTANCE | PLAY | TYPE SET |
| CHRTOUTF8 | DISPLAY SAVEFILE | INSTR | PLAY SOUND | TYPE/END TYPE |
| CHRTOUTF8$ | DOUBLE | INT | PLAY WAVE | UCASE |
| CLASS/END CLASS | END | INTEGER | POKE | UCASE$ |
| CLIP | ENVIRON | INTERFACE / END INTERFACE (DIRECT) | PREFIX | UCODE$ |
| CLIP$ | ENVIRON$ | INTERFACE/END INTERFACE (IDBIND) | PRINT# | UCODEPAGE |
| CLIPBOARD | EOF | ISEVEN | PRINTER$ | UDP CLOSE |
| CLOSE | EQV | ISFALSE | PRINTERCOUNT | UDP NOTIFY |
| CLS | ERASE | ISFILE | PROCESS GET PRIORITY | UDP OPEN |
| CODEPTR | ERF | ISFOLDER | PROCESS SET PRIORITY | UDP RECV |
| COLOR | ERL | ISINFINITE | PROFILE | UDP SEND |
| COMBOBOX ADD | ERL$ | ISNORMAL | PROGRESSBAR GET POS | UNLOCK |
| COMBOBOX DELETE | ERR | ISODD | PROGRESSBAR GET RANGE | UNWRAP |
| COMBOBOX FIND | ERRCLEAR | ISTRUE | PROGRESSBAR SET POS | UNWRAP$ |
| COMBOBOX FIND EXACT | ERROR | ISWIN | PROGRESSBAR SET RANGE | USING |
| COMBOBOX GET COUNT | ERROR$ | ITERATE | PROGRESSBAR SET STEP | USING$ |
| COMBOBOX GET SELCOUNT | EVENT SOURCE | JOIN$ | PROGRESSBAR STEP | UTF8TOCHR |
| COMBOBOX GET SELECT | EVENTS | KILL | PUT | UTF8TOCHR$ |
| COMBOBOX GET STATE | EXE | LCASE | PUT STR | VAL |
| COMBOBOX GET TEXT | EXIST | LCASE$ | PUT WSTR | VARPTR |
| COMBOBOX GET USER | EXIT | LEFT | PUT$ | VERIFY |
| COMBOBOX INSERT | EXP | LEFT$ | PUT$$ | WAITKEY |
| COMBOBOX RESET | EXP10 | LEN | QUAD | WAITKEY$ |
| COMBOBOX SELECT | EXP2 | LET | RAISEEVENT | WINDOW GET |
| COMBOBOX SET TEXT | EXPM1 | LET *(WITH OBJECTS)* | RANDOMIZE | WINDOW SET |
| COMBOBOX SET USER | EXTRACT | LET *(WITH TYPES)* | READ | WORD |
| COMBOBOX UNSELECT | EXTRACT$ | LET *(WITH VARIANTS)* | READ$ | WRAP |
| COMM CLOSE | FIELD | LINE INPUT# | REDIM | WRAP$ |
| COMM LINE | FILEATTR | LISTBOX ADD | REGEXPR | WRITE |
| COMM LINE INPUT | FILECOPY | LISTBOX DELETE | REGISTER | WRITE# |
| COMM OPEN | FILENAME | LISTBOX FIND | REGREPL | XOR |
| COMM PRINT | FILENAME$ | LISTBOX FIND EXACT | REM | XPRINT ARC |
| COMM RECV | FILESCAN | LISTBOX GET COUNT | REMAIN | XPRINT ATTACH |
| COMM RESET | FIX | LISTBOX GET SELCOUNT | REMAIN$ | XPRINT BOX |
| COMM SEND | FLOOR | LISTBOX GET SELECT | REMOVE | XPRINT CANCEL |
| COMM SET | FLUSH | LISTBOX GET STATE | REMOVE$ | XPRINT CELL |
| COMM TIMEOUT | FONT END | LISTBOX GET TEXT | REPEAT | XPRINT CELL SIZE |
| COMMAND | FONT NEW | LISTBOX GET USER | REPEAT$ | XPRINT CHR SIZE |
| COMMAND$ | FOR / NEXT | LISTBOX INSERT | REPLACE | XPRINT CLOSE |
| CONTROL ADD | FORMAT | LISTBOX RESET | RESET | XPRINT COLOR |
| CONTROL ADD BUTTON | FORMAT$ | LISTBOX SELECT | RESOURCE SAVE FILE | XPRINT COPY |
| CONTROL ADD CHECK3STATE | FRAC | LISTBOX SET TEXT | RESOURCE$ | XPRINT ELLIPSE |
| CONTROL ADD CHECKBOX | FRE | LISTBOX SET USER | RESUME | XPRINT FORMFEED |
| CONTROL ADD COMBOBOX | FREEFILE | LISTBOX UNSELECT | RETAIN | XPRINT GET ATTACH |
| CONTROL ADD FRAME | FUNCNAME | LISTVIEW DELETE COLUMN | RETAIN$ | XPRINT GET CANVAS |
| CONTROL ADD GRAPHIC | FUNCNAME$ | LISTVIEW DELETE ITEM | RETURN | XPRINT GET CLIENT |
| CONTROL ADD HEADER | FUNCTION / END FUNCTION | LISTVIEW FIND | RGB | XPRINT GET CLIP |
| CONTROL ADD HOTKEY | GET | LISTVIEW FIND EXACT | RIGHT | XPRINT GET COLLATE |
| CONTROL ADD IMAGE | GET STR | LISTVIEW FIT CONTENT | RIGHT$ | XPRINT GET COLOR |
| CONTROL ADD IMAGEX | GET WSTR | LISTVIEW FIT HEADER | RMDIR | XPRINT GET COLORMODE |
| CONTROL ADD IMGBUTTON | GET$ | LISTVIEW GET COLUMN | RND | XPRINT GET COPIES |
| CONTROL ADD IMGBUTTONX | GET$$ | LISTVIEW GET COUNT | ROTATE | XPRINT GET DC |
| CONTROL ADD IPADDRESS | GETATTR | LISTVIEW GET HEADER | ROUND | XPRINT GET DUPLEX |
| CONTROL ADD LABEL | GLOBAL | LISTVIEW GET HEADERID | RSET | XPRINT GET LINES |
| CONTROL ADD LINE | GLOBALMEM | LISTVIEW GET MODE | RSET$ | XPRINT GET MARGIN |
| CONTROL ADD LISTBOX | GRAPHIC ARC | LISTVIEW GET SELCOUNT | RTRIM | XPRINT GET MIX |
| CONTROL ADD LISTVIEW | GRAPHIC ATTACH | LISTVIEW GET SELECT | RTRIM$ | XPRINT GET ORIENTATION |
| CONTROL ADD OPTION | GRAPHIC BITMAP END | LISTVIEW GET STATE | SCROLLBAR GET PAGESIZE | XPRINT GET OVERLAP |
| CONTROL ADD PROGRESSBAR | GRAPHIC BITMAP LOAD | LISTVIEW GET STYLEXX | SCROLLBAR GET POS | XPRINT GET PAGES |
| CONTROL ADD SCROLLBAR | GRAPHIC BITMAP NEW | LISTVIEW GET TEXT | SCROLLBAR GET RANGE | XPRINT GET PAPER |
| CONTROL ADD STATUSBAR | GRAPHIC BOX | LISTVIEW GET USER | SCROLLBAR GET TRACKPOS | XPRINT GET PAPERS |
| CONTROL ADD TAB | GRAPHIC CELL | LISTVIEW INSERT COLUMN | SCROLLBAR SET PAGESIZE | XPRINT GET PIXEL |
| CONTROL ADD TEXTBOX | GRAPHIC CELL SIZE | LISTVIEW INSERT ITEM | SCROLLBAR SET POS | XPRINT GET POS |
| CONTROL ADD TOOLBAR | GRAPHIC CHR SIZE | LISTVIEW RESET | SCROLLBAR SET RANGE | XPRINT GET PPI |
| CONTROL ADD TRACKBAR | GRAPHIC CIRCLE | LISTVIEW SELECT | SEC | XPRINT GET QUALITY |
| CONTROL ADD TREEVIEW | GRAPHIC CLEAR | LISTVIEW SET COLUMN | SECH | XPRINT GET SCALE |
| CONTROL ADD UPDOWN | GRAPHIC COLOR | LISTVIEW SET HEADER | SEEK | XPRINT GET SELECTION |
| CONTROL DISABLE | GRAPHIC COPY | LISTVIEW SET IMAGE | SELECT CASE/END SELECT | XPRINT GET SIZE |
| CONTROL ENABLE | GRAPHIC DETACH | LISTVIEW SET IMAGE2 | SETATTR | XPRINT GET STRETCHMODE |
| CONTROL GET CHECK | GRAPHIC ELLIPSE | LISTVIEW SET IMAGELIST | SETEOF | XPRINT GET TEXTALIGN |
| CONTROL GET CLIENT | GRAPHIC GET BITS | LISTVIEW SET MODE | SGN | XPRINT GET TRAY |
| CONTROL GET LOC | GRAPHIC GET CANVAS | LISTVIEW SET OVERLAY | SHELL | XPRINT GET TRAYS |
| CONTROL GET SIZE | GRAPHIC GET CAPTION | LISTVIEW SET STYLEXX | SHIFT | XPRINT GET WORDWRAP |
| CONTROL GET TEXT | GRAPHIC GET CLIENT | LISTVIEW SET TEXT | SHRINK | XPRINT GET WRAP |
| CONTROL GET USER | GRAPHIC GET CLIP | LISTVIEW SET USER | SHRINK$ | XPRINT IMAGELIST |
| CONTROL HANDLE | GRAPHIC GET DC | LISTVIEW SORT | SIN | XPRINT LINE |
| CONTROL HIDE | GRAPHIC GET LINES | LISTVIEW UNSELECT | SINGLE | XPRINT PIE |
| CONTROL KILL | GRAPHIC GET LOC | LISTVIEW VISIBLE | SINH | XPRINT POLYGON |
| CONTROL NORMALIZE | GRAPHIC GET MIX | LO | SIZEOF | XPRINT POLYLINE |
| CONTROL POST | GRAPHIC GET OVERLAP | LOC | SLEEP | XPRINT PREVIEW |
| CONTROL REDRAW | GRAPHIC GET PIXEL | LOCAL | SPACE | XPRINT PRINT |
| CONTROL SEND | GRAPHIC GET POS | LOCK | SPACE$ | XPRINT RENDER |
| CONTROL SET CHECK | GRAPHIC GET PPI | LOF | SPLIT | XPRINT SCALE |
| CONTROL SET CLIENT | GRAPHIC GET SCALE | LOG | SQR | XPRINT SET CLIP |
| CONTROL SET COLOR | GRAPHIC GET SCROLLTEXT | LOG10 | STATIC | XPRINT SET COLLATE |
| CONTROL SET FOCUS | GRAPHIC GET SIZE | LOG1P | STATUSBAR SET PARTS | XPRINT SET COLOR |
| CONTROL SET FONT | GRAPHIC GET STRETCHMODE | LOG2 | STATUSBAR SET TEXT | XPRINT SET COLORMODE |
| CONTROL SET IMAGE | GRAPHIC GET TEXTALIGN | LONG | STR | XPRINT SET COPIES |
| CONTROL SET IMAGEX | GRAPHIC GET VIEW | LOWRD | STR$ | XPRINT SET DUPLEX |
| CONTROL SET IMGBUTTON | GRAPHIC GET WORDWRAP | LPRINT | STRDELETE | XPRINT SET FONT |
| CONTROL SET IMGBUTTONX | GRAPHIC GET WRAP | LPRINT ATTACH | STRDELETE$ | XPRINT SET MIX |
| CONTROL SET LOC | GRAPHIC IMAGELIST | LPRINT CLOSE | STRING | XPRINT SET ORIENTATION |
| CONTROL SET OPTION | GRAPHIC INKEY$ | LPRINT FLUSH | STRINSERT | XPRINT SET OVERLAP |
| CONTROL SET SIZE | GRAPHIC INPUT | LPRINT FORMFEED | STRINSERT$ | XPRINT SET PAGES |
| CONTROL SET TEXT | GRAPHIC INPUT FLUSH | LPRINT$ | STRPTR | XPRINT SET PAPER |
| CONTROL SET USER | GRAPHIC INSTAT | LSET | STRREVERSE | XPRINT SET PIXEL |
| CONTROL SHOW STATE | GRAPHIC LINE | LSET$ | STRREVERSE$ | XPRINT SET POS |
| COS | GRAPHIC LINE INPUT | LTRIM | SWAP | XPRINT SET QUALITY |
| COSH | GRAPHIC PAINT | LTRIM$ | SWITCH | XPRINT SET STRETCHMODE |
| COT | GRAPHIC PIE | MACRO/END MACRO | SWITCH$ | XPRINT SET TEXTALIGN |
| COTH | GRAPHIC POLYGON | MAK | TAB DELETE | XPRINT SET TRAY |
| CQUAD | GRAPHIC POLYLINE | MAT | TAB GET COUNT | XPRINT SET WORDWRAP |
| CSC | GRAPHIC PRINT | MAX | TAB GET DIALOG | XPRINT SET WRAP |
| CSCH | GRAPHIC REDRAW | MCASE | TAB GET IMAGE | XPRINT SPLIT |
| CSET | GRAPHIC RENDER | MCASE$ | TAB GET PAGE | XPRINT STRETCH |
| CSET$ | GRAPHIC SAVE | MEMORY | TAB GET SELECT | XPRINT STYLE |
| CSNG | GRAPHIC SCALE | MEMORY FILL | TAB GET TEXT | XPRINT TEXT SIZE |
| CSTR | GRAPHIC SCALE PIXELS | MEMORY FILLS | TAB INSERT PAGE | XPRINT WIDTH |
| CULNG | GRAPHIC SET AUTOSIZE | MENU ADD POPUP | TAB RESET |  |
| CURDIR | GRAPHIC SET BITS | MENU ADD STRING | TAB SELECT |  |

## **Newly** implemented by this branch

> **710 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DIALOG POST | INT | SETEOF |
| ACCEL ATTACH | DIALOG REDRAW | INTEGER | SGN |
| ACODE | DIALOG SEND | ISEVEN | SHRINK |
| ACODE$ | DIALOG SET CLIENT | ISFALSE | SHRINK$ |
| ACOS | DIALOG SET COLOR | ISFILE | SIN |
| ACOSH | DIALOG SET ICON | ISFOLDER | SINGLE |
| ARRAY ADD | DIALOG SET LOC | ISINFINITE | SINH |
| ARRAY ARRAYIX | DIALOG SET SIZE | ISNORMAL | SIZEOF |
| ARRAY ASSIGN | DIALOG SET TEXT | ISODD | SPACE |
| ARRAY COPY | DIALOG SET USER | ISTRUE | SPACE$ |
| ARRAY DELETE | DIALOG SHOW MODAL | ISWIN | SQR |
| ARRAY INSERT | DIALOG SHOW MODELESS | JOIN$ | STATUSBAR SET PARTS |
| ARRAY REDIM DECR | DIALOG SHOW STATE | LCASE | STATUSBAR SET TEXT |
| ARRAY REDIM INCR | DIALOG STABILIZE | LCASE$ | STR |
| ARRAY REVERSE | DIALOG UNITS | LEFT | STR$ |
| ARRAY SCAN | DIM | LEFT$ | STRDELETE |
| ARRAY SHUFFLE | DIR | LEN | STRDELETE$ |
| ARRAY SWAP | DIR CLOSE | LET *(WITH OBJECTS)* | STRING |
| ARRAY TAGARRAY | DIR$ | LET *(WITH VARIANTS)* | STRINSERT |
| ARRAY TAGARRAY ERASE | DISKFREE | LINE INPUT# | STRINSERT$ |
| ARRAY UNIQUE | DISKSIZE | LISTBOX ADD | STRPTR |
| ASIN | DISPLAY BROWSE | LISTBOX DELETE | STRREVERSE |
| ASINH | DISPLAY COLOR | LISTBOX FIND | STRREVERSE$ |
| ATANH | DISPLAY FONT | LISTBOX FIND EXACT | SWITCH |
| ATN | DISPLAY OPENFILE | LISTBOX GET COUNT | SWITCH$ |
| ATN2 | DISPLAY SAVEFILE | LISTBOX GET SELCOUNT | TAB DELETE |
| BGR | DOUBLE | LISTBOX GET SELECT | TAB GET COUNT |
| BIN | ENVIRON$ | LISTBOX GET STATE | TAB GET DIALOG |
| BIN$ | EOF | LISTBOX GET TEXT | TAB GET IMAGE |
| BITS | EQV | LISTBOX GET USER | TAB GET PAGE |
| BITS$ | ERF | LISTBOX INSERT | TAB GET SELECT |
| BITSE | ERL | LISTBOX RESET | TAB GET TEXT |
| BUILD | ERL$ | LISTBOX SELECT | TAB INSERT PAGE |
| BUILD$ | ERR | LISTBOX SET TEXT | TAB RESET |
| BYTE | ERRCLEAR | LISTBOX SET USER | TAB SELECT |
| CALLSTK$ | ERROR$ | LISTBOX UNSELECT | TAB SET IMAGE |
| CALLSTKCOUNT | EVENT SOURCE | LISTVIEW DELETE COLUMN | TAB SET IMAGELIST |
| CBOOL | EVENTS | LISTVIEW DELETE ITEM | TAB SET TEXT |
| CBRT | EXE | LISTVIEW FIND | TAB$ |
| CBYTE | EXIST | LISTVIEW FIND EXACT | TALLY |
| CDBL | EXP | LISTVIEW FIT CONTENT | TAN |
| CDWORD | EXP10 | LISTVIEW FIT HEADER | TANH |
| CEIL | EXP2 | LISTVIEW GET COLUMN | TCP NOTIFY |
| CFLT | EXPM1 | LISTVIEW GET COUNT | THREADCOUNT |
| CHOOSE | EXTRACT | LISTVIEW GET HEADER | THREADID |
| CHR | EXTRACT$ | LISTVIEW GET HEADERID | TIMER |
| CHR$ | FILEATTR | LISTVIEW GET MODE | TOOLBAR ADD BUTTON |
| CHRBYTES | FILENAME | LISTVIEW GET SELCOUNT | TOOLBAR ADD SEPARATOR |
| CHRTOOEM | FILENAME$ | LISTVIEW GET SELECT | TOOLBAR DELETE BUTTON |
| CHRTOOEM$ | FIX | LISTVIEW GET STATE | TOOLBAR GET COUNT |
| CHRTOUTF8 | FLOOR | LISTVIEW GET STYLEXX | TOOLBAR GET STATE |
| CHRTOUTF8$ | FONT END | LISTVIEW GET TEXT | TOOLBAR SET IMAGELIST |
| CLIP | FONT NEW | LISTVIEW GET USER | TOOLBAR SET STATE |
| CLIP$ | FORMAT | LISTVIEW INSERT COLUMN | TREEVIEW DELETE |
| CLOSE | FORMAT$ | LISTVIEW INSERT ITEM | TREEVIEW GET BOLD |
| CODEPTR | FRAC | LISTVIEW RESET | TREEVIEW GET CHECK |
| COMBOBOX ADD | FRE | LISTVIEW SELECT | TREEVIEW GET CHILD |
| COMBOBOX DELETE | FREEFILE | LISTVIEW SET COLUMN | TREEVIEW GET COUNT |
| COMBOBOX FIND | FUNCNAME | LISTVIEW SET HEADER | TREEVIEW GET EXPANDED |
| COMBOBOX FIND EXACT | FUNCNAME$ | LISTVIEW SET IMAGE | TREEVIEW GET NEXT |
| COMBOBOX GET COUNT | GET | LISTVIEW SET IMAGE2 | TREEVIEW GET PARENT |
| COMBOBOX GET SELCOUNT | GET STR | LISTVIEW SET IMAGELIST | TREEVIEW GET PREVIOUS |
| COMBOBOX GET SELECT | GET WSTR | LISTVIEW SET MODE | TREEVIEW GET ROOT |
| COMBOBOX GET STATE | GET$ | LISTVIEW SET OVERLAY | TREEVIEW GET SELECT |
| COMBOBOX GET TEXT | GET$$ | LISTVIEW SET STYLEXX | TREEVIEW GET TEXT |
| COMBOBOX GET USER | GETATTR | LISTVIEW SET TEXT | TREEVIEW GET USER |
| COMBOBOX INSERT | GRAPHIC ARC | LISTVIEW SET USER | TREEVIEW INSERT ITEM |
| COMBOBOX RESET | GRAPHIC ATTACH | LISTVIEW SORT | TREEVIEW RESET |
| COMBOBOX SELECT | GRAPHIC BITMAP END | LISTVIEW UNSELECT | TREEVIEW SELECT |
| COMBOBOX SET TEXT | GRAPHIC BITMAP LOAD | LISTVIEW VISIBLE | TREEVIEW SET BOLD |
| COMBOBOX SET USER | GRAPHIC BITMAP NEW | LO | TREEVIEW SET CHECK |
| COMBOBOX UNSELECT | GRAPHIC BOX | LOC | TREEVIEW SET EXPANDED |
| COMM LINE INPUT | GRAPHIC CELL | LOF | TREEVIEW SET IMAGELIST |
| COMMAND | GRAPHIC CELL SIZE | LOG | TREEVIEW SET TEXT |
| COMMAND$ | GRAPHIC CHR SIZE | LOG10 | TREEVIEW SET USER |
| CONTROL ADD | GRAPHIC CIRCLE | LOG1P | TREEVIEW UNSELECT |
| CONTROL ADD BUTTON | GRAPHIC CLEAR | LOG2 | TRIM |
| CONTROL ADD CHECK3STATE | GRAPHIC COLOR | LONG | TRIM$ |
| CONTROL ADD CHECKBOX | GRAPHIC COPY | LOWRD | TRUNC |
| CONTROL ADD COMBOBOX | GRAPHIC DETACH | LPRINT$ | UCASE |
| CONTROL ADD FRAME | GRAPHIC ELLIPSE | LSET$ | UCASE$ |
| CONTROL ADD GRAPHIC | GRAPHIC GET BITS | LTRIM | UCODE$ |
| CONTROL ADD HEADER | GRAPHIC GET CANVAS | LTRIM$ | UCODEPAGE |
| CONTROL ADD HOTKEY | GRAPHIC GET CAPTION | MAK | UDP NOTIFY |
| CONTROL ADD IMAGE | GRAPHIC GET CLIENT | MAX | UNWRAP |
| CONTROL ADD IMAGEX | GRAPHIC GET CLIP | MCASE | UNWRAP$ |
| CONTROL ADD IMGBUTTON | GRAPHIC GET DC | MCASE$ | USING |
| CONTROL ADD IMGBUTTONX | GRAPHIC GET LINES | MEMORY FILL | USING$ |
| CONTROL ADD IPADDRESS | GRAPHIC GET LOC | MEMORY FILLS | UTF8TOCHR |
| CONTROL ADD LABEL | GRAPHIC GET MIX | MENU ADD POPUP | UTF8TOCHR$ |
| CONTROL ADD LINE | GRAPHIC GET OVERLAP | MENU ADD STRING | VARPTR |
| CONTROL ADD LISTBOX | GRAPHIC GET PIXEL | MENU ATTACH | VERIFY |
| CONTROL ADD LISTVIEW | GRAPHIC GET POS | MENU CONTEXT | WAITKEY |
| CONTROL ADD OPTION | GRAPHIC GET PPI | MENU DELETE | WAITKEY$ |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET SCALE | MENU DRAW BAR | WORD |
| CONTROL ADD SCROLLBAR | GRAPHIC GET SCROLLTEXT | MENU GET STATE | WRAP |
| CONTROL ADD STATUSBAR | GRAPHIC GET SIZE | MENU GET TEXT | WRAP$ |
| CONTROL ADD TAB | GRAPHIC GET STRETCHMODE | MENU NEW BAR | WRITE |
| CONTROL ADD TEXTBOX | GRAPHIC GET TEXTALIGN | MENU NEW POPUP | WRITE# |
| CONTROL ADD TOOLBAR | GRAPHIC GET VIEW | MENU SET STATE | XPRINT ARC |
| CONTROL ADD TRACKBAR | GRAPHIC GET WORDWRAP | MENU SET TEXT | XPRINT ATTACH |
| CONTROL ADD TREEVIEW | GRAPHIC GET WRAP | METRICS | XPRINT BOX |
| CONTROL ADD UPDOWN | GRAPHIC IMAGELIST | MID | XPRINT CANCEL |
| CONTROL DISABLE | GRAPHIC INKEY$ | MIN | XPRINT CELL |
| CONTROL ENABLE | GRAPHIC INPUT | MKBYT | XPRINT CELL SIZE |
| CONTROL GET CHECK | GRAPHIC INPUT FLUSH | MKCUX | XPRINT CHR SIZE |
| CONTROL GET CLIENT | GRAPHIC INSTAT | MKDWD | XPRINT CLOSE |
| CONTROL GET LOC | GRAPHIC LINE | MKE | XPRINT COLOR |
| CONTROL GET SIZE | GRAPHIC LINE INPUT | MKS | XPRINT COPY |
| CONTROL GET TEXT | GRAPHIC PAINT | MKWRD | XPRINT ELLIPSE |
| CONTROL GET USER | GRAPHIC PIE | MOD | XPRINT FORMFEED |
| CONTROL HANDLE | GRAPHIC POLYGON | MONTHNAME | XPRINT GET ATTACH |
| CONTROL HIDE | GRAPHIC POLYLINE | MONTHNAME$ | XPRINT GET CANVAS |
| CONTROL KILL | GRAPHIC PRINT | NUL$ | XPRINT GET CLIENT |
| CONTROL NORMALIZE | GRAPHIC REDRAW | OCT | XPRINT GET CLIP |
| CONTROL POST | GRAPHIC RENDER | OCT$ | XPRINT GET COLLATE |
| CONTROL REDRAW | GRAPHIC SAVE | OEMTOCHR | XPRINT GET COLOR |
| CONTROL SEND | GRAPHIC SCALE | OEMTOCHR$ | XPRINT GET COLORMODE |
| CONTROL SET CHECK | GRAPHIC SCALE PIXELS | ON CALL | XPRINT GET COPIES |
| CONTROL SET CLIENT | GRAPHIC SET AUTOSIZE | ON GOSUB | XPRINT GET DC |
| CONTROL SET COLOR | GRAPHIC SET BITS | ON GOTO | XPRINT GET DUPLEX |
| CONTROL SET FOCUS | GRAPHIC SET CAPTION | OPEN | XPRINT GET LINES |
| CONTROL SET FONT | GRAPHIC SET CLIENT | PARSE$ | XPRINT GET MARGIN |
| CONTROL SET IMAGE | GRAPHIC SET CLIP | PARSECOUNT | XPRINT GET MIX |
| CONTROL SET IMAGEX | GRAPHIC SET FIXED | PATHNAME | XPRINT GET ORIENTATION |
| CONTROL SET IMGBUTTON | GRAPHIC SET FOCUS | PATHNAME$ | XPRINT GET OVERLAP |
| CONTROL SET IMGBUTTONX | GRAPHIC SET FONT | PATHSCAN | XPRINT GET PAGES |
| CONTROL SET LOC | GRAPHIC SET LOC | PATHSCAN$ | XPRINT GET PAPER |
| CONTROL SET OPTION | GRAPHIC SET MIX | PEEK | XPRINT GET PAPERS |
| CONTROL SET SIZE | GRAPHIC SET OVERLAP | PLAY | XPRINT GET PIXEL |
| CONTROL SET TEXT | GRAPHIC SET PIXEL | PLAY SOUND | XPRINT GET POS |
| CONTROL SET USER | GRAPHIC SET POS | POKE | XPRINT GET PPI |
| CONTROL SHOW STATE | GRAPHIC SET SCROLLTEXT | PRINTER$ | XPRINT GET QUALITY |
| COS | GRAPHIC SET SIZE | PRINTERCOUNT | XPRINT GET SCALE |
| COSH | GRAPHIC SET STRETCHMODE | PROGRESSBAR GET POS | XPRINT GET SELECTION |
| COT | GRAPHIC SET TEXTALIGN | PROGRESSBAR GET RANGE | XPRINT GET SIZE |
| COTH | GRAPHIC SET VIEW | PROGRESSBAR SET POS | XPRINT GET STRETCHMODE |
| CQUAD | GRAPHIC SET VIRTUAL | PROGRESSBAR SET RANGE | XPRINT GET TEXTALIGN |
| CSC | GRAPHIC SET WORDWRAP | PROGRESSBAR SET STEP | XPRINT GET TRAY |
| CSCH | GRAPHIC SET WRAP | PROGRESSBAR STEP | XPRINT GET TRAYS |
| CSET$ | GRAPHIC SPLIT | PUT | XPRINT GET WORDWRAP |
| CSNG | GRAPHIC STRETCH | PUT STR | XPRINT GET WRAP |
| CSTR | GRAPHIC STYLE | PUT WSTR | XPRINT IMAGELIST |
| CULNG | GRAPHIC TEXT SIZE | PUT$ | XPRINT LINE |
| CURDIR | GRAPHIC WAITKEY$ | PUT$$ | XPRINT PIE |
| CURDIR$ | GRAPHIC WIDTH | QUAD | XPRINT POLYGON |
| CVBYT | GRAPHIC WINDOW | RAISEEVENT | XPRINT POLYLINE |
| CVCUX | GRAPHIC WINDOW CLICK | READ | XPRINT PREVIEW |
| CVDWD | GRAPHIC WINDOW END | READ$ | XPRINT PRINT |
| CVL | GRAPHIC WINDOW HIDE | REDIM | XPRINT RENDER |
| CVQ | GRAPHIC WINDOW MINIMIZE | REMAIN | XPRINT SCALE |
| CVW | GRAPHIC WINDOW NONSTABLE | REMAIN$ | XPRINT SET CLIP |
| CWORD | GRAPHIC WINDOW NORMALIZE | REMOVE | XPRINT SET COLLATE |
| DATACOUNT | GRAPHIC WINDOW STABILIZE | REMOVE$ | XPRINT SET COLOR |
| DAYNAME | HEADER GET COUNT | REPEAT | XPRINT SET COLORMODE |
| DAYNAME$ | HEADER GET ITEM | REPEAT$ | XPRINT SET COPIES |
| DEC | HEADER SEND | RESOURCE SAVE FILE | XPRINT SET DUPLEX |
| DEC$ | HEADER SET ITEM | RESOURCE$ | XPRINT SET FONT |
| DECLARE | HEX | RETAIN | XPRINT SET MIX |
| DEF | HEX$ | RETAIN$ | XPRINT SET ORIENTATION |
| DESKTOP GET PPI | HI | RGB | XPRINT SET OVERLAP |
| DIALOG DEFAULT FONT | HIWRD | RIGHT | XPRINT SET PAGES |
| DIALOG DISABLE | HYPOT | RIGHT$ | XPRINT SET PAPER |
| DIALOG DOEVENTS | IIF | RND | XPRINT SET PIXEL |
| DIALOG ENABLE | IMAGELIST ADD BITMAP | ROUND | XPRINT SET POS |
| DIALOG END | IMAGELIST ADD ICON | RSET$ | XPRINT SET QUALITY |
| DIALOG GET CLIENT | IMAGELIST ADD MASKED | RTRIM | XPRINT SET STRETCHMODE |
| DIALOG GET LOC | IMAGELIST COUNT | RTRIM$ | XPRINT SET TEXTALIGN |
| DIALOG GET SIZE | IMAGELIST GET COUNT | SCROLLBAR GET PAGESIZE | XPRINT SET TRAY |
| DIALOG GET TEXT | IMAGELIST KILL | SCROLLBAR GET POS | XPRINT SET WORDWRAP |
| DIALOG GET USER | IMAGELIST NEW | SCROLLBAR GET RANGE | XPRINT SET WRAP |
| DIALOG HIDE | IMAGELIST NEW BITMAP | SCROLLBAR GET TRACKPOS | XPRINT SPLIT |
| DIALOG MAXIMIZE | IMAGELIST NEW ICON | SCROLLBAR SET PAGESIZE | XPRINT STRETCH |
| DIALOG MINIMIZE | IMAGELIST SET OVERLAY | SCROLLBAR SET POS | XPRINT STYLE |
| DIALOG NEW | IMP | SCROLLBAR SET RANGE | XPRINT TEXT SIZE |
| DIALOG NONSTABLE | INPUT# | SEC | XPRINT WIDTH |
| DIALOG NORMALIZE | INSTANCE | SECH |  |
| DIALOG PIXELS | INSTR | SEEK |  |
