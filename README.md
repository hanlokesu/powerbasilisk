...
> Summary:
> - **743** official PowerBASIC statement/function keywords currently available for use
>   - **595** implemented or completed by this fork
...
> - **113** DDT/GUI-class keywords deferred (Tier 3)
> - **0** documented keywords not yet implemented
>
> Counts are computed directly from [statement-coverage.csv](docs/statement-coverage.csv) (858 rows, deduplicated). **0** official keywords are not implemented yet; **2** entry is a fork extension, implemented here but not an official PB keyword (ARRAY SELECT, DIALOG CENTER). Rows whose internal codegen name uses an underscore are shown here in their spaced form - `GRAPHIC_CIRCLE` appears as `GRAPHIC CIRCLE`; the raw names are in [statement-coverage.md](docs/statement-coverage.md).
> Updated through batch 169 (v0.2.026). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 743 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | DECLARE | GRAPHIC SET CLIP | MKS | TANH |
| ACCEL ATTACH | DECR | GRAPHIC SET FIXED | MKS$ | TCP ACCEPT |
| ACODE | DEF | GRAPHIC SET FONT | MKWRD | TCP CLOSE |
| ACODE$ | DESKTOP GET CLIENT | GRAPHIC SET MIX | MKWRD$ | TCP LINE INPUT |
| ACOS | DESKTOP GET LOC | GRAPHIC SET PIXEL | MOD | TCP NOTIFY |
| ACOSH | DESKTOP GET PPI | GRAPHIC SET POS | MONTHNAME | TCP OPEN |
| AND | DESKTOP GET SIZE | GRAPHIC SET SIZE | MONTHNAME$ | TCP PRINT |
| ARRAY ADD | DIALOG DEFAULT FONT | GRAPHIC SET STRETCHMODE | MOUSEPTR | TCP RECV |
| ARRAY ARRAYIX | DIALOG DISABLE | GRAPHIC SET TEXTALIGN | MSGBOX | TCP SEND |
| ARRAY ASSIGN | DIALOG DOEVENTS | GRAPHIC SET VIEW | NAME | THREAD CLOSE |
| ARRAY COPY | DIALOG ENABLE | GRAPHIC SET VIRTUAL | NOT | THREAD CREATE |
| ARRAY DELETE | DIALOG END | GRAPHIC SET WORDWRAP | NUL | THREAD GET PRIORITY |
| ARRAY INSERT | DIALOG GET CLIENT | GRAPHIC SET WRAP | NUL$ | THREAD RESUME |
| ARRAY REDIM DECR | DIALOG GET LOC | GRAPHIC STYLE | OBJECT | THREAD SET PRIORITY |
| ARRAY REDIM INCR | DIALOG GET SIZE | GRAPHIC TEXT SIZE | OCT | THREAD STATUS |
| ARRAY REVERSE | DIALOG GET TEXT | GRAPHIC WIDTH | OCT$ | THREAD SUSPEND |
| ARRAY SCAN | DIALOG GET USER | HEADER GET COUNT | OEMTOCHR | THREADCOUNT |
| ARRAY SHUFFLE | DIALOG HIDE | HEADER GET ITEM | OEMTOCHR$ | THREADED |
| ARRAY SORT | DIALOG MAXIMIZE | HEADER SEND | ON CALL | THREADID |
| ARRAY SWAP | DIALOG MINIMIZE | HEADER SET ITEM | ON ERROR | TIMER |
| ARRAY TAGARRAY | DIALOG NEW | HEX | ON GOSUB | TIX |
| ARRAY TAGARRAY ERASE | DIALOG NONSTABLE | HEX$ | ON GOTO | TOOLBAR ADD BUTTON |
| ARRAY UNIQUE | DIALOG NORMALIZE | HI | OPEN | TOOLBAR ADD SEPARATOR |
| ASC | DIALOG PIXELS | HIWRD | OPTION EXPLICIT | TOOLBAR DELETE BUTTON |
| ASIN | DIALOG POST | HOST ADDR | OR | TOOLBAR GET COUNT |
| ASINH | DIALOG REDRAW | HOST NAME | PARSE | TOOLBAR GET STATE |
| ASM | DIALOG SEND | HYPOT | PARSE$ | TOOLBAR SET IMAGELIST |
| ASMDATA / END ASMDATA | DIALOG SET CLIENT | IF | PARSECOUNT | TOOLBAR SET STATE |
| ATANH | DIALOG SET COLOR | IF/END IF | PATHNAME | TRACE |
| ATN | DIALOG SET ICON | IIF | PATHNAME$ | TREEVIEW DELETE |
| ATN2 | DIALOG SET LOC | IMAGELIST COUNT | PATHSCAN | TREEVIEW GET COUNT |
| BEEP | DIALOG SET SIZE | IMAGELIST GET COUNT | PATHSCAN$ | TREEVIEW GET TEXT |
| BGR | DIALOG SET TEXT | IMAGELIST KILL | PEEK | TREEVIEW INSERT ITEM |
| BIN | DIALOG SET USER | IMAGELIST NEW | PLAY | TREEVIEW RESET |
| BIN$ | DIALOG SHOW MODAL | IMAGELIST NEW BITMAP | PLAY SOUND | TRIM |
| BIT | DIALOG SHOW MODELESS | IMP | PLAY WAVE | TRIM$ |
| BIT CALC | DIALOG SHOW STATE | IMPORT | POKE | TRUNC |
| BITS | DIALOG STABILIZE | INCR | PREFIX | TRY/END TRY |
| BITS$ | DIALOG UNITS | INPUT FLUSH | PRINT# | TYPE SET |
| BITSE | DIM | INPUT# | PRINTER$ | TYPE/END TYPE |
| BUILD | DIR | INSTANCE | PRINTERCOUNT | UCASE |
| BUILD$ | DIR CLOSE | INSTR | PROCESS GET PRIORITY | UCASE$ |
| BYTE | DIR$ | INT | PROCESS SET PRIORITY | UCODE$ |
| CALL | DISKFREE | INTEGER | PROFILE | UCODEPAGE |
| CALL DWORD | DISKSIZE | INTERFACE / END INTERFACE (DIRECT) | PROGRESSBAR GET POS | UDP CLOSE |
| CALLSTK | DISPLAY BROWSE | INTERFACE/END INTERFACE (IDBIND) | PROGRESSBAR GET RANGE | UDP NOTIFY |
| CALLSTK$ | DISPLAY COLOR | ISEVEN | PROGRESSBAR SET POS | UDP OPEN |
| CALLSTKCOUNT | DISPLAY FONT | ISFALSE | PROGRESSBAR SET RANGE | UDP RECV |
| CBOOL | DISPLAY OPENFILE | ISFILE | PROGRESSBAR SET STEP | UDP SEND |
| CBRT | DISPLAY SAVEFILE | ISFOLDER | PROGRESSBAR STEP | UNLOCK |
| CBYTE | DOUBLE | ISINFINITE | PUT | UNWRAP |
| CDBL | END | ISNORMAL | PUT STR | UNWRAP$ |
| CDWORD | ENVIRON | ISODD | PUT WSTR | USING |
| CEIL | ENVIRON$ | ISTRUE | PUT$ | USING$ |
| CFLT | EOF | ISWIN | PUT$$ | UTF8TOCHR |
| CHDIR | EQV | ITERATE | QUAD | UTF8TOCHR$ |
| CHDRIVE | ERASE | JOIN$ | RAISEEVENT | VAL |
| CHOOSE | ERF | KILL | RANDOMIZE | VARPTR |
| CHR | ERL | LCASE | READ | VERIFY |
| CHR$ | ERL$ | LCASE$ | READ$ | WAITKEY |
| CHRBYTES | ERR | LEFT | REDIM | WAITKEY$ |
| CHRTOOEM | ERRCLEAR | LEFT$ | REGEXPR | WINDOW GET |
| CHRTOOEM$ | ERROR | LEN | REGISTER | WINDOW SET |
| CHRTOUTF8 | ERROR$ | LET | REGREPL | WORD |
| CHRTOUTF8$ | EVENT SOURCE | LET *(WITH OBJECTS)* | REM | WRAP |
| CLASS/END CLASS | EVENTS | LET *(WITH TYPES)* | REMAIN | WRAP$ |
| CLIP | EXE | LET *(WITH VARIANTS)* | REMAIN$ | WRITE |
| CLIP$ | EXIST | LINE INPUT# | REMOVE | WRITE# |
| CLIPBOARD | EXIT | LISTBOX ADD | REMOVE$ | XOR |
| CLOSE | EXP | LISTBOX DELETE | REPEAT | XPRINT ARC |
| CLS | EXP10 | LISTBOX FIND | REPEAT$ | XPRINT ATTACH |
| CODEPTR | EXP2 | LISTBOX FIND EXACT | REPLACE | XPRINT BOX |
| COLOR | EXPM1 | LISTBOX GET COUNT | RESET | XPRINT CANCEL |
| COMBOBOX ADD | EXTRACT | LISTBOX GET SELCOUNT | RESOURCE SAVE FILE | XPRINT CELL |
| COMBOBOX DELETE | EXTRACT$ | LISTBOX GET SELECT | RESOURCE$ | XPRINT CELL SIZE |
| COMBOBOX FIND | FIELD | LISTBOX GET STATE | RESUME | XPRINT CHR SIZE |
| COMBOBOX FIND EXACT | FILEATTR | LISTBOX GET TEXT | RETAIN | XPRINT CLOSE |
| COMBOBOX GET COUNT | FILECOPY | LISTBOX GET USER | RETAIN$ | XPRINT COLOR |
| COMBOBOX GET SELCOUNT | FILENAME | LISTBOX INSERT | RETURN | XPRINT COPY |
| COMBOBOX GET SELECT | FILENAME$ | LISTBOX RESET | RGB | XPRINT ELLIPSE |
| COMBOBOX GET STATE | FILESCAN | LISTBOX SELECT | RIGHT | XPRINT FORMFEED |
| COMBOBOX GET TEXT | FIX | LISTBOX SET TEXT | RIGHT$ | XPRINT GET ATTACH |
| COMBOBOX GET USER | FLOOR | LISTBOX SET USER | RMDIR | XPRINT GET CANVAS |
| COMBOBOX INSERT | FLUSH | LISTBOX UNSELECT | RND | XPRINT GET CLIENT |
| COMBOBOX RESET | FONT END | LISTVIEW DELETE ITEM | ROTATE | XPRINT GET CLIP |
| COMBOBOX SELECT | FONT NEW | LISTVIEW GET COUNT | ROUND | XPRINT GET COLLATE |
| COMBOBOX SET TEXT | FOR / NEXT | LISTVIEW GET TEXT | RSET | XPRINT GET COLOR |
| COMBOBOX SET USER | FORMAT | LISTVIEW INSERT COLUMN | RSET$ | XPRINT GET COLORMODE |
| COMBOBOX UNSELECT | FORMAT$ | LISTVIEW INSERT ITEM | RTRIM | XPRINT GET COPIES |
| COMM CLOSE | FRAC | LISTVIEW RESET | RTRIM$ | XPRINT GET DC |
| COMM LINE | FRE | LISTVIEW SET TEXT | SCROLLBAR GET PAGESIZE | XPRINT GET DUPLEX |
| COMM LINE INPUT | FREEFILE | LO | SCROLLBAR GET POS | XPRINT GET LINES |
| COMM OPEN | FUNCNAME | LOC | SCROLLBAR GET RANGE | XPRINT GET MARGIN |
| COMM PRINT | FUNCNAME$ | LOCAL | SCROLLBAR GET TRACKPOS | XPRINT GET MIX |
| COMM RECV | FUNCTION / END FUNCTION | LOCK | SCROLLBAR SET PAGESIZE | XPRINT GET ORIENTATION |
| COMM RESET | GET | LOF | SCROLLBAR SET POS | XPRINT GET OVERLAP |
| COMM SEND | GET STR | LOG | SCROLLBAR SET RANGE | XPRINT GET PAGES |
| COMM SET | GET WSTR | LOG10 | SEC | XPRINT GET PAPER |
| COMM TIMEOUT | GET$ | LOG1P | SECH | XPRINT GET PAPERS |
| COMMAND | GET$$ | LOG2 | SEEK | XPRINT GET PIXEL |
| COMMAND$ | GETATTR | LONG | SELECT CASE/END SELECT | XPRINT GET POS |
| CONTROL ADD | GLOBAL | LOWRD | SETATTR | XPRINT GET PPI |
| CONTROL ADD BUTTON | GLOBALMEM | LPRINT | SETEOF | XPRINT GET QUALITY |
| CONTROL ADD CHECKBOX | GRAPHIC ARC | LPRINT ATTACH | SGN | XPRINT GET SCALE |
| CONTROL ADD COMBOBOX | GRAPHIC ATTACH | LPRINT CLOSE | SHELL | XPRINT GET SELECTION |
| CONTROL ADD LABEL | GRAPHIC BITMAP END | LPRINT FLUSH | SHIFT | XPRINT GET SIZE |
| CONTROL ADD LISTBOX | GRAPHIC BITMAP LOAD | LPRINT FORMFEED | SHRINK | XPRINT GET STRETCHMODE |
| CONTROL ADD LISTVIEW | GRAPHIC BITMAP NEW | LPRINT$ | SHRINK$ | XPRINT GET TEXTALIGN |
| CONTROL ADD PROGRESSBAR | GRAPHIC BOX | LSET | SIN | XPRINT GET TRAY |
| CONTROL ADD SCROLLBAR | GRAPHIC CELL | LSET$ | SINGLE | XPRINT GET TRAYS |
| CONTROL ADD STATUSBAR | GRAPHIC CELL SIZE | LTRIM | SINH | XPRINT GET WORDWRAP |
| CONTROL ADD TAB | GRAPHIC CHR SIZE | LTRIM$ | SIZEOF | XPRINT GET WRAP |
| CONTROL ADD TOOLBAR | GRAPHIC CIRCLE | MACRO/END MACRO | SLEEP | XPRINT IMAGELIST |
| CONTROL ADD TREEVIEW | GRAPHIC CLEAR | MAK | SPACE | XPRINT LINE |
| CONTROL DISABLE | GRAPHIC COLOR | MAT | SPACE$ | XPRINT PIE |
| CONTROL ENABLE | GRAPHIC COPY | MAX | SPLIT | XPRINT POLYGON |
| CONTROL GET CHECK | GRAPHIC DETACH | MCASE | SQR | XPRINT POLYLINE |
| CONTROL GET TEXT | GRAPHIC ELLIPSE | MCASE$ | STATIC | XPRINT PREVIEW |
| CONTROL HIDE | GRAPHIC GET BITS | MEMORY | STATUSBAR SET PARTS | XPRINT PRINT |
| CONTROL KILL | GRAPHIC GET CANVAS | MEMORY FILL | STATUSBAR SET TEXT | XPRINT RENDER |
| CONTROL SET CHECK | GRAPHIC GET CAPTION | MEMORY FILLS | STR | XPRINT SCALE |
| CONTROL SET TEXT | GRAPHIC GET CLIENT | MENU ADD POPUP | STR$ | XPRINT SET CLIP |
| COS | GRAPHIC GET CLIP | MENU ADD STRING | STRDELETE | XPRINT SET COLLATE |
| COSH | GRAPHIC GET DC | MENU DELETE | STRDELETE$ | XPRINT SET COLOR |
| COT | GRAPHIC GET LINES | MENU GET STATE | STRING | XPRINT SET COLORMODE |
| COTH | GRAPHIC GET LOC | MENU GET TEXT | STRINSERT | XPRINT SET COPIES |
| CQUAD | GRAPHIC GET MIX | MENU NEW BAR | STRINSERT$ | XPRINT SET DUPLEX |
| CSC | GRAPHIC GET PIXEL | MENU NEW POPUP | STRPTR | XPRINT SET FONT |
| CSCH | GRAPHIC GET POS | MENU SET STATE | STRREVERSE | XPRINT SET MIX |
| CSET | GRAPHIC GET PPI | MENU SET TEXT | STRREVERSE$ | XPRINT SET ORIENTATION |
| CSET$ | GRAPHIC GET SCALE | METHOD / END METHOD | SWAP | XPRINT SET OVERLAP |
| CSNG | GRAPHIC GET SIZE | METRICS | SWITCH | XPRINT SET PAGES |
| CSTR | GRAPHIC GET STRETCHMODE | MID | SWITCH$ | XPRINT SET PAPER |
| CULNG | GRAPHIC GET TEXTALIGN | MID$ | TAB DELETE | XPRINT SET PIXEL |
| CURDIR | GRAPHIC GET VIEW | MIN | TAB GET COUNT | XPRINT SET POS |
| CURDIR$ | GRAPHIC GET WORDWRAP | MKBYT | TAB GET DIALOG | XPRINT SET QUALITY |
| CVBYT | GRAPHIC GET WRAP | MKBYT$ | TAB GET IMAGE | XPRINT SET STRETCHMODE |
| CVCUX | GRAPHIC LINE | MKCUR$ | TAB GET PAGE | XPRINT SET TEXTALIGN |
| CVDWD | GRAPHIC PAINT | MKCUX | TAB GET SELECT | XPRINT SET TRAY |
| CVL | GRAPHIC PIE | MKCUX$ | TAB GET TEXT | XPRINT SET WORDWRAP |
| CVQ | GRAPHIC POLYGON | MKD$ | TAB INSERT PAGE | XPRINT SET WRAP |
| CVW | GRAPHIC POLYLINE | MKDIR | TAB RESET | XPRINT SPLIT |
| CWORD | GRAPHIC PRINT | MKDWD | TAB SELECT | XPRINT STRETCH |
| DATA | GRAPHIC SAVE | MKDWD$ | TAB SET IMAGE | XPRINT STYLE |
| DATACOUNT | GRAPHIC SCALE | MKE | TAB SET IMAGELIST | XPRINT TEXT SIZE |
| DAYNAME | GRAPHIC SCALE PIXELS | MKE$ | TAB SET TEXT | XPRINT WIDTH |
| DAYNAME$ | GRAPHIC SET AUTOSIZE | MKI$ | TAB$ |  |
| DEC | GRAPHIC SET BITS | MKL$ | TALLY |  |
| DEC$ | GRAPHIC SET CAPTION | MKQ$ | TAN |  |

## **Newly** implemented by this branch

> **595 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DIALOG SET TEXT | ISWIN | STATUSBAR SET PARTS |
| ACCEL ATTACH | DIALOG SET USER | JOIN$ | STATUSBAR SET TEXT |
| ACODE | DIALOG SHOW MODAL | LCASE | STR |
| ACODE$ | DIALOG SHOW MODELESS | LCASE$ | STR$ |
| ACOS | DIALOG SHOW STATE | LEFT | STRDELETE |
| ACOSH | DIALOG STABILIZE | LEFT$ | STRDELETE$ |
| ARRAY ADD | DIALOG UNITS | LEN | STRING |
| ARRAY ARRAYIX | DIM | LET *(WITH OBJECTS)* | STRINSERT |
| ARRAY ASSIGN | DIR | LET *(WITH VARIANTS)* | STRINSERT$ |
| ARRAY COPY | DIR CLOSE | LINE INPUT# | STRPTR |
| ARRAY DELETE | DIR$ | LISTBOX ADD | STRREVERSE |
| ARRAY INSERT | DISKFREE | LISTBOX DELETE | STRREVERSE$ |
| ARRAY REDIM DECR | DISKSIZE | LISTBOX FIND | SWITCH |
| ARRAY REDIM INCR | DISPLAY BROWSE | LISTBOX FIND EXACT | SWITCH$ |
| ARRAY REVERSE | DISPLAY COLOR | LISTBOX GET COUNT | TAB DELETE |
| ARRAY SCAN | DISPLAY FONT | LISTBOX GET SELCOUNT | TAB GET COUNT |
| ARRAY SHUFFLE | DISPLAY OPENFILE | LISTBOX GET SELECT | TAB GET DIALOG |
| ARRAY SWAP | DISPLAY SAVEFILE | LISTBOX GET STATE | TAB GET IMAGE |
| ARRAY TAGARRAY | DOUBLE | LISTBOX GET TEXT | TAB GET PAGE |
| ARRAY TAGARRAY ERASE | ENVIRON$ | LISTBOX GET USER | TAB GET SELECT |
| ARRAY UNIQUE | EOF | LISTBOX INSERT | TAB GET TEXT |
| ASIN | EQV | LISTBOX RESET | TAB INSERT PAGE |
| ASINH | ERF | LISTBOX SELECT | TAB RESET |
| ATANH | ERL | LISTBOX SET TEXT | TAB SELECT |
| ATN | ERL$ | LISTBOX SET USER | TAB SET IMAGE |
| ATN2 | ERR | LISTBOX UNSELECT | TAB SET IMAGELIST |
| BGR | ERRCLEAR | LISTVIEW DELETE ITEM | TAB SET TEXT |
| BIN | ERROR$ | LISTVIEW GET COUNT | TAB$ |
| BIN$ | EVENT SOURCE | LISTVIEW GET TEXT | TALLY |
| BITS | EVENTS | LISTVIEW INSERT COLUMN | TAN |
| BITS$ | EXE | LISTVIEW INSERT ITEM | TANH |
| BITSE | EXIST | LISTVIEW RESET | TCP NOTIFY |
| BUILD | EXP | LISTVIEW SET TEXT | THREADCOUNT |
| BUILD$ | EXP10 | LO | THREADID |
| BYTE | EXP2 | LOC | TIMER |
| CALLSTK$ | EXPM1 | LOF | TOOLBAR ADD BUTTON |
| CALLSTKCOUNT | EXTRACT | LOG | TOOLBAR ADD SEPARATOR |
| CBOOL | EXTRACT$ | LOG10 | TOOLBAR DELETE BUTTON |
| CBRT | FILEATTR | LOG1P | TOOLBAR GET COUNT |
| CBYTE | FILENAME | LOG2 | TOOLBAR GET STATE |
| CDBL | FILENAME$ | LONG | TOOLBAR SET IMAGELIST |
| CDWORD | FIX | LOWRD | TOOLBAR SET STATE |
| CEIL | FLOOR | LPRINT$ | TREEVIEW DELETE |
| CFLT | FONT END | LSET$ | TREEVIEW GET COUNT |
| CHOOSE | FONT NEW | LTRIM | TREEVIEW GET TEXT |
| CHR | FORMAT | LTRIM$ | TREEVIEW INSERT ITEM |
| CHR$ | FORMAT$ | MAK | TREEVIEW RESET |
| CHRBYTES | FRAC | MAX | TRIM |
| CHRTOOEM | FRE | MCASE | TRIM$ |
| CHRTOOEM$ | FREEFILE | MCASE$ | TRUNC |
| CHRTOUTF8 | FUNCNAME | MEMORY FILL | UCASE |
| CHRTOUTF8$ | FUNCNAME$ | MEMORY FILLS | UCASE$ |
| CLIP | GET | MENU ADD POPUP | UCODE$ |
| CLIP$ | GET STR | MENU ADD STRING | UCODEPAGE |
| CLOSE | GET WSTR | MENU DELETE | UDP NOTIFY |
| CODEPTR | GET$ | MENU GET STATE | UNWRAP |
| COMBOBOX ADD | GET$$ | MENU GET TEXT | UNWRAP$ |
| COMBOBOX DELETE | GETATTR | MENU NEW BAR | USING |
| COMBOBOX FIND | GRAPHIC ARC | MENU NEW POPUP | USING$ |
| COMBOBOX FIND EXACT | GRAPHIC ATTACH | MENU SET STATE | UTF8TOCHR |
| COMBOBOX GET COUNT | GRAPHIC BITMAP END | MENU SET TEXT | UTF8TOCHR$ |
| COMBOBOX GET SELCOUNT | GRAPHIC BITMAP LOAD | METRICS | VARPTR |
| COMBOBOX GET SELECT | GRAPHIC BITMAP NEW | MID | VERIFY |
| COMBOBOX GET STATE | GRAPHIC BOX | MIN | WAITKEY |
| COMBOBOX GET TEXT | GRAPHIC CELL | MKBYT | WAITKEY$ |
| COMBOBOX GET USER | GRAPHIC CELL SIZE | MKCUX | WORD |
| COMBOBOX INSERT | GRAPHIC CHR SIZE | MKDWD | WRAP |
| COMBOBOX RESET | GRAPHIC CIRCLE | MKE | WRAP$ |
| COMBOBOX SELECT | GRAPHIC CLEAR | MKS | WRITE |
| COMBOBOX SET TEXT | GRAPHIC COLOR | MKWRD | WRITE# |
| COMBOBOX SET USER | GRAPHIC COPY | MOD | XPRINT ARC |
| COMBOBOX UNSELECT | GRAPHIC DETACH | MONTHNAME | XPRINT ATTACH |
| COMM LINE INPUT | GRAPHIC ELLIPSE | MONTHNAME$ | XPRINT BOX |
| COMMAND | GRAPHIC GET BITS | NUL$ | XPRINT CANCEL |
| COMMAND$ | GRAPHIC GET CANVAS | OCT | XPRINT CELL |
| CONTROL ADD | GRAPHIC GET CAPTION | OCT$ | XPRINT CELL SIZE |
| CONTROL ADD BUTTON | GRAPHIC GET CLIENT | OEMTOCHR | XPRINT CHR SIZE |
| CONTROL ADD CHECKBOX | GRAPHIC GET CLIP | OEMTOCHR$ | XPRINT CLOSE |
| CONTROL ADD COMBOBOX | GRAPHIC GET DC | ON CALL | XPRINT COLOR |
| CONTROL ADD LABEL | GRAPHIC GET LINES | ON GOSUB | XPRINT COPY |
| CONTROL ADD LISTBOX | GRAPHIC GET LOC | ON GOTO | XPRINT ELLIPSE |
| CONTROL ADD LISTVIEW | GRAPHIC GET MIX | OPEN | XPRINT FORMFEED |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET PIXEL | PARSE$ | XPRINT GET ATTACH |
| CONTROL ADD SCROLLBAR | GRAPHIC GET POS | PARSECOUNT | XPRINT GET CANVAS |
| CONTROL ADD STATUSBAR | GRAPHIC GET PPI | PATHNAME | XPRINT GET CLIENT |
| CONTROL ADD TAB | GRAPHIC GET SCALE | PATHNAME$ | XPRINT GET CLIP |
| CONTROL ADD TOOLBAR | GRAPHIC GET SIZE | PATHSCAN | XPRINT GET COLLATE |
| CONTROL ADD TREEVIEW | GRAPHIC GET STRETCHMODE | PATHSCAN$ | XPRINT GET COLOR |
| CONTROL DISABLE | GRAPHIC GET TEXTALIGN | PEEK | XPRINT GET COLORMODE |
| CONTROL ENABLE | GRAPHIC GET VIEW | PLAY | XPRINT GET COPIES |
| CONTROL GET CHECK | GRAPHIC GET WORDWRAP | PLAY SOUND | XPRINT GET DC |
| CONTROL GET TEXT | GRAPHIC GET WRAP | POKE | XPRINT GET DUPLEX |
| CONTROL HIDE | GRAPHIC LINE | PRINTER$ | XPRINT GET LINES |
| CONTROL KILL | GRAPHIC PAINT | PRINTERCOUNT | XPRINT GET MARGIN |
| CONTROL SET CHECK | GRAPHIC PIE | PROGRESSBAR GET POS | XPRINT GET MIX |
| CONTROL SET TEXT | GRAPHIC POLYGON | PROGRESSBAR GET RANGE | XPRINT GET ORIENTATION |
| COS | GRAPHIC POLYLINE | PROGRESSBAR SET POS | XPRINT GET OVERLAP |
| COSH | GRAPHIC PRINT | PROGRESSBAR SET RANGE | XPRINT GET PAGES |
| COT | GRAPHIC SAVE | PROGRESSBAR SET STEP | XPRINT GET PAPER |
| COTH | GRAPHIC SCALE | PROGRESSBAR STEP | XPRINT GET PAPERS |
| CQUAD | GRAPHIC SCALE PIXELS | PUT | XPRINT GET PIXEL |
| CSC | GRAPHIC SET AUTOSIZE | PUT STR | XPRINT GET POS |
| CSCH | GRAPHIC SET BITS | PUT WSTR | XPRINT GET PPI |
| CSET$ | GRAPHIC SET CAPTION | PUT$ | XPRINT GET QUALITY |
| CSNG | GRAPHIC SET CLIP | PUT$$ | XPRINT GET SCALE |
| CSTR | GRAPHIC SET FIXED | QUAD | XPRINT GET SELECTION |
| CULNG | GRAPHIC SET FONT | RAISEEVENT | XPRINT GET SIZE |
| CURDIR | GRAPHIC SET MIX | READ | XPRINT GET STRETCHMODE |
| CURDIR$ | GRAPHIC SET PIXEL | READ$ | XPRINT GET TEXTALIGN |
| CVBYT | GRAPHIC SET POS | REDIM | XPRINT GET TRAY |
| CVCUX | GRAPHIC SET SIZE | REMAIN | XPRINT GET TRAYS |
| CVDWD | GRAPHIC SET STRETCHMODE | REMAIN$ | XPRINT GET WORDWRAP |
| CVL | GRAPHIC SET TEXTALIGN | REMOVE | XPRINT GET WRAP |
| CVQ | GRAPHIC SET VIEW | REMOVE$ | XPRINT IMAGELIST |
| CVW | GRAPHIC SET VIRTUAL | REPEAT | XPRINT LINE |
| CWORD | GRAPHIC SET WORDWRAP | REPEAT$ | XPRINT PIE |
| DATACOUNT | GRAPHIC SET WRAP | RESOURCE SAVE FILE | XPRINT POLYGON |
| DAYNAME | GRAPHIC STYLE | RESOURCE$ | XPRINT POLYLINE |
| DAYNAME$ | GRAPHIC TEXT SIZE | RETAIN | XPRINT PREVIEW |
| DEC | GRAPHIC WIDTH | RETAIN$ | XPRINT PRINT |
| DEC$ | HEADER GET COUNT | RGB | XPRINT RENDER |
| DECLARE | HEADER GET ITEM | RIGHT | XPRINT SCALE |
| DEF | HEADER SEND | RIGHT$ | XPRINT SET CLIP |
| DESKTOP GET PPI | HEADER SET ITEM | RND | XPRINT SET COLLATE |
| DIALOG DEFAULT FONT | HEX | ROUND | XPRINT SET COLOR |
| DIALOG DISABLE | HEX$ | RSET$ | XPRINT SET COLORMODE |
| DIALOG DOEVENTS | HI | RTRIM | XPRINT SET COPIES |
| DIALOG ENABLE | HIWRD | RTRIM$ | XPRINT SET DUPLEX |
| DIALOG END | HYPOT | SCROLLBAR GET PAGESIZE | XPRINT SET FONT |
| DIALOG GET CLIENT | IIF | SCROLLBAR GET POS | XPRINT SET MIX |
| DIALOG GET LOC | IMAGELIST COUNT | SCROLLBAR GET RANGE | XPRINT SET ORIENTATION |
| DIALOG GET SIZE | IMAGELIST GET COUNT | SCROLLBAR GET TRACKPOS | XPRINT SET OVERLAP |
| DIALOG GET TEXT | IMAGELIST KILL | SCROLLBAR SET PAGESIZE | XPRINT SET PAGES |
| DIALOG GET USER | IMAGELIST NEW | SCROLLBAR SET POS | XPRINT SET PAPER |
| DIALOG HIDE | IMAGELIST NEW BITMAP | SCROLLBAR SET RANGE | XPRINT SET PIXEL |
| DIALOG MAXIMIZE | IMP | SEC | XPRINT SET POS |
| DIALOG MINIMIZE | INPUT# | SECH | XPRINT SET QUALITY |
| DIALOG NEW | INSTANCE | SEEK | XPRINT SET STRETCHMODE |
| DIALOG NONSTABLE | INSTR | SETEOF | XPRINT SET TEXTALIGN |
| DIALOG NORMALIZE | INT | SGN | XPRINT SET TRAY |
| DIALOG PIXELS | INTEGER | SHRINK | XPRINT SET WORDWRAP |
| DIALOG POST | ISEVEN | SHRINK$ | XPRINT SET WRAP |
| DIALOG REDRAW | ISFALSE | SIN | XPRINT SPLIT |
| DIALOG SEND | ISFILE | SINGLE | XPRINT STRETCH |
| DIALOG SET CLIENT | ISFOLDER | SINH | XPRINT STYLE |
| DIALOG SET COLOR | ISINFINITE | SIZEOF | XPRINT TEXT SIZE |
| DIALOG SET ICON | ISNORMAL | SPACE | XPRINT WIDTH |
| DIALOG SET LOC | ISODD | SPACE$ |  |
| DIALOG SET SIZE | ISTRUE | SQR |  |
