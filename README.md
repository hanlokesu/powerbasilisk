...
> Summary:
> - **633** official PowerBASIC statement/function keywords currently available for use
>   - **485** implemented or completed by this fork
...
>
> Counts are computed directly from [statement-coverage.csv](docs/statement-coverage.csv) (738 rows, deduplicated). **2** official keywords are not implemented yet (METRICS, UCODE$); **1** entry is a fork extension, implemented here but not an official PB keyword (ARRAY SELECT). Rows whose internal codegen name uses an underscore are shown here in their spaced form - `GRAPHIC_CIRCLE` appears as `GRAPHIC CIRCLE`; the raw names are in [statement-coverage.md](docs/statement-coverage.md).
> Updated through batch 159 (v0.2.018). **2 official keyword(s) remain unimplemented: METRICS, UCODE$; all other non-Tier-3 official keywords are implemented.**

**All 633 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | DAYNAME$ | GRAPHIC SET CAPTION | MOD | THREAD CLOSE |
| ACCEL ATTACH | DEC | GRAPHIC SET CLIP | MONTHNAME | THREAD CREATE |
| ACODE | DEC$ | GRAPHIC SET FIXED | MONTHNAME$ | THREAD GET PRIORITY |
| ACODE$ | DECLARE | GRAPHIC SET FONT | MOUSEPTR | THREAD RESUME |
| ACOS | DECR | GRAPHIC SET MIX | MSGBOX | THREAD SET PRIORITY |
| ACOSH | DEF | GRAPHIC SET PIXEL | NAME | THREAD STATUS |
| AND | DESKTOP GET CLIENT | GRAPHIC SET POS | NOT | THREAD SUSPEND |
| ARRAY ADD | DESKTOP GET LOC | GRAPHIC SET SIZE | NUL | THREADCOUNT |
| ARRAY ARRAYIX | DESKTOP GET PPI | GRAPHIC SET STRETCHMODE | NUL$ | THREADED |
| ARRAY ASSIGN | DESKTOP GET SIZE | GRAPHIC SET TEXTALIGN | OBJECT | THREADID |
| ARRAY COPY | DIALOG CENTER | GRAPHIC SET VIEW | OCT | TIMER |
| ARRAY DELETE | DIALOG DOEVENTS | GRAPHIC SET VIRTUAL | OCT$ | TIX |
| ARRAY INSERT | DIALOG END | GRAPHIC SET WORDWRAP | OEMTOCHR | TRACE |
| ARRAY REDIM DECR | DIALOG GET TEXT | GRAPHIC SET WRAP | OEMTOCHR$ | TREEVIEW |
| ARRAY REDIM INCR | DIALOG NEW | GRAPHIC STYLE | ON CALL | TRIM |
| ARRAY REVERSE | DIALOG SET TEXT | GRAPHIC TEXT SIZE | ON ERROR | TRIM$ |
| ARRAY SCAN | DIALOG SHOW MODAL | GRAPHIC WIDTH | ON GOSUB | TRUNC |
| ARRAY SHUFFLE | DIALOG SHOW STATE | HEADER | ON GOTO | TRY/END TRY |
| ARRAY SORT | DIM | HEADER CTRL | OPEN | TYPE SET |
| ARRAY SWAP | DIR | HEX | OPTION EXPLICIT | TYPE/END TYPE |
| ARRAY TAGARRAY | DIR CLOSE | HEX$ | OR | UCASE |
| ARRAY TAGARRAY ERASE | DIR$ | HI | PARSE | UCASE$ |
| ARRAY UNIQUE | DISKFREE | HIWRD | PARSE$ | UCODEPAGE |
| ASC | DISKSIZE | HOST ADDR | PARSECOUNT | UDP CLOSE |
| ASIN | DISPLAY BROWSE | HOST NAME | PATHNAME | UDP NOTIFY |
| ASINH | DISPLAY COLOR | HYPOT | PATHNAME$ | UDP OPEN |
| ASM | DISPLAY FONT | IF | PATHSCAN | UDP RECV |
| ASMDATA / END ASMDATA | DISPLAY OPENFILE | IF/END IF | PATHSCAN$ | UDP SEND |
| ATANH | DISPLAY SAVEFILE | IIF | PEEK | UNLOCK |
| ATN | DOUBLE | IMAGELIST COUNT | PLAY | UNWRAP |
| ATN2 | END | IMAGELIST KILL | PLAY SOUND | UNWRAP$ |
| BEEP | ENVIRON | IMAGELIST NEW | PLAY WAVE | USING |
| BGR | ENVIRON$ | IMP | POKE | USING$ |
| BIN | EOF | IMPORT | PREFIX | UTF8TOCHR |
| BIN$ | EQV | INCR | PRINT# | UTF8TOCHR$ |
| BIT | ERASE | INPUT FLUSH | PRINTER$ | VAL |
| BIT CALC | ERF | INPUT# | PRINTERCOUNT | VARPTR |
| BITS | ERL | INSTANCE | PROCESS GET PRIORITY | VERIFY |
| BITS$ | ERL$ | INSTR | PROCESS SET PRIORITY | WAITKEY |
| BITSE | ERR | INT | PROFILE | WAITKEY$ |
| BUILD | ERRCLEAR | INTEGER | PROGRESSBAR | WINDOW GET |
| BUILD$ | ERROR | INTERFACE / END INTERFACE (DIRECT) | PUT | WINDOW SET |
| BYTE | ERROR$ | INTERFACE/END INTERFACE (IDBIND) | PUT STR | WORD |
| CALL | EVENT SOURCE | ISEVEN | PUT WSTR | WRAP |
| CALL DWORD | EVENTS | ISFALSE | PUT$ | WRAP$ |
| CALLSTK | EXE | ISFILE | PUT$$ | WRITE |
| CALLSTK$ | EXIST | ISFOLDER | QUAD | WRITE# |
| CALLSTKCOUNT | EXIT | ISINFINITE | RAISEEVENT | XOR |
| CBOOL | EXP | ISNORMAL | RANDOMIZE | XPRINT ARC |
| CBRT | EXP10 | ISODD | READ | XPRINT ATTACH |
| CBYTE | EXP2 | ISTRUE | READ$ | XPRINT BOX |
| CDBL | EXPM1 | ISWIN | REDIM | XPRINT CANCEL |
| CDWORD | EXTRACT | ITERATE | REGEXPR | XPRINT CELL |
| CEIL | EXTRACT$ | JOIN$ | REGISTER | XPRINT CELL SIZE |
| CFLT | FIELD | KILL | REGREPL | XPRINT CHR SIZE |
| CHDIR | FILEATTR | LCASE | REM | XPRINT CLOSE |
| CHDRIVE | FILECOPY | LCASE$ | REMAIN | XPRINT COLOR |
| CHOOSE | FILENAME | LEFT | REMAIN$ | XPRINT COPY |
| CHR | FILENAME$ | LEFT$ | REMOVE | XPRINT ELLIPSE |
| CHR$ | FILESCAN | LEN | REMOVE$ | XPRINT FORMFEED |
| CHRBYTES | FIX | LET | REPEAT | XPRINT GET ATTACH |
| CHRTOOEM | FLOOR | LET *(WITH OBJECTS)* | REPEAT$ | XPRINT GET CANVAS |
| CHRTOOEM$ | FLUSH | LET *(WITH TYPES)* | REPLACE | XPRINT GET CLIENT |
| CHRTOUTF8 | FONT END | LET *(WITH VARIANTS)* | RESET | XPRINT GET CLIP |
| CHRTOUTF8$ | FONT NEW | LINE INPUT# | RESOURCE SAVE FILE | XPRINT GET COLLATE |
| CLASS/END CLASS | FOR / NEXT | LISTVIEW | RESOURCE$ | XPRINT GET COLOR |
| CLIP | FORMAT | LO | RESUME | XPRINT GET COLORMODE |
| CLIP$ | FORMAT$ | LOC | RETAIN | XPRINT GET COPIES |
| CLIPBOARD | FRAC | LOCAL | RETAIN$ | XPRINT GET DC |
| CLOSE | FRE | LOCK | RETURN | XPRINT GET DUPLEX |
| CLS | FREEFILE | LOF | RGB | XPRINT GET LINES |
| CODEPTR | FUNCNAME | LOG | RIGHT | XPRINT GET MARGIN |
| COLOR | FUNCNAME$ | LOG10 | RIGHT$ | XPRINT GET MIX |
| COMM CLOSE | FUNCTION / END FUNCTION | LOG1P | RMDIR | XPRINT GET ORIENTATION |
| COMM LINE | GET | LOG2 | RND | XPRINT GET OVERLAP |
| COMM LINE INPUT | GET STR | LONG | ROTATE | XPRINT GET PAGES |
| COMM OPEN | GET WSTR | LOWRD | ROUND | XPRINT GET PAPER |
| COMM PRINT | GET$ | LPRINT | RSET | XPRINT GET PAPERS |
| COMM RECV | GET$$ | LPRINT ATTACH | RSET$ | XPRINT GET PIXEL |
| COMM RESET | GETATTR | LPRINT CLOSE | RTRIM | XPRINT GET POS |
| COMM SEND | GLOBAL | LPRINT FLUSH | RTRIM$ | XPRINT GET PPI |
| COMM SET | GLOBALMEM | LPRINT FORMFEED | SEC | XPRINT GET QUALITY |
| COMM TIMEOUT | GRAPHIC ARC | LPRINT$ | SECH | XPRINT GET SCALE |
| COMMAND | GRAPHIC ATTACH | LSET | SEEK | XPRINT GET SELECTION |
| COMMAND$ | GRAPHIC BITMAP END | LSET$ | SELECT CASE/END SELECT | XPRINT GET SIZE |
| CONTROL ADD BUTTON | GRAPHIC BITMAP LOAD | LTRIM | SETATTR | XPRINT GET STRETCHMODE |
| CONTROL ADD CHECKBOX | GRAPHIC BITMAP NEW | LTRIM$ | SETEOF | XPRINT GET TEXTALIGN |
| CONTROL ADD COMBOBOX | GRAPHIC BOX | MACRO/END MACRO | SGN | XPRINT GET TRAY |
| CONTROL ADD LABEL | GRAPHIC CELL | MAK | SHELL | XPRINT GET TRAYS |
| CONTROL ADD LISTBOX | GRAPHIC CELL SIZE | MAT | SHIFT | XPRINT GET WORDWRAP |
| CONTROL ADD LISTVIEW | GRAPHIC CHR SIZE | MAX | SHRINK | XPRINT GET WRAP |
| CONTROL ADD PROGRESSBAR | GRAPHIC CIRCLE | MCASE | SHRINK$ | XPRINT IMAGELIST |
| CONTROL ADD SCROLLBAR | GRAPHIC CLEAR | MCASE$ | SIN | XPRINT LINE |
| CONTROL ADD TREEVIEW | GRAPHIC COLOR | MEMORY | SINGLE | XPRINT PIE |
| CONTROL ADDSTRING | GRAPHIC COPY | MEMORY FILL | SINH | XPRINT POLYGON |
| CONTROL DISABLE | GRAPHIC DETACH | MEMORY FILLS | SIZEOF | XPRINT POLYLINE |
| CONTROL ENABLE | GRAPHIC ELLIPSE | MENU ADD POPUP | SLEEP | XPRINT PREVIEW |
| CONTROL GET CHECK | GRAPHIC GET BITS | MENU ADD STRING | SPACE | XPRINT PRINT |
| CONTROL GET TEXT | GRAPHIC GET CANVAS | MENU DELETE | SPACE$ | XPRINT RENDER |
| CONTROL HIDE | GRAPHIC GET CAPTION | MENU GET STATE | SPLIT | XPRINT SCALE |
| CONTROL KILL | GRAPHIC GET CLIENT | MENU GET TEXT | SQR | XPRINT SET CLIP |
| CONTROL SET CHECK | GRAPHIC GET CLIP | MENU NEW BAR | STATIC | XPRINT SET COLLATE |
| CONTROL SET TEXT | GRAPHIC GET DC | MENU NEW POPUP | STR | XPRINT SET COLOR |
| COS | GRAPHIC GET LINES | MENU SET STATE | STR$ | XPRINT SET COLORMODE |
| COSH | GRAPHIC GET LOC | MENU SET TEXT | STRDELETE | XPRINT SET COPIES |
| COT | GRAPHIC GET MIX | METHOD / END METHOD | STRDELETE$ | XPRINT SET DUPLEX |
| COTH | GRAPHIC GET PIXEL | MID | STRING | XPRINT SET FONT |
| CQUAD | GRAPHIC GET POS | MID$ | STRINSERT | XPRINT SET MIX |
| CSC | GRAPHIC GET PPI | MIN | STRINSERT$ | XPRINT SET ORIENTATION |
| CSCH | GRAPHIC GET SCALE | MKBYT | STRPTR | XPRINT SET OVERLAP |
| CSET | GRAPHIC GET SIZE | MKBYT$ | STRREVERSE | XPRINT SET PAGES |
| CSET$ | GRAPHIC GET STRETCHMODE | MKCUR$ | STRREVERSE$ | XPRINT SET PAPER |
| CSNG | GRAPHIC GET TEXTALIGN | MKCUX | SWAP | XPRINT SET PIXEL |
| CSTR | GRAPHIC GET VIEW | MKCUX$ | SWITCH | XPRINT SET POS |
| CULNG | GRAPHIC GET WORDWRAP | MKD$ | SWITCH$ | XPRINT SET QUALITY |
| CURDIR | GRAPHIC GET WRAP | MKDIR | TAB$ | XPRINT SET STRETCHMODE |
| CURDIR$ | GRAPHIC LINE | MKDWD | TALLY | XPRINT SET TEXTALIGN |
| CVBYT | GRAPHIC PAINT | MKDWD$ | TAN | XPRINT SET TRAY |
| CVCUX | GRAPHIC PIE | MKE | TANH | XPRINT SET WORDWRAP |
| CVDWD | GRAPHIC POLYGON | MKE$ | TCP ACCEPT | XPRINT SET WRAP |
| CVL | GRAPHIC POLYLINE | MKI$ | TCP CLOSE | XPRINT SPLIT |
| CVQ | GRAPHIC PRINT | MKL$ | TCP LINE INPUT | XPRINT STRETCH |
| CVW | GRAPHIC SAVE | MKQ$ | TCP NOTIFY | XPRINT STYLE |
| CWORD | GRAPHIC SCALE | MKS | TCP OPEN | XPRINT TEXT SIZE |
| DATA | GRAPHIC SCALE PIXELS | MKS$ | TCP PRINT | XPRINT WIDTH |
| DATACOUNT | GRAPHIC SET AUTOSIZE | MKWRD | TCP RECV |  |
| DAYNAME | GRAPHIC SET BITS | MKWRD$ | TCP SEND |  |

## **Newly** implemented by this branch

> **485 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.
...
| ACCEL ATTACH | DISPLAY SAVEFILE | ISFILE | STRDELETE |
| ACODE | DOUBLE | ISFOLDER | STRDELETE$ |
| ACODE$ | ENVIRON$ | ISINFINITE | STRING |
| ACOS | EOF | ISNORMAL | STRINSERT |
| ACOSH | EQV | ISODD | STRINSERT$ |
| ARRAY ADD | ERF | ISTRUE | STRPTR |
| ARRAY ARRAYIX | ERL | ISWIN | STRREVERSE |
| ARRAY ASSIGN | ERL$ | JOIN$ | STRREVERSE$ |
| ARRAY COPY | ERR | LCASE | SWITCH |
| ARRAY DELETE | ERRCLEAR | LCASE$ | SWITCH$ |
| ARRAY INSERT | ERROR$ | LEFT | TAB$ |
| ARRAY REDIM DECR | EVENT SOURCE | LEFT$ | TALLY |
| ARRAY REDIM INCR | EVENTS | LEN | TAN |
| ARRAY REVERSE | EXE | LET *(WITH OBJECTS)* | TANH |
| ARRAY SCAN | EXIST | LET *(WITH VARIANTS)* | TCP NOTIFY |
| ARRAY SHUFFLE | EXP | LINE INPUT# | THREADCOUNT |
| ARRAY SWAP | EXP10 | LISTVIEW | THREADID |
| ARRAY TAGARRAY | EXP2 | LO | TIMER |
| ARRAY TAGARRAY ERASE | EXPM1 | LOC | TREEVIEW |
| ARRAY UNIQUE | EXTRACT | LOF | TRIM |
| ASIN | EXTRACT$ | LOG | TRIM$ |
| ASINH | FILEATTR | LOG10 | TRUNC |
| ATANH | FILENAME | LOG1P | UCASE |
| ATN | FILENAME$ | LOG2 | UCASE$ |
| ATN2 | FIX | LONG | UCODEPAGE |
| BGR | FLOOR | LOWRD | UDP NOTIFY |
| BIN | FONT END | LPRINT$ | UNWRAP |
| BIN$ | FONT NEW | LSET$ | UNWRAP$ |
| BITS | FORMAT | LTRIM | USING |
| BITS$ | FORMAT$ | LTRIM$ | USING$ |
| BITSE | FRAC | MAK | UTF8TOCHR |
| BUILD | FRE | MAX | UTF8TOCHR$ |
| BUILD$ | FREEFILE | MCASE | VARPTR |
| BYTE | FUNCNAME | MCASE$ | VERIFY |
| CALLSTK$ | FUNCNAME$ | MEMORY FILL | WAITKEY |
| CALLSTKCOUNT | GET | MEMORY FILLS | WAITKEY$ |
| CBOOL | GET STR | MENU ADD POPUP | WORD |
| CBRT | GET WSTR | MENU ADD STRING | WRAP |
| CBYTE | GET$ | MENU DELETE | WRAP$ |
| CDBL | GET$$ | MENU GET STATE | WRITE |
| CDWORD | GETATTR | MENU GET TEXT | WRITE# |
| CEIL | GRAPHIC ARC | MENU NEW BAR | XPRINT ARC |
| CFLT | GRAPHIC ATTACH | MENU NEW POPUP | XPRINT ATTACH |
| CHOOSE | GRAPHIC BITMAP END | MENU SET STATE | XPRINT BOX |
| CHR | GRAPHIC BITMAP LOAD | MENU SET TEXT | XPRINT CANCEL |
...
| DESKTOP GET PPI | HEADER | RSET$ | XPRINT SET PAGES |
| DIALOG CENTER | HEADER CTRL | RTRIM | XPRINT SET PAPER |
| DIALOG DOEVENTS | HEX | RTRIM$ | XPRINT SET PIXEL |
| DIALOG END | HEX$ | SEC | XPRINT SET POS |
| DIALOG GET TEXT | HI | SECH | XPRINT SET QUALITY |
| DIALOG NEW | HIWRD | SEEK | XPRINT SET STRETCHMODE |
| DIALOG SET TEXT | HYPOT | SETEOF | XPRINT SET TEXTALIGN |
| DIALOG SHOW MODAL | IIF | SGN | XPRINT SET TRAY |
| DIALOG SHOW STATE | IMAGELIST COUNT | SHRINK | XPRINT SET WORDWRAP |
| DIM | IMAGELIST KILL | SHRINK$ | XPRINT SET WRAP |
| DIR | IMAGELIST NEW | SIN | XPRINT SPLIT |
| DIR CLOSE | IMP | SINGLE | XPRINT STRETCH |
| DIR$ | INPUT# | SINH | XPRINT STYLE |
| DISKFREE | INSTANCE | SIZEOF | XPRINT TEXT SIZE |
| DISKSIZE | INSTR | SPACE | XPRINT WIDTH |
| DISPLAY BROWSE | INT | SPACE$ |  |
| DISPLAY COLOR | INTEGER | SQR |  |
| DISPLAY FONT | ISEVEN | STR |  |
