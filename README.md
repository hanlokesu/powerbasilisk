...
> Summary:
> - **636** official PowerBASIC statement/function keywords currently available for use
>   - **488** implemented or completed by this fork
...
> - **100** DDT/GUI-class keywords deferred (Tier 3)
...
> Updated through batch 163 (v0.2.020). **No official keyword remains unimplemented - every non-Tier-3 official keyword is implemented.**
...
**All 636 official keywords currently available for use (alphabetical):**

| Keyword | Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|---------|
| ABS | DEC | GRAPHIC SET CAPTION | MONTHNAME | THREAD GET PRIORITY |
| ACCEL ATTACH | DEC$ | GRAPHIC SET CLIP | MONTHNAME$ | THREAD RESUME |
| ACODE | DECLARE | GRAPHIC SET FIXED | MOUSEPTR | THREAD SET PRIORITY |
| ACODE$ | DECR | GRAPHIC SET FONT | MSGBOX | THREAD STATUS |
| ACOS | DEF | GRAPHIC SET MIX | NAME | THREAD SUSPEND |
| ACOSH | DESKTOP GET CLIENT | GRAPHIC SET PIXEL | NOT | THREADCOUNT |
| AND | DESKTOP GET LOC | GRAPHIC SET POS | NUL | THREADED |
| ARRAY ADD | DESKTOP GET PPI | GRAPHIC SET SIZE | NUL$ | THREADID |
| ARRAY ARRAYIX | DESKTOP GET SIZE | GRAPHIC SET STRETCHMODE | OBJECT | TIMER |
| ARRAY ASSIGN | DIALOG CENTER | GRAPHIC SET TEXTALIGN | OCT | TIX |
| ARRAY COPY | DIALOG DOEVENTS | GRAPHIC SET VIEW | OCT$ | TRACE |
| ARRAY DELETE | DIALOG END | GRAPHIC SET VIRTUAL | OEMTOCHR | TREEVIEW |
| ARRAY INSERT | DIALOG GET SIZE | GRAPHIC SET WORDWRAP | OEMTOCHR$ | TRIM |
| ARRAY REDIM DECR | DIALOG GET TEXT | GRAPHIC SET WRAP | ON CALL | TRIM$ |
| ARRAY REDIM INCR | DIALOG NEW | GRAPHIC STYLE | ON ERROR | TRUNC |
| ARRAY REVERSE | DIALOG SET SIZE | GRAPHIC TEXT SIZE | ON GOSUB | TRY/END TRY |
| ARRAY SCAN | DIALOG SET TEXT | GRAPHIC WIDTH | ON GOTO | TYPE SET |
| ARRAY SHUFFLE | DIALOG SHOW MODAL | HEADER | OPEN | TYPE/END TYPE |
| ARRAY SORT | DIALOG SHOW STATE | HEX | OPTION EXPLICIT | UCASE |
| ARRAY SWAP | DIM | HEX$ | OR | UCASE$ |
| ARRAY TAGARRAY | DIR | HI | PARSE | UCODE$ |
| ARRAY TAGARRAY ERASE | DIR CLOSE | HIWRD | PARSE$ | UCODEPAGE |
| ARRAY UNIQUE | DIR$ | HOST ADDR | PARSECOUNT | UDP CLOSE |
| ASC | DISKFREE | HOST NAME | PATHNAME | UDP NOTIFY |
| ASIN | DISKSIZE | HYPOT | PATHNAME$ | UDP OPEN |
| ASINH | DISPLAY BROWSE | IF | PATHSCAN | UDP RECV |
| ASM | DISPLAY COLOR | IF/END IF | PATHSCAN$ | UDP SEND |
| ASMDATA / END ASMDATA | DISPLAY FONT | IIF | PEEK | UNLOCK |
| ATANH | DISPLAY OPENFILE | IMAGELIST COUNT | PLAY | UNWRAP |
| ATN | DISPLAY SAVEFILE | IMAGELIST KILL | PLAY SOUND | UNWRAP$ |
| ATN2 | DOUBLE | IMAGELIST NEW | PLAY WAVE | USING |
| BEEP | END | IMP | POKE | USING$ |
| BGR | ENVIRON | IMPORT | PREFIX | UTF8TOCHR |
| BIN | ENVIRON$ | INCR | PRINT# | UTF8TOCHR$ |
| BIN$ | EOF | INPUT FLUSH | PRINTER$ | VAL |
| BIT | EQV | INPUT# | PRINTERCOUNT | VARPTR |
| BIT CALC | ERASE | INSTANCE | PROCESS GET PRIORITY | VERIFY |
| BITS | ERF | INSTR | PROCESS SET PRIORITY | WAITKEY |
| BITS$ | ERL | INT | PROFILE | WAITKEY$ |
| BITSE | ERL$ | INTEGER | PROGRESSBAR | WINDOW GET |
| BUILD | ERR | INTERFACE / END INTERFACE (DIRECT) | PUT | WINDOW SET |
| BUILD$ | ERRCLEAR | INTERFACE/END INTERFACE (IDBIND) | PUT STR | WORD |
| BYTE | ERROR | ISEVEN | PUT WSTR | WRAP |
| CALL | ERROR$ | ISFALSE | PUT$ | WRAP$ |
| CALL DWORD | EVENT SOURCE | ISFILE | PUT$$ | WRITE |
| CALLSTK | EVENTS | ISFOLDER | QUAD | WRITE# |
| CALLSTK$ | EXE | ISINFINITE | RAISEEVENT | XOR |
| CALLSTKCOUNT | EXIST | ISNORMAL | RANDOMIZE | XPRINT ARC |
| CBOOL | EXIT | ISODD | READ | XPRINT ATTACH |
| CBRT | EXP | ISTRUE | READ$ | XPRINT BOX |
| CBYTE | EXP10 | ISWIN | REDIM | XPRINT CANCEL |
| CDBL | EXP2 | ITERATE | REGEXPR | XPRINT CELL |
| CDWORD | EXPM1 | JOIN$ | REGISTER | XPRINT CELL SIZE |
| CEIL | EXTRACT | KILL | REGREPL | XPRINT CHR SIZE |
| CFLT | EXTRACT$ | LCASE | REM | XPRINT CLOSE |
| CHDIR | FIELD | LCASE$ | REMAIN | XPRINT COLOR |
| CHDRIVE | FILEATTR | LEFT | REMAIN$ | XPRINT COPY |
| CHOOSE | FILECOPY | LEFT$ | REMOVE | XPRINT ELLIPSE |
| CHR | FILENAME | LEN | REMOVE$ | XPRINT FORMFEED |
| CHR$ | FILENAME$ | LET | REPEAT | XPRINT GET ATTACH |
| CHRBYTES | FILESCAN | LET *(WITH OBJECTS)* | REPEAT$ | XPRINT GET CANVAS |
| CHRTOOEM | FIX | LET *(WITH TYPES)* | REPLACE | XPRINT GET CLIENT |
| CHRTOOEM$ | FLOOR | LET *(WITH VARIANTS)* | RESET | XPRINT GET CLIP |
| CHRTOUTF8 | FLUSH | LINE INPUT# | RESOURCE SAVE FILE | XPRINT GET COLLATE |
| CHRTOUTF8$ | FONT END | LISTVIEW | RESOURCE$ | XPRINT GET COLOR |
| CLASS/END CLASS | FONT NEW | LO | RESUME | XPRINT GET COLORMODE |
| CLIP | FOR / NEXT | LOC | RETAIN | XPRINT GET COPIES |
| CLIP$ | FORMAT | LOCAL | RETAIN$ | XPRINT GET DC |
| CLIPBOARD | FORMAT$ | LOCK | RETURN | XPRINT GET DUPLEX |
| CLOSE | FRAC | LOF | RGB | XPRINT GET LINES |
| CLS | FRE | LOG | RIGHT | XPRINT GET MARGIN |
| CODEPTR | FREEFILE | LOG10 | RIGHT$ | XPRINT GET MIX |
| COLOR | FUNCNAME | LOG1P | RMDIR | XPRINT GET ORIENTATION |
| COMM CLOSE | FUNCNAME$ | LOG2 | RND | XPRINT GET OVERLAP |
| COMM LINE | FUNCTION / END FUNCTION | LONG | ROTATE | XPRINT GET PAGES |
| COMM LINE INPUT | GET | LOWRD | ROUND | XPRINT GET PAPER |
| COMM OPEN | GET STR | LPRINT | RSET | XPRINT GET PAPERS |
| COMM PRINT | GET WSTR | LPRINT ATTACH | RSET$ | XPRINT GET PIXEL |
| COMM RECV | GET$ | LPRINT CLOSE | RTRIM | XPRINT GET POS |
| COMM RESET | GET$$ | LPRINT FLUSH | RTRIM$ | XPRINT GET PPI |
| COMM SEND | GETATTR | LPRINT FORMFEED | SEC | XPRINT GET QUALITY |
| COMM SET | GLOBAL | LPRINT$ | SECH | XPRINT GET SCALE |
| COMM TIMEOUT | GLOBALMEM | LSET | SEEK | XPRINT GET SELECTION |
| COMMAND | GRAPHIC ARC | LSET$ | SELECT CASE/END SELECT | XPRINT GET SIZE |
| COMMAND$ | GRAPHIC ATTACH | LTRIM | SETATTR | XPRINT GET STRETCHMODE |
| CONTROL ADD BUTTON | GRAPHIC BITMAP END | LTRIM$ | SETEOF | XPRINT GET TEXTALIGN |
| CONTROL ADD CHECKBOX | GRAPHIC BITMAP LOAD | MACRO/END MACRO | SGN | XPRINT GET TRAY |
| CONTROL ADD COMBOBOX | GRAPHIC BITMAP NEW | MAK | SHELL | XPRINT GET TRAYS |
| CONTROL ADD LABEL | GRAPHIC BOX | MAT | SHIFT | XPRINT GET WORDWRAP |
| CONTROL ADD LISTBOX | GRAPHIC CELL | MAX | SHRINK | XPRINT GET WRAP |
| CONTROL ADD LISTVIEW | GRAPHIC CELL SIZE | MCASE | SHRINK$ | XPRINT IMAGELIST |
| CONTROL ADD PROGRESSBAR | GRAPHIC CHR SIZE | MCASE$ | SIN | XPRINT LINE |
| CONTROL ADD SCROLLBAR | GRAPHIC CIRCLE | MEMORY | SINGLE | XPRINT PIE |
| CONTROL ADD TREEVIEW | GRAPHIC CLEAR | MEMORY FILL | SINH | XPRINT POLYGON |
| CONTROL ADDSTRING | GRAPHIC COLOR | MEMORY FILLS | SIZEOF | XPRINT POLYLINE |
| CONTROL DISABLE | GRAPHIC COPY | MENU ADD POPUP | SLEEP | XPRINT PREVIEW |
| CONTROL ENABLE | GRAPHIC DETACH | MENU ADD STRING | SPACE | XPRINT PRINT |
| CONTROL GET CHECK | GRAPHIC ELLIPSE | MENU DELETE | SPACE$ | XPRINT RENDER |
| CONTROL GET TEXT | GRAPHIC GET BITS | MENU GET STATE | SPLIT | XPRINT SCALE |
| CONTROL HIDE | GRAPHIC GET CANVAS | MENU GET TEXT | SQR | XPRINT SET CLIP |
| CONTROL KILL | GRAPHIC GET CAPTION | MENU NEW BAR | STATIC | XPRINT SET COLLATE |
| CONTROL SET CHECK | GRAPHIC GET CLIENT | MENU NEW POPUP | STR | XPRINT SET COLOR |
| CONTROL SET TEXT | GRAPHIC GET CLIP | MENU SET STATE | STR$ | XPRINT SET COLORMODE |
| COS | GRAPHIC GET DC | MENU SET TEXT | STRDELETE | XPRINT SET COPIES |
| COSH | GRAPHIC GET LINES | METHOD / END METHOD | STRDELETE$ | XPRINT SET DUPLEX |
| COT | GRAPHIC GET LOC | METRICS | STRING | XPRINT SET FONT |
| COTH | GRAPHIC GET MIX | MID | STRINSERT | XPRINT SET MIX |
| CQUAD | GRAPHIC GET PIXEL | MID$ | STRINSERT$ | XPRINT SET ORIENTATION |
| CSC | GRAPHIC GET POS | MIN | STRPTR | XPRINT SET OVERLAP |
| CSCH | GRAPHIC GET PPI | MKBYT | STRREVERSE | XPRINT SET PAGES |
| CSET | GRAPHIC GET SCALE | MKBYT$ | STRREVERSE$ | XPRINT SET PAPER |
| CSET$ | GRAPHIC GET SIZE | MKCUR$ | SWAP | XPRINT SET PIXEL |
| CSNG | GRAPHIC GET STRETCHMODE | MKCUX | SWITCH | XPRINT SET POS |
| CSTR | GRAPHIC GET TEXTALIGN | MKCUX$ | SWITCH$ | XPRINT SET QUALITY |
| CULNG | GRAPHIC GET VIEW | MKD$ | TAB$ | XPRINT SET STRETCHMODE |
| CURDIR | GRAPHIC GET WORDWRAP | MKDIR | TALLY | XPRINT SET TEXTALIGN |
| CURDIR$ | GRAPHIC GET WRAP | MKDWD | TAN | XPRINT SET TRAY |
| CVBYT | GRAPHIC LINE | MKDWD$ | TANH | XPRINT SET WORDWRAP |
| CVCUX | GRAPHIC PAINT | MKE | TCP ACCEPT | XPRINT SET WRAP |
| CVDWD | GRAPHIC PIE | MKE$ | TCP CLOSE | XPRINT SPLIT |
| CVL | GRAPHIC POLYGON | MKI$ | TCP LINE INPUT | XPRINT STRETCH |
| CVQ | GRAPHIC POLYLINE | MKL$ | TCP NOTIFY | XPRINT STYLE |
| CVW | GRAPHIC PRINT | MKQ$ | TCP OPEN | XPRINT TEXT SIZE |
| CWORD | GRAPHIC SAVE | MKS | TCP PRINT | XPRINT WIDTH |
| DATA | GRAPHIC SCALE | MKS$ | TCP RECV |  |
| DATACOUNT | GRAPHIC SCALE PIXELS | MKWRD | TCP SEND |  |
| DAYNAME | GRAPHIC SET AUTOSIZE | MKWRD$ | THREAD CLOSE |  |
| DAYNAME$ | GRAPHIC SET BITS | MOD | THREAD CREATE |  |

## **Newly** implemented by this branch

> **488 official keywords implemented by this fork** (every official keyword this fork added or completed). Verified via live compilation and testing. Listed alphabetically below.

**The keywords implemented by this fork (alphabetical):**

| Keyword | Keyword | Keyword | Keyword |
|---------|---------|---------|---------|
| ABS | DISPLAY COLOR | ISEVEN | SQR |
| ACCEL ATTACH | DISPLAY FONT | ISFALSE | STR |
| ACODE | DISPLAY OPENFILE | ISFILE | STR$ |
| ACODE$ | DISPLAY SAVEFILE | ISFOLDER | STRDELETE |
| ACOS | DOUBLE | ISINFINITE | STRDELETE$ |
| ACOSH | ENVIRON$ | ISNORMAL | STRING |
| ARRAY ADD | EOF | ISODD | STRINSERT |
| ARRAY ARRAYIX | EQV | ISTRUE | STRINSERT$ |
| ARRAY ASSIGN | ERF | ISWIN | STRPTR |
| ARRAY COPY | ERL | JOIN$ | STRREVERSE |
| ARRAY DELETE | ERL$ | LCASE | STRREVERSE$ |
| ARRAY INSERT | ERR | LCASE$ | SWITCH |
| ARRAY REDIM DECR | ERRCLEAR | LEFT | SWITCH$ |
| ARRAY REDIM INCR | ERROR$ | LEFT$ | TAB$ |
| ARRAY REVERSE | EVENT SOURCE | LEN | TALLY |
| ARRAY SCAN | EVENTS | LET *(WITH OBJECTS)* | TAN |
| ARRAY SHUFFLE | EXE | LET *(WITH VARIANTS)* | TANH |
| ARRAY SWAP | EXIST | LINE INPUT# | TCP NOTIFY |
| ARRAY TAGARRAY | EXP | LISTVIEW | THREADCOUNT |
| ARRAY TAGARRAY ERASE | EXP10 | LO | THREADID |
| ARRAY UNIQUE | EXP2 | LOC | TIMER |
| ASIN | EXPM1 | LOF | TREEVIEW |
| ASINH | EXTRACT | LOG | TRIM |
| ATANH | EXTRACT$ | LOG10 | TRIM$ |
| ATN | FILEATTR | LOG1P | TRUNC |
| ATN2 | FILENAME | LOG2 | UCASE |
| BGR | FILENAME$ | LONG | UCASE$ |
| BIN | FIX | LOWRD | UCODE$ |
| BIN$ | FLOOR | LPRINT$ | UCODEPAGE |
| BITS | FONT END | LSET$ | UDP NOTIFY |
| BITS$ | FONT NEW | LTRIM | UNWRAP |
| BITSE | FORMAT | LTRIM$ | UNWRAP$ |
| BUILD | FORMAT$ | MAK | USING |
| BUILD$ | FRAC | MAX | USING$ |
| BYTE | FRE | MCASE | UTF8TOCHR |
| CALLSTK$ | FREEFILE | MCASE$ | UTF8TOCHR$ |
| CALLSTKCOUNT | FUNCNAME | MEMORY FILL | VARPTR |
| CBOOL | FUNCNAME$ | MEMORY FILLS | VERIFY |
| CBRT | GET | MENU ADD POPUP | WAITKEY |
| CBYTE | GET STR | MENU ADD STRING | WAITKEY$ |
| CDBL | GET WSTR | MENU DELETE | WORD |
| CDWORD | GET$ | MENU GET STATE | WRAP |
| CEIL | GET$$ | MENU GET TEXT | WRAP$ |
| CFLT | GETATTR | MENU NEW BAR | WRITE |
| CHOOSE | GRAPHIC ARC | MENU NEW POPUP | WRITE# |
| CHR | GRAPHIC ATTACH | MENU SET STATE | XPRINT ARC |
| CHR$ | GRAPHIC BITMAP END | MENU SET TEXT | XPRINT ATTACH |
| CHRBYTES | GRAPHIC BITMAP LOAD | METRICS | XPRINT BOX |
| CHRTOOEM | GRAPHIC BITMAP NEW | MID | XPRINT CANCEL |
| CHRTOOEM$ | GRAPHIC BOX | MIN | XPRINT CELL |
| CHRTOUTF8 | GRAPHIC CELL | MKBYT | XPRINT CELL SIZE |
| CHRTOUTF8$ | GRAPHIC CELL SIZE | MKCUX | XPRINT CHR SIZE |
| CLIP | GRAPHIC CHR SIZE | MKDWD | XPRINT CLOSE |
| CLIP$ | GRAPHIC CIRCLE | MKE | XPRINT COLOR |
| CLOSE | GRAPHIC CLEAR | MKS | XPRINT COPY |
| CODEPTR | GRAPHIC COLOR | MKWRD | XPRINT ELLIPSE |
| COMM LINE INPUT | GRAPHIC COPY | MOD | XPRINT FORMFEED |
| COMMAND | GRAPHIC DETACH | MONTHNAME | XPRINT GET ATTACH |
| COMMAND$ | GRAPHIC ELLIPSE | MONTHNAME$ | XPRINT GET CANVAS |
| CONTROL ADD BUTTON | GRAPHIC GET BITS | NUL$ | XPRINT GET CLIENT |
| CONTROL ADD CHECKBOX | GRAPHIC GET CANVAS | OCT | XPRINT GET CLIP |
| CONTROL ADD COMBOBOX | GRAPHIC GET CAPTION | OCT$ | XPRINT GET COLLATE |
| CONTROL ADD LABEL | GRAPHIC GET CLIENT | OEMTOCHR | XPRINT GET COLOR |
| CONTROL ADD LISTBOX | GRAPHIC GET CLIP | OEMTOCHR$ | XPRINT GET COLORMODE |
| CONTROL ADD LISTVIEW | GRAPHIC GET DC | ON CALL | XPRINT GET COPIES |
| CONTROL ADD PROGRESSBAR | GRAPHIC GET LINES | ON GOSUB | XPRINT GET DC |
| CONTROL ADD SCROLLBAR | GRAPHIC GET LOC | ON GOTO | XPRINT GET DUPLEX |
| CONTROL ADD TREEVIEW | GRAPHIC GET MIX | OPEN | XPRINT GET LINES |
| CONTROL ADDSTRING | GRAPHIC GET PIXEL | PARSE$ | XPRINT GET MARGIN |
| CONTROL DISABLE | GRAPHIC GET POS | PARSECOUNT | XPRINT GET MIX |
| CONTROL ENABLE | GRAPHIC GET PPI | PATHNAME | XPRINT GET ORIENTATION |
| CONTROL GET CHECK | GRAPHIC GET SCALE | PATHNAME$ | XPRINT GET OVERLAP |
| CONTROL GET TEXT | GRAPHIC GET SIZE | PATHSCAN | XPRINT GET PAGES |
| CONTROL HIDE | GRAPHIC GET STRETCHMODE | PATHSCAN$ | XPRINT GET PAPER |
| CONTROL KILL | GRAPHIC GET TEXTALIGN | PEEK | XPRINT GET PAPERS |
| CONTROL SET CHECK | GRAPHIC GET VIEW | PLAY | XPRINT GET PIXEL |
| CONTROL SET TEXT | GRAPHIC GET WORDWRAP | PLAY SOUND | XPRINT GET POS |
| COS | GRAPHIC GET WRAP | POKE | XPRINT GET PPI |
| COSH | GRAPHIC LINE | PRINTER$ | XPRINT GET QUALITY |
| COT | GRAPHIC PAINT | PRINTERCOUNT | XPRINT GET SCALE |
| COTH | GRAPHIC PIE | PROGRESSBAR | XPRINT GET SELECTION |
| CQUAD | GRAPHIC POLYGON | PUT | XPRINT GET SIZE |
| CSC | GRAPHIC POLYLINE | PUT STR | XPRINT GET STRETCHMODE |
| CSCH | GRAPHIC PRINT | PUT WSTR | XPRINT GET TEXTALIGN |
| CSET$ | GRAPHIC SAVE | PUT$ | XPRINT GET TRAY |
| CSNG | GRAPHIC SCALE | PUT$$ | XPRINT GET TRAYS |
| CSTR | GRAPHIC SCALE PIXELS | QUAD | XPRINT GET WORDWRAP |
| CULNG | GRAPHIC SET AUTOSIZE | RAISEEVENT | XPRINT GET WRAP |
| CURDIR | GRAPHIC SET BITS | READ | XPRINT IMAGELIST |
| CURDIR$ | GRAPHIC SET CAPTION | READ$ | XPRINT LINE |
| CVBYT | GRAPHIC SET CLIP | REDIM | XPRINT PIE |
| CVCUX | GRAPHIC SET FIXED | REMAIN | XPRINT POLYGON |
| CVDWD | GRAPHIC SET FONT | REMAIN$ | XPRINT POLYLINE |
| CVL | GRAPHIC SET MIX | REMOVE | XPRINT PREVIEW |
| CVQ | GRAPHIC SET PIXEL | REMOVE$ | XPRINT PRINT |
| CVW | GRAPHIC SET POS | REPEAT | XPRINT RENDER |
| CWORD | GRAPHIC SET SIZE | REPEAT$ | XPRINT SCALE |
| DATACOUNT | GRAPHIC SET STRETCHMODE | RESOURCE SAVE FILE | XPRINT SET CLIP |
| DAYNAME | GRAPHIC SET TEXTALIGN | RESOURCE$ | XPRINT SET COLLATE |
| DAYNAME$ | GRAPHIC SET VIEW | RETAIN | XPRINT SET COLOR |
| DEC | GRAPHIC SET VIRTUAL | RETAIN$ | XPRINT SET COLORMODE |
| DEC$ | GRAPHIC SET WORDWRAP | RGB | XPRINT SET COPIES |
| DECLARE | GRAPHIC SET WRAP | RIGHT | XPRINT SET DUPLEX |
| DEF | GRAPHIC STYLE | RIGHT$ | XPRINT SET FONT |
| DESKTOP GET PPI | GRAPHIC TEXT SIZE | RND | XPRINT SET MIX |
| DIALOG CENTER | GRAPHIC WIDTH | ROUND | XPRINT SET ORIENTATION |
| DIALOG DOEVENTS | HEADER | RSET$ | XPRINT SET OVERLAP |
| DIALOG END | HEX | RTRIM | XPRINT SET PAGES |
| DIALOG GET SIZE | HEX$ | RTRIM$ | XPRINT SET PAPER |
| DIALOG GET TEXT | HI | SEC | XPRINT SET PIXEL |
| DIALOG NEW | HIWRD | SECH | XPRINT SET POS |
| DIALOG SET SIZE | HYPOT | SEEK | XPRINT SET QUALITY |
| DIALOG SET TEXT | IIF | SETEOF | XPRINT SET STRETCHMODE |
| DIALOG SHOW MODAL | IMAGELIST COUNT | SGN | XPRINT SET TEXTALIGN |
| DIALOG SHOW STATE | IMAGELIST KILL | SHRINK | XPRINT SET TRAY |
| DIM | IMAGELIST NEW | SHRINK$ | XPRINT SET WORDWRAP |
| DIR | IMP | SIN | XPRINT SET WRAP |
| DIR CLOSE | INPUT# | SINGLE | XPRINT SPLIT |
| DIR$ | INSTANCE | SINH | XPRINT STRETCH |
| DISKFREE | INSTR | SIZEOF | XPRINT STYLE |
| DISKSIZE | INT | SPACE | XPRINT TEXT SIZE |
| DISPLAY BROWSE | INTEGER | SPACE$ | XPRINT WIDTH |
