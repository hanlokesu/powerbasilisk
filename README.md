...
**All 374 supported keywords (alphabetical):**
...
| #STACK METASTATEMENT | #TOOLS METASTATEMENT | #UNIQUE METASTATEMENT | #UTILITY METASTATEMENT | ARRAY ADD |
| ARRAY ARRAYIX | ARRAY ASSIGN | ARRAY COPY | ARRAY DELETE | ARRAY INSERT |
| ARRAY REDIM INCR/DECR | ARRAY REVERSE | ARRAY SCAN | ARRAY SELECT | ARRAY SHUFFLE |
| ARRAY SORT | ARRAY SWAP | ARRAY TAGARRAY | ARRAY TAGARRAY ERASE | ARRAY UNIQUE |
| ASC | ASM | ASMDATA / END ASMDATA | BEEP | BIT |
| BIT CALC | CALL | CALL DWORD | CALLSTK | CHDIR |
| CHDRIVE | CLASS/END CLASS | CLIPBOARD | CLOSE | CLS |
| COLOR | COMM CLOSE | COMM LINE | COMM OPEN | COMM PRINT |
| COMM RECV | COMM RESET | COMM SEND | COMM SET | COMM TIMEOUT |
| CSET | DATA | DECLARE | DECR | DESKTOP GET CLIENT |
| DESKTOP GET LOC | DESKTOP GET PPI | DESKTOP GET SIZE | DIM | DIR FUNCTION AND |
| DISPLAY BROWSE | DISPLAY COLOR | DISPLAY FONT | DISPLAY OPENFILE | DISPLAY SAVEFILE |
| END | ENVIRON | ERASE | ERROR | EVENT SOURCE |
| EVENTS | EXIT | FIELD | FILECOPY | FILESCAN |
| FLUSH | FONT END | FONT NEW | FOR / NEXT | FUNCTION / END FUNCTION |
...
| GRAPHIC PIE | GRAPHIC POLYGON | GRAPHIC POLYLINE | GRAPHIC SAVE | GRAPHIC SCALE |
| GRAPHIC SET AUTOSIZE | GRAPHIC SET BITS | GRAPHIC SET CAPTION | GRAPHIC SET CLIP | GRAPHIC SET FIXED |
| GRAPHIC SET FONT | GRAPHIC SET MIX | GRAPHIC SET PIXEL | GRAPHIC SET POS | GRAPHIC SET SIZE |
| GRAPHIC SET STRETCHMODE | GRAPHIC SET TEXTALIGN | GRAPHIC SET VIEW | GRAPHIC SET VIRTUAL | GRAPHIC SET WORDWRAP |
| GRAPHIC SET WRAP | GRAPHIC STYLE | GRAPHIC TEXT SIZE | GRAPHIC WIDTH | HEADER |
| HOST ADDR | HOST NAME | IF | IF/END IF | IMAGELIST |
| IMPORT | INCR | INPUT FLUSH | INPUT# | INSTANCE |
| INTERFACE / END INTERFACE (DIRECT) | INTERFACE/END INTERFACE (IDBIND) | ISINFINITE | ISNORMAL | ITERATE |
| KILL | LET | LET *(WITH OBJECTS)* | LET *(WITH TYPES)* | LET *(WITH VARIANTS)* |
| LINE INPUT# | LOCAL | LOCK | LPRINT | LPRINT ATTACH |
| LPRINT CLOSE | LPRINT FLUSH | LPRINT FORMFEED | LSET | MACRO/END MACRO |
...
| MOUSEPTR | MSGBOX | NAME | OBJECT | ON CALL |
| ON ERROR | ON GOSUB | ON GOTO | OPEN | OPTION EXPLICIT |
| PARSE | PLAY SOUND | PLAY WAVE | PREFIX | PRINT# |
| PROCESS GET PRIORITY | PROCESS SET PRIORITY | PROFILE | PROGRESSBAR | PUT |
| PUT$ | PUT$$ | RAISEEVENT | RANDOMIZE | REDIM |
| REGEXPR | REGISTER | REGREPL | REM | REPLACE |
| RESET | RESOURCE SAVE FILE | RESUME | RETURN | RMDIR |
| ROTATE | RSET | SEEK | SELECT CASE/END SELECT | SETATTR |
| SETEOF | SHELL | SHIFT | SLEEP | SPLIT |
| STATIC | SWAP | TCP ACCEPT | TCP CLOSE | TCP LINE INPUT |
...
| THREAD STATUS | THREAD SUSPEND | THREADED | TIX | TRACE |
| TRY/END TRY | TYPE SET | TYPE/END TYPE | UCODEPAGE | UDP CLOSE |
| UDP NOTIFY | UDP OPEN | UDP RECV | UDP SEND | UNLOCK |
| VAL | WINDOW GET | WINDOW SET | WRITE# | XPRINT ARC |
...
Tests: examples/batch80_test.bas (9/9), official regression 15/15 ALL PASS, 32-bit clang clean, fmt + clippy clean.
...
Tests: examples/batch79_test.bas (6/6), official regression 15/15 ALL PASS, 32-bit clang clean, fmt + clippy clean.
...
- Tests: examples/batch77_test.bas (ALL PASS), official regression 15/15 ALL PASS, fmt + clippy clean, 32+64-bit runtime compile clean.
...
- Tests: examples/batch76_test.bas (ALL PASS), official regression 15/15 ALL PASS, fmt + clippy clean, 32+64-bit runtime compile clean.
...
- Tests: examples/batch67_test.bas (4/4 — LONG, SINGLE, BYTE, QUAD all verified), official regression 15/15 ALL PASS, fmt + clippy clean.
