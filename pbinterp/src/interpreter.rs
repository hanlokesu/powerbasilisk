...
impl Interpreter {
    ...
    fn exec_statement(&mut self, stmt: &Statement) -> PbResult<Flow> {
        match stmt {
            ...
            Statement::Regexpr { .. } | Statement::Regrepl { .. } => {
                // REGEXPR/REGREPL are codegen-only in the interpreter
                Ok(Flow::Normal)
            }
            Statement::Profile(_) => {
            ...
            Statement::InputConsole(_) => Ok(Flow::Normal),
            Statement::LineInputConsole(_) => Ok(Flow::Normal),
